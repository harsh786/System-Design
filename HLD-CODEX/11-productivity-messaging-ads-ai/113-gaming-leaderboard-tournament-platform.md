# Gaming Leaderboard / Tournament Platform - Complete System Design
...
| --- | --- | --- |
| Source of truth / primary store | Redis Sorted Sets/Aerospike for live leaderboard reads plus Cassandra/ScyllaDB/DynamoDB for durable score events and PostgreSQL for tournament metadata/rules | leaderboards need very fast rank updates and reads, but final results require durable replayable score events and auditable rules |
| Hot serving / cache | Redis Cluster sorted sets partitioned by tournament/season/bucket | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
| Event stream / outbox | Kafka/Pulsar for score ingestion, anti-cheat, ranking recomputation, and rewards | decouples projections, notifications, analytics, search indexing, and recovery from the write path |
| Search / analytics | ClickHouse/Druid for score analytics, anti-cheat features, and historical leaderboards | serves text/filter/OLAP queries without overloading transactional tables |
| Large immutable payloads | object storage for match proofs, replay files, and exports | large or immutable data is cheaper, durable, and easier to lifecycle outside OLTP rows |
...
- Primary partition key: `tournament_id/season_id + bucket; player_id for history; time bucket for events`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `tournament_id + player_id / score_event_id` should be single-partition whenever possible.
...
- Hot partition mitigation: bucket giant leaderboards, use top-K materialization, and recompute authoritative ranks from durable events.
...
- Required secondary indexes: `player_id + created_at, tournament_id + score, anti_cheat_state, reward_state`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for final scores/rewards | strong consistency for score acceptance, tournament rules, reward issuance, and final rank publication | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for live leaderboard views that can be corrected | Eventual consistency for live ranks, anti-cheat features, notifications, and analytics | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
