...
public class Problem42_DesignSkiplist {
    // LC 1206: Design Skiplist
    static final int MAX_LEVEL = 16;
    static final double P = 0.5;
    static Random rand = new Random();

    static class Node { int val; Node[] next; Node(int v, int level) { val = v; next = new Node[level + 1]; } }

    Node head = new Node(Integer.MIN_VALUE, MAX_LEVEL);
    int level = 0;

    int randomLevel() { int lvl = 0; while (rand.nextDouble() < P && lvl < MAX_LEVEL) lvl++; return lvl; }
    ...
    public static void main(String[] args) {
        Problem42_DesignSkiplist sl = new Problem42_DesignSkiplist();
        sl.add(1); sl.add(2); sl.add(3);
        System.out.println(sl.search(0)); // false
        sl.add(4);
        System.out.println(sl.search(1)); // true
        System.out.println(sl.erase(0)); // false
        System.out.println(sl.erase(1)); // true
        System.out.println(sl.search(1)); // false
    }
}
