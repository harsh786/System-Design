# Design shopping cart - System Design Deep Dive

**Problem #62**  
**Category:** Commerce  
**Primary pattern:** commerce booking  
**Deep-dive focus:** session state, price changes, inventory holds, checkout
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the shopping cart service boundary.
...
| Core domain services | Cart or quote service, Inventory availability service, Reservation/hold service, Order/booking orchestrator, Catalog/search service | Own shopping cart business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **session state**: Make session state explicit in the data model, APIs, scaling plan, and operational dashboards.
- **price changes**: Snapshot price/quote at checkout and define expiry.
- **inventory holds**: Use atomic hold creation with expiration and release workers.
- **checkout**: Orchestrate checkout with saga steps and compensating actions.
...
- **Hot path clarity:** start from `session state` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `session state`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `session state`.
