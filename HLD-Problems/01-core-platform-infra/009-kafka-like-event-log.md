# Design a Kafka-like event log - System Design Deep Dive

**Problem #9**  
**Category:** Streaming  
**Primary pattern:** core platform  
**Deep-dive focus:** partitions, replication, offsets, retention, consumer groups

## 0. Interview Framing

A platform primitive where correctness, low latency, predictable scaling, and operability matter more than product breadth.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **partitions**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Append ordered events to partitions.
- Track consumer offsets and retention.
- Replicate partitions for durability.
- Support replay, compaction, and consumer groups.
...
```http
GET /v1/topics?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/topics/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
GET /v1/topics/{id}/metrics?from=2026-05-25T00:00:00Z&to=2026-05-25T01:00:00Z
```

```http
POST /v1/topics/{id}/admin/quarantine
Idempotency-Key: <uuid>
...
```protobuf
service TopicPartitionService {
  rpc CreateTopicPartition(CreateTopicPartitionRequest) returns (TopicPartition);
  rpc GetTopicPartition(GetTopicPartitionRequest) returns (TopicPartition);
  rpc UpdateTopicPartition(UpdateTopicPartitionRequest) returns (TopicPartition);
  rpc EvaluateTopicPartition(EvaluateTopicPartitionRequest) returns (EvaluateTopicPartitionResponse);
  rpc ListTopicPartitionEvents(ListTopicPartitionEventsRequest) returns (stream TopicPartitionEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "topics.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "topics-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "topicPartition_123",
...
- `topics.created.v1`
- `topics.updated.v1`
- `topics.state_changed.v1`
- `topics.deleted_or_expired.v1`
- `topics.policy_denied.v1`
- `topics.operation_failed.v1`
...
```text
a Kafka-like event log Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Kafka-like event log service boundary.
...
| Core domain services | Hot-path data-plane service, Control-plane API, Shard manager, Policy/config cache | Own a Kafka-like event log business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `TopicPartitionController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `TopicPartitionApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `TopicPartitionDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `TopicPartitionRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `TopicPartitionPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `TopicPartitionEventPublisher`: writes outbox records and publishes domain events to the broker.
- `TopicPartitionReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface TopicPartitionRepository {
    Optional<TopicPartition> findById(TopicPartitionId id, ReadConsistency consistency);
    Page<TopicPartition> list(TopicPartitionQuery query, Cursor cursor, int limit);
    TopicPartition save(TopicPartition aggregate, ExpectedVersion expectedVersion);
}

interface TopicPartitionPolicyEvaluator {
    PolicyDecision canRead(Principal principal, TopicPartition resource);
    PolicyDecision canMutate(Principal principal, TopicPartition resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> TopicPartition
  -> emits -> TopicPartitionEvent
...
|---|---|
| `topics` | `id, tenant_id, name, partition_count, retention_policy, compaction_policy, state` |
| `partitions` | `topic_id, partition_id, leader_broker, replicas, high_watermark, state` |
| `segments` | `topic_id, partition_id, segment_id, offset_start, offset_end, object_ref, checksum` |
| `consumer_offsets` | `group_id, topic_id, partition_id, committed_offset, committed_at` |
...
| --- | --- | --- |
| Source of truth / primary store | replicated append-only log segments on local disks/object storage plus KRaft/Raft metadata quorum for topics, partitions, ISR, and controller state | the core database is the log; metadata quorum prevents split-brain leadership and partition ownership |
| Hot serving / cache | page cache and broker memory for hot segment reads; compacted topics for small lookup streams | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
| Search / analytics | ClickHouse/Druid/Pinot/Snowflake for analytical consumption, not broker serving | serves text/filter/OLAP queries without overloading transactional tables |
| Large immutable payloads | tiered object storage for cold log segments and long retention | large or immutable data is cheaper, durable, and easier to lifecycle outside OLTP rows |
...
- Primary partition key: `topic + partition; key hash chooses partition`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `topic + partition + offset` should be single-partition whenever possible.
...
- Hot partition mitigation: increase partitions, isolate hot topics, throttle producers, and use key design that distributes writes.
...
- Required secondary indexes: `consumer_group + topic + partition, producer_id + sequence, transaction_id`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for partition leadership and committed offsets under quorum | leader-based per-partition ordering with acks=all and min.insync.replicas for durable commits | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP-like continued reads from replicas are only allowed when staleness semantics are explicit | Eventual consistency for consumer projections, search/warehouse sinks, lag dashboards, and compacted-view freshness | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **partitions**: Partition by key for parallelism while preserving ordering where required.
- **replication**: Use replication for availability, read scale, and durability; state the consistency trade-off.
- **offsets**: Store offsets outside processing code paths and support replay from checkpoints.
- **retention**: Separate hot retention, compliance retention, and analytics retention.
- **consumer groups**: Use consumer groups for scalable parallel consumption while preserving per-partition order.
...
- **Hot path clarity:** start from `partitions` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `topics.created.v1, topics.updated.v1, topics.state_changed.v1, topics.deleted_or_expired.v1, topics.policy_denied.v1, topics.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `partitions`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `partitions`.
