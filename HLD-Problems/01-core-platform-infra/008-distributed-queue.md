# Design a distributed queue - System Design Deep Dive

**Problem #8**  
**Category:** Messaging  
**Primary pattern:** core platform  
**Deep-dive focus:** visibility timeout, retries, DLQ, priority, ordering

## 0. Interview Framing

A platform primitive where correctness, low latency, predictable scaling, and operability matter more than product breadth.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **visibility timeout**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Enqueue messages durably.
- Deliver messages with visibility timeout and retries.
- Support DLQ, priority, and delayed delivery.
- Expose queue depth and consumer lag.
...
```http
GET /v1/queue-messages/{id}
Authorization: Bearer <token>
```

```http
GET /v1/queue-messages?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```

```http
PATCH /v1/queue-messages/{id}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/queue-messages/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/queue-messages/evaluate
Idempotency-Key: <uuid>
...
```http
GET /v1/queue-messages/{id}/metrics?from=2026-05-25T00:00:00Z&to=2026-05-25T01:00:00Z
```

```http
POST /v1/queue-messages/{id}/admin/quarantine
Idempotency-Key: <uuid>
...
service QueueMessageService {
  rpc CreateQueueMessage(CreateQueueMessageRequest) returns (QueueMessage);
  rpc GetQueueMessage(GetQueueMessageRequest) returns (QueueMessage);
  rpc UpdateQueueMessage(UpdateQueueMessageRequest) returns (QueueMessage);
  rpc EvaluateQueueMessage(EvaluateQueueMessageRequest) returns (EvaluateQueueMessageResponse);
  rpc ListQueueMessageEvents(ListQueueMessageEventsRequest) returns (stream QueueMessageEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "queue.messages.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "queue-messages-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "queueMessage_123",
...
- `queue.messages.created.v1`
- `queue.messages.updated.v1`
- `queue.messages.state_changed.v1`
- `queue.messages.deleted_or_expired.v1`
- `queue.messages.policy_denied.v1`
- `queue.messages.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the distributed queue service boundary.
...
| Core domain services | Hot-path data-plane service, Control-plane API, Shard manager, Policy/config cache | Own a distributed queue business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `QueueMessageController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `QueueMessageApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `QueueMessageDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `QueueMessageRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `QueueMessagePolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `QueueMessageEventPublisher`: writes outbox records and publishes domain events to the broker.
- `QueueMessageReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface QueueMessageRepository {
    Optional<QueueMessage> findById(QueueMessageId id, ReadConsistency consistency);
    Page<QueueMessage> list(QueueMessageQuery query, Cursor cursor, int limit);
    QueueMessage save(QueueMessage aggregate, ExpectedVersion expectedVersion);
}

interface QueueMessagePolicyEvaluator {
    PolicyDecision canRead(Principal principal, QueueMessage resource);
    PolicyDecision canMutate(Principal principal, QueueMessage resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> QueueMessage
  -> emits -> QueueMessageEvent
...
|---|---|
| `queues` | `id, tenant_id, name, visibility_timeout_sec, retention_sec, dlq_id, priority_policy` |
| `messages` | `id, queue_id, partition_key, payload_ref, visible_at, receive_count, state, created_at` |
| `leases` | `message_id, consumer_id, lease_until, fencing_token, attempt` |
| `dead_letter_messages` | `id, tenant_id, queuemessage_ref, state, version, created_at, updated_at` |
...
| --- | --- | --- |
| Source of truth / primary store | replicated append-only segment store for messages plus PostgreSQL/CockroachDB or Raft metadata for queues, leases, and consumer state | messages need durable ordered append and replay; queue metadata needs transactional state transitions |
| Hot serving / cache | Redis only for ephemeral rate limits or scheduling wheels, not as the durable queue source of truth | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
- Primary partition key: `queue_name + partition_id; message_id/time bucket for history`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `queue_name + partition + offset/message_id` should be single-partition whenever possible.
...
- Hot partition mitigation: split hot queues into partitions, isolate tenants, and use priority/dead-letter queues.
...
- Required secondary indexes: `visibility_deadline, state + next_attempt_at, producer_id + idempotency_key`.
...
| --- | --- | --- | --- |
| Canonical command path | CP per partition for no-lost-message guarantees | strong consistency for enqueue, lease, ack, and visibility-timeout state per partition | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP only for approximate metrics and non-authoritative queue depth estimates | Eventual consistency for metrics, queue dashboards, delayed analytics, and consumer lag summaries | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **visibility timeout**: Lease messages during processing and make handlers idempotent because duplicate delivery is expected.
- **retries**: Use bounded retries with jitter, DLQs, poison-message handling, and replay tooling.
- **dlq**: Treat DLQ as an operational workflow with ownership, alerts, and redrive policy.
- **priority**: Separate priority queues or scheduling weights so low-priority work cannot starve critical work.
- **ordering**: Define the exact ordering scope, usually per key, conversation, partition, or aggregate.
...
- **Hot path clarity:** start from `visibility timeout` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `queue.messages.created.v1, queue.messages.updated.v1, queue.messages.state_changed.v1, queue.messages.deleted_or_expired.v1, queue.messages.policy_denied.v1, queue.messages.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `visibility timeout`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `visibility timeout`.
