# Design a rate limiter for APIs - System Design Deep Dive

**Problem #3**  
**Category:** Edge/platform  
**Primary pattern:** core platform  
**Deep-dive focus:** token bucket, sliding window, distributed counters, fairness

## 0. Interview Framing

A platform primitive where correctness, low latency, predictable scaling, and operability matter more than product breadth.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **token bucket**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Define global, tenant, user, API-key, and route limits.
- Return allow/deny decisions in the request hot path.
- Support burst and sustained quotas.
- Expose remaining quota and retry-after metadata.
...
```http
GET /v1/rate-limits?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```

```http
PATCH /v1/rate-limits/{id}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/rate-limits/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
GET /v1/rate-limits/{id}/metrics?from=2026-05-25T00:00:00Z&to=2026-05-25T01:00:00Z
```

```http
POST /v1/rate-limits/{id}/admin/quarantine
Idempotency-Key: <uuid>
...
```protobuf
service RateLimitRuleService {
  rpc CreateRateLimitRule(CreateRateLimitRuleRequest) returns (RateLimitRule);
  rpc GetRateLimitRule(GetRateLimitRuleRequest) returns (RateLimitRule);
  rpc UpdateRateLimitRule(UpdateRateLimitRuleRequest) returns (RateLimitRule);
  rpc EvaluateRateLimitRule(EvaluateRateLimitRuleRequest) returns (EvaluateRateLimitRuleResponse);
  rpc ListRateLimitRuleEvents(ListRateLimitRuleEventsRequest) returns (stream RateLimitRuleEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "rate.limits.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "rate-limits-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "rateLimitRule_123",
...
- `rate.limits.created.v1`
- `rate.limits.updated.v1`
- `rate.limits.state_changed.v1`
- `rate.limits.deleted_or_expired.v1`
- `rate.limits.policy_denied.v1`
- `rate.limits.operation_failed.v1`
...
```text
a rate limiter for APIs Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the rate limiter for APIs service boundary.
...
| Core domain services | Hot-path data-plane service, Control-plane API, Shard manager, Policy/config cache | Own a rate limiter for APIs business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `RateLimitRuleController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `RateLimitRuleApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `RateLimitRuleDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `RateLimitRuleRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `RateLimitRulePolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `RateLimitRuleEventPublisher`: writes outbox records and publishes domain events to the broker.
- `RateLimitRuleReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface RateLimitRuleRepository {
    Optional<RateLimitRule> findById(RateLimitRuleId id, ReadConsistency consistency);
    Page<RateLimitRule> list(RateLimitRuleQuery query, Cursor cursor, int limit);
    RateLimitRule save(RateLimitRule aggregate, ExpectedVersion expectedVersion);
}

interface RateLimitRulePolicyEvaluator {
    PolicyDecision canRead(Principal principal, RateLimitRule resource);
    PolicyDecision canMutate(Principal principal, RateLimitRule resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> RateLimitRule
  -> emits -> RateLimitRuleEvent
...
|---|---|
| `rate_limit_rules` | `id, tenant_id, subject_type, route_pattern, algorithm, limit_value, window_sec, burst, state` |
| `rate_limit_counters` | `rule_id, subject_id, window_start, shard_id, tokens_or_count, updated_at` |
| `quota_subjects` | `tenant_id, subject_id, attributes_json, risk_score, state` |
| `decision_logs` | `request_id, rule_id, subject_id, decision, remaining, retry_after_ms, ts` |
...
| --- | --- | --- |
| Source of truth / primary store | Redis Cluster/KeyDB/Aerospike for online counters; Cassandra/ClickHouse for decision logs; PostgreSQL for policies and tenants | online limit checks require atomic low-latency counters, while policy metadata and audit logs have different access patterns |
| Hot serving / cache | local in-process cache for policies plus Redis Cluster for counters and token buckets | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
- Primary partition key: `tenant_id + limiter_key hash + time window`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `limiter_key + window_start` should be single-partition whenever possible.
...
- Hot partition mitigation: salt hot limiter keys, use hierarchical limits, local prefetch tokens, and shed abusive callers at the edge.
...
- Required secondary indexes: `tenant_id + policy_id, route + policy_id, decision + time_bucket`.
...
| --- | --- | --- | --- |
| Canonical command path | CP per limiter key when correctness matters; bounded local fallback when dependency is degraded | atomic per-key consistency for counter updates within a window | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP fail-open/fail-closed policy is explicit per route and risk tier | Eventual consistency for usage analytics, customer dashboards, billing reports, and anomaly detection | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **token bucket**: Use token bucket for burst tolerance and refill-rate fairness.
- **sliding window**: Use sliding log/counter for stricter rolling-window semantics at higher storage/write cost.
- **distributed counters**: Use local admission plus global reconciliation for extreme scale; use strong counters only for small critical quotas.
- **fairness**: Partition limits by tenant, API key, user, route, and region to avoid noisy neighbors.
...
- **Hot path clarity:** start from `token bucket` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `rate.limits.created.v1, rate.limits.updated.v1, rate.limits.state_changed.v1, rate.limits.deleted_or_expired.v1, rate.limits.policy_denied.v1, rate.limits.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `token bucket`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `token bucket`.
