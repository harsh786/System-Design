# Design a CDN and static asset platform - System Design Deep Dive

**Problem #6**  
**Category:** Edge  
**Primary pattern:** core platform  
**Deep-dive focus:** edge cache, origin shield, invalidation, signed URLs, geo routing

## 0. Interview Framing

A platform primitive where correctness, low latency, predictable scaling, and operability matter more than product breadth.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **edge cache**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Serve static and cacheable dynamic content from edge locations.
- Invalidate or version assets safely.
- Protect private content with signed URLs/cookies.
- Shield origins from spikes and DDoS.
...
```http
GET /v1/assets?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/assets/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
GET /v1/assets/{id}/metrics?from=2026-05-25T00:00:00Z&to=2026-05-25T01:00:00Z
```

```http
POST /v1/assets/{id}/admin/quarantine
Idempotency-Key: <uuid>
...
service AssetService {
  ...
  rpc EvaluateAsset(EvaluateAssetRequest) returns (EvaluateAssetResponse);
  rpc ListAssetEvents(ListAssetEventsRequest) returns (stream AssetEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "assets.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "assets-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "asset_123",
...
- `assets.created.v1`
- `assets.updated.v1`
- `assets.state_changed.v1`
- `assets.deleted_or_expired.v1`
- `assets.policy_denied.v1`
- `assets.operation_failed.v1`
...
```text
a CDN and static asset platform Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the CDN and static asset platform service boundary.
...
| Core domain services | Hot-path data-plane service, Control-plane API, Shard manager, Policy/config cache | Own a CDN and static asset platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `AssetController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `AssetApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `AssetDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `AssetRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `AssetPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `AssetEventPublisher`: writes outbox records and publishes domain events to the broker.
- `AssetReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface AssetRepository {
    Optional<Asset> findById(AssetId id, ReadConsistency consistency);
    Page<Asset> list(AssetQuery query, Cursor cursor, int limit);
    Asset save(Asset aggregate, ExpectedVersion expectedVersion);
}

interface AssetPolicyEvaluator {
    PolicyDecision canRead(Principal principal, Asset resource);
    PolicyDecision canMutate(Principal principal, Asset resource, Action action);
}
...
|---|---|
| `assets` | `id, owner_id, origin_path, cache_key, content_hash, ttl_sec, visibility, state` |
| `cache_policies` | `id, path_pattern, ttl_sec, vary_headers, vary_query, signed_url_required, state` |
| `origin_configs` | `id, origin_url, shield_region, timeout_ms, retry_policy, failover_origin_id` |
| `invalidation_jobs` | `id, tenant_id, asset_ref, state, version, created_at, updated_at` |
...
| --- | --- | --- |
| Source of truth / primary store | CockroachDB/Spanner/PostgreSQL for asset metadata, purge jobs, signing policies, and tenant config; object storage for origin assets | metadata and purge state need auditable transactions, while bytes are served from object/origin storage and edge caches |
| Hot serving / cache | edge KV/cache plus Redis for purge state, signed URL validation, and origin shield coordination | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
- Primary partition key: `tenant_id + asset_key hash; region/edge_pop for cache metadata; time bucket for logs`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `asset_key / cache_key / purge_id` should be single-partition whenever possible.
...
- Hot partition mitigation: use hierarchical cache, origin shield, request coalescing, and cache-key normalization for viral objects.
...
- Required secondary indexes: `tenant_id + created_at, hostname + path, cache_policy_id, purge_state + updated_at`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for signing policy, purge command acceptance, and tenant configuration | strong consistency for source-of-truth writes and policy gates | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP for edge serving with bounded stale windows and explicit purge/TTL semantics | Eventual consistency for cache invalidation propagation, logs, analytics, usage billing, and search over assets | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **edge cache**: Push cacheable reads to edge, define cache keys carefully, and keep private data out of shared caches.
- **origin shield**: Use regional origin shield and request coalescing to protect origins from miss storms.
- **invalidation**: Prefer versioned URLs for immutable assets; use explicit invalidation for emergency rollback/takedown.
- **signed urls**: Use short-lived signed URLs/cookies for private assets and validate at the edge or origin.
- **geo routing**: Route by latency, availability, legal residency, and data locality, not just nearest region.
...
- **Hot path clarity:** start from `edge cache` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `assets.created.v1, assets.updated.v1, assets.state_changed.v1, assets.deleted_or_expired.v1, assets.policy_denied.v1, assets.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `edge cache`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `edge cache`.
