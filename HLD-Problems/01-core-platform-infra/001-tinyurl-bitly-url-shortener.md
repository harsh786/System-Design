...
- Create short links with optional custom aliases and TTL.
...
```http
GET /v1/links?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/links/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
GET /v1/links/{id}/metrics?from=2026-05-25T00:00:00Z&to=2026-05-25T01:00:00Z
```

```http
POST /v1/links/{id}/admin/quarantine
Idempotency-Key: <uuid>
...
service ShortLinkService {
  ...
  rpc EvaluateShortLink(EvaluateShortLinkRequest) returns (EvaluateShortLinkResponse);
...
{
  "event_id": "evt_01H...",
  "event_type": "links.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "links-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "shortLink_123",
...
- `links.created.v1`
- `links.updated.v1`
- `links.state_changed.v1`
- `links.deleted_or_expired.v1`
- `links.policy_denied.v1`
- `links.operation_failed.v1`
...
Data Stores: Short-link metadata DB + redirect cache + click event log + abuse/quarantine store + analytics warehouse
Ops/Integrations: Abuse scanner + link preview/safety checks + analytics rollups + admin quarantine/replay
```

### Request And Data Flow

1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the TinyURL / Bitly URL shortener service boundary.
...
| Core domain services | Hot-path data-plane service, Control-plane API, Shard manager, Policy/config cache | Own TinyURL / Bitly URL shortener business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `ShortLinkController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `ShortLinkApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `ShortLinkDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `ShortLinkRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `ShortLinkPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `ShortLinkEventPublisher`: writes outbox records and publishes domain events to the broker.
- `ShortLinkReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface ShortLinkRepository {
    Optional<ShortLink> findById(ShortLinkId id, ReadConsistency consistency);
    Page<ShortLink> list(ShortLinkQuery query, Cursor cursor, int limit);
    ShortLink save(ShortLink aggregate, ExpectedVersion expectedVersion);
}

interface ShortLinkPolicyEvaluator {
    PolicyDecision canRead(Principal principal, ShortLink resource);
    PolicyDecision canMutate(Principal principal, ShortLink resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> ShortLink
  -> emits -> ShortLinkEvent
...
|---|---|
| `links` | `id, alias, long_url_hash, long_url_encrypted, owner_id, domain_id, ttl_at, status, created_at` |
| `custom_domains` | `id, owner_id, hostname, tls_status, verification_token, created_at` |
| `redirect_events` | `alias, ts_bucket, country, device, referrer_hash, request_id` |
...
| --- | --- | --- |
| Source of truth / primary store | DynamoDB/Cassandra/Bigtable/ScyllaDB for alias -> URL redirect lookup; PostgreSQL for accounts, custom domains, abuse cases, and admin metadata | redirects are key-value reads at huge scale, while account/domain/admin data needs relational constraints |
| Hot serving / cache | CDN edge cache plus Redis Cluster for hottest aliases and abuse/rate-limit state | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
- Primary partition key: `alias hash with virtual shards; tenant_id for admin/account tables`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `alias / short_code` should be single-partition whenever possible.
...
- Hot partition mitigation: cache viral links at edge, split analytics writes by alias hash + time bucket, and rate-limit abusive aliases.
...
- Required secondary indexes: `owner_id + created_at, custom_domain + alias, ttl_at, abuse_status, long_url_hash`.
...
- **key generation**: Keep ID generation separate from request handling; use preallocated ranges, Snowflake-style IDs, or collision-checked random IDs based on ordering and privacy needs.
...
- **abuse prevention**: Combine rate limits, reputation, content scanning, user reports, and fast takedown workflows.
...
- **Hot path clarity:** start from `key generation` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `links.created.v1, links.updated.v1, links.state_changed.v1, links.deleted_or_expired.v1, links.policy_denied.v1, links.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `key generation`, and what exact write makes it durable?
