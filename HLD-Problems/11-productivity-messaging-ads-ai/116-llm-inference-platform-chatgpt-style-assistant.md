# Design LLM Inference Platform / ChatGPT-Style Assistant Backend - System Design Deep Dive

**Problem #116**  
**Category:** AI platform/realtime  
**Primary pattern:** model gateway + context + tools + safety  
**Deep-dive focus:** streaming inference, context, tools, retrieval, safety, quotas, cost

## 0. Interview Framing

An assistant backend manages conversations, context, streaming, tool calls, retrieval, safety, quotas, and cost across models and providers.

Actors: end users, applications, tool services, model providers, safety/ops. The highest-risk path is **chat request -> safety/quota -> context -> model stream -> tool loop -> usage**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Create conversations, messages, streaming responses, attachments, files/images, tools, retrieval, and history.
- Route requests by model capability, latency, cost, region, tenant policy, and availability.
- Assemble prompts from system/developer/user messages, memory, retrieval chunks, tool schemas, and safety instructions.
- Execute tools with authorization, sandboxing, timeout, redaction, and audit.
- Track tokens, latency, safety, feedback, and billing/quotas.
- Expose support/admin operations to inspect, replay, correct, and annotate completion_request history.
...
- Streaming first-token latency and total latency are predictable.
- Tenant data isolation and tool authorization are mandatory.
- Safety cannot be bypassed by prompt injection or tool output.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: completion_requests plus model_invocations/tool_calls; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| Users | 10M DAU |
| Requests | 100M/day |
| Tokens | Billions-trillions/day |
| Tool calls | 0-10 per turn |
| Cost | GPU/model tokens dominate |
...
- Keep the online completion_request aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/conversations/{id}/messages | messages, model, tools, stream | Chat turn. |
| POST /v1/files | file, purpose, tenant | Upload. |
| POST /v1/retrieval/query | conversation_id, query, acl | Retrieve chunks. |
| GET /v1/usage | tenant_id, date_range | Usage/cost. |
| GET /v1/completion-requests/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/completion-requests/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service LlmInferenceService {
  rpc Create(CreateLlmInferenceRequest) returns (LlmInference);
  rpc Get(GetLlmInferenceRequest) returns (LlmInference);
  rpc Transition(TransitionLlmInferenceRequest) returns (LlmInference);
  rpc Reconcile(ReconcileLlmInferenceRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "llm_inference.state_changed.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "llm_inference-service",
  "tenant_or_region_id": "region_123",
  "aggregate_id": "completion_request_123",
...
- llm_inference.created.v1
- llm_inference.validated.v1
- llm_inference.state_changed.v1
- llm_inference.committed.v1
- llm_inference.failed.v1
- llm_inference.reversed.v1
- llm_inference.reconciliation_completed.v1
- llm_inference.manual_review_required.v1
...
```text
LLM Inference Platform / ChatGPT-Style Assistant Backend Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Model gateway -> Conversation service -> Context builder -> Tool orchestrator -> Safety service -> Idempotency service
        |       -> Source-of-Truth Write + Transactional Outbox
        |
        +--> Query / Serving Path
        |       -> Conversation service / Retrieval service
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the LLM Inference Platform / ChatGPT-Style Assistant Backend service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Model gateway -> Conversation service -> Context builder -> Tool orchestrator -> Safety service -> Idempotency service; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
3. **Query path:** read-heavy traffic is served by Conversation service / Retrieval service; strong reads fall back to the canonical store when stale projections are unsafe.
...
- **Model gateway**: auth, quota, routing, streaming, usage.
- **Conversation service**: messages, attachments, summaries, metadata.
- **Context builder**: history, summaries, retrieval, tool schemas, token budget.
- **Tool orchestrator**: scoped credentials, timeout, sandbox, audit.
- **Retrieval service**: permission-filtered chunks.
- **Safety service**: input/output/tool policy.
...
| Core domain services | Model gateway, Conversation service, Context builder, Tool orchestrator, Safety service, Idempotency service | Own LLM Inference Platform / ChatGPT-Style Assistant Backend business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Conversation service, Retrieval service | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
...
- **Inference request orchestrator**: owns completion_request state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed completion_request transitions using expected_version, policy decisions, and source-of-truth reads.
...
- LLM Inference Platform / ChatGPT-Style Assistant Backend: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| conversations | conversation_id, tenant_id, user_id, state | Conversation. |
| messages | message_id, conversation_id, role, content_ref, token_count | Message. |
| completion_requests | request_id, conversation_id, model, state, usage | Request. |
| model_invocations | invocation_id, request_id, provider, latency, tokens | Model call. |
| tool_calls | tool_call_id, request_id, tool_name, state, auth_scope | Tool. |
| retrieval_chunks | chunk_id, document_id, embedding_ref, acl_hash | RAG. |
| safety_decisions | decision_id, request_id, stage, action | Safety. |
...
| --- | --- | --- |
| Source of truth / primary store | PostgreSQL/CockroachDB for tenants, users, conversations, tool grants, quotas, and billing metadata; vector database for embeddings; object storage for transcripts, files, prompts, and model artifacts | identity, quota, billing, and tool permissions require transactions while retrieval and artifacts use specialized stores |
| Hot serving / cache | Redis for conversation session cache, rate limits, token budgets, tool-call idempotency, and streaming state | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
| Event stream / outbox | Kafka/Pulsar for inference events, audit, safety review, feedback, billing, and offline eval pipelines | decouples projections, notifications, analytics, search indexing, and recovery from the write path |
| Search / analytics | Vector DB/ANN index for retrieval; OpenSearch for transcript/admin search; ClickHouse/Snowflake for usage analytics | serves text/filter/OLAP queries without overloading transactional tables |
| Large immutable payloads | object storage for uploaded files, chunked documents, transcripts, eval datasets, prompt/model artifacts, and logs with retention policy | large or immutable data is cheaper, durable, and easier to lifecycle outside OLTP rows |
...
- Primary partition key: `tenant_id + conversation_id/user_id; embedding collection + document_id; time bucket for inference events`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `conversation_id/message_id/request_id/document_id` should be single-partition whenever possible.
...
- Hot partition mitigation: shard large tenants, cache retrieval results, enforce per-model queues, and isolate long-running tool calls.
...
- Required secondary indexes: `tenant_id + created_at, user_id + updated_at, tool_call_id, safety_state, idempotency_key`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for quota, billing, and permission gates before model/tool execution | strong consistency for quotas, billing, tool permissions, safety blocks, and conversation append acceptance | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for retrieval/search indexes and analytics with visible freshness limits | Eventual consistency for vector index freshness, analytics, feedback aggregation, evals, and search over transcripts | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
1. Chat request authenticates, checks quota, classifies safety, builds context, and streams model output.
2. Tool call validates permission, executes tool, redacts result, appends tool message, and resumes model.
3. Retrieval filters by ACL and records citations/lineage.
4. Usage finalization records tokens, cache hits, latency, model/provider cost.
...
- Context management summarizes old turns and drops low-value content.
- Tool calls are security boundaries; model intent is not authorization.
- Prompt/result caching lowers cost where privacy permits.
- Source-of-truth boundary: completion_requests plus model_invocations/tool_calls owns correctness; all low-latency read models are disposable projections.
...
- Protect conversation content, files, embeddings, tool credentials, and usage.
- Detect prompt injection, data exfiltration, jailbreaks, tool abuse, credential leakage, and spam.
...
- SLIs: completion_request mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do you stream while applying safety?
- How do tool permissions work?
- How do you control GPU/token cost?
- Where exactly is completion_request source of truth stored?
...
- **Correctness boundary:** completion_requests plus model_invocations/tool_calls is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `chat request -> safety/quota -> context -> model stream -> tool loop -> usage` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Model gateway, Conversation service, Context builder, Tool orchestrator, Retrieval service, Safety service; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `conversations, messages, completion_requests, model_invocations, tool_calls, retrieval_chunks` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `llm_inference.created.v1, llm_inference.validated.v1, llm_inference.state_changed.v1, llm_inference.committed.v1, llm_inference.failed.v1, llm_inference.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `chat request -> safety/quota -> context -> model stream -> tool loop -> usage`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `chat request -> safety/quota -> context -> model stream -> tool loop -> usage`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Inference request orchestrator, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
