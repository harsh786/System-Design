# Design Google Calendar / Calendar Scheduling - System Design Deep Dive

**Problem #107**  
**Category:** Productivity/collaboration  
**Primary pattern:** event store + recurrence + free/busy + sync  
**Deep-dive focus:** recurrence, attendees, reminders, free/busy, sync tokens

## 0. Interview Framing

Calendar systems combine event storage, recurrence, attendee state, reminders, free/busy, and multi-device sync.

Actors: users, organizers, attendees, rooms, clients, external calendars. The highest-risk path is **create event -> invite -> expand recurrence -> notify -> sync clients**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Create calendars/events with recurrence, exceptions, attendees, resources, reminders, conferencing, attachments, and visibility.
- Support RSVP, updates, cancellations, delegation, sharing permissions, and room booking.
- Provide incremental sync for mobile/web and external CalDAV/ICS integrations.
- Send invitation updates and reminders with dedupe.
- Expose support/admin operations to inspect, replay, correct, and annotate event history.
...
- Recurring expansion is correct and bounded.
- Free/busy is low-latency and privacy-preserving.
- Sync tokens do not miss ordered changes.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Event service plus per-calendar change log; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| Users | 1B |
| Events | Billions |
| Free/busy | High scheduling QPS |
| Reminders | Massive time-bucket fanout |
| Devices | Multiple per user |
...
- Keep the online event aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/calendars/{id}/events | start, end, attendees, rrule, reminders | Create event. |
| PATCH /v1/events/{id} | expected_version, changes, recurrence_scope | Update. |
| POST /v1/freebusy | users/resources, time_range | Availability. |
| GET /v1/calendars/{id}/sync | sync_token | Incremental sync. |
| GET /v1/events/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/events/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service CalendarService {
  rpc Create(CreateCalendarRequest) returns (Calendar);
  rpc Get(GetCalendarRequest) returns (Calendar);
  rpc Transition(TransitionCalendarRequest) returns (Calendar);
  rpc Reconcile(ReconcileCalendarRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "calendar.state_changed.v1",
...
- calendar.created.v1
- calendar.validated.v1
- calendar.state_changed.v1
- calendar.committed.v1
- calendar.failed.v1
- calendar.reversed.v1
- calendar.reconciliation_completed.v1
- calendar.manual_review_required.v1
...
```text
Google Calendar / Calendar Scheduling Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Calendar service -> Idempotency service -> Calendar event service -> State transition engine -> Event service -> Sync service
        ...
        |       -> Reminder scheduler / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager / Reconciliation worker
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Google Calendar / Calendar Scheduling service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Calendar service -> Idempotency service -> Calendar event service -> State transition engine -> Event service -> Sync service; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
4. **Async path:** Reminder scheduler / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager / Reconciliation worker consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
- **Calendar service**: calendars, ACLs, sharing.
- **Event service**: event master, recurrence, exceptions, attendees.
- **Free/busy**: compact availability windows with privacy filtering.
- **Reminder scheduler**: time-bucket reminder delivery.
- **Sync service**: change logs and tokens.
...
| Core domain services | Calendar service, Idempotency service, Calendar event service, State transition engine, Event service, Sync service | Own Google Calendar / Calendar Scheduling business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Calendar event service**: owns event state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed event transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Google Calendar / Calendar Scheduling: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| calendars | calendar_id, owner_id, timezone, acl_policy | Calendar. |
| events | event_id, calendar_id, organizer_id, start, end, rrule | Event. |
| event_instances | event_id, instance_start, exception_data | Exceptions/materialized window. |
| attendees | event_id, user_id/email, response_status | Attendees. |
| reminders | reminder_id, event_id, fire_at, state | Reminder. |
| sync_changes | calendar_id, change_seq, entity_ref, op | Sync log. |
...
| --- | --- | --- |
| Source of truth / primary store | Spanner/CockroachDB/PostgreSQL for calendar events, ACLs, labels, mailbox metadata, and sync tokens; Bigtable/Cassandra/ScyllaDB for high-volume mail/message bodies and mailbox indexes | collaboration permissions and sync cursors need transactions, while mailbox/message history is high-volume and partitionable |
| Hot serving / cache | Redis for free/busy cache, unread counters, sync cursors, session state, and hot thread metadata | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
| Search / analytics | OpenSearch/Lucene-style index for mail/calendar search; ClickHouse for deliverability and engagement analytics | serves text/filter/OLAP queries without overloading transactional tables |
| Large immutable payloads | object storage for attachments, raw MIME, exports, and long-retention archives | large or immutable data is cheaper, durable, and easier to lifecycle outside OLTP rows |
...
- Primary partition key: `user_id/mailbox_id/calendar_id + time bucket; thread_id/event_id for object lookup`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `message_id/event_id/thread_id` should be single-partition whenever possible.
...
- Hot partition mitigation: split large mailboxes/calendars by time bucket, precompute free/busy windows, and isolate bulk imports/index rebuilds.
...
- Required secondary indexes: `user_id + start_time, user_id + received_at, thread_id + message_time, label_id + updated_at, sync_token`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for permissions, event mutation, and mailbox source-of-truth state | strong consistency for ACLs, event updates, invites, mailbox labels that gate visibility, and sync token advancement | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for search/free-busy caches with read-your-writes where possible | Eventual consistency for search index, spam features, notifications, free/busy cache refresh, and analytics | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
For this design, use CQRS read models for mailbox/calendar views, Kafka for mail/event/change streams, Debezium/CDC for sync/index projections, Redis for unread/free-busy caches, S3 for attachments/raw MIME, and Iceberg/Athena/Redshift for compliance and analytics.
...
1. Create event writes organizer event, attendee refs/copies, resource reservation, and notifications.
2. Free/busy expands bounded time window and hides details by ACL.
3. Recurring update applies to one instance, future, or all via exceptions.
4. Client sync fetches ordered changes since token.
...
- Do not globally expand infinite recurrence; expand/cache bounded windows.
- Per-calendar change sequence powers sync.
- Room calendars use atomic reservation.
- Source-of-truth boundary: Event service plus per-calendar change log owns correctness; all low-latency read models are disposable projections.
...
- Protect private event details, attendees, location, attachments, and free/busy visibility.
- Detect spam invites, malicious ICS links, account takeover, and reminder floods.
...
- SLIs: event mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do recurrence exceptions work?
- How do sync tokens avoid missed updates?
- How do you prevent room double booking?
- Where exactly is event source of truth stored?
...
- **Correctness boundary:** Event service plus per-calendar change log is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `create event -> invite -> expand recurrence -> notify -> sync clients` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Calendar service, Event service, Free/busy, Reminder scheduler, Sync service, Idempotency service; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `calendars, events, event_instances, attendees, reminders, sync_changes` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `calendar.created.v1, calendar.validated.v1, calendar.state_changed.v1, calendar.committed.v1, calendar.failed.v1, calendar.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `create event -> invite -> expand recurrence -> notify -> sync clients`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `create event -> invite -> expand recurrence -> notify -> sync clients`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Calendar event service, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
