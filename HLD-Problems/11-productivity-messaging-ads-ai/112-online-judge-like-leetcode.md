# Design Online Judge Like LeetCode - System Design Deep Dive

**Problem #112**  
**Category:** Developer platform  
**Primary pattern:** submission queue + sandbox execution + verdicts  
**Deep-dive focus:** problem catalog, sandboxing, tests, contests, plagiarism

## 0. Interview Framing

An online judge securely executes untrusted code at scale while protecting hidden tests and producing deterministic verdicts.

Actors: users, problem setters, contest admins, judge workers. The highest-risk path is **submit code -> queue judge -> sandbox run -> verdict -> ranking**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Users browse problems, submit code, run custom tests, view verdicts/history/discussions/rankings.
- Admins manage statements, constraints, hidden tests, validators, special judges, editorials, contests.
- Workers compile/run submissions with CPU/memory/time/network limits.
- Support contest freeze, plagiarism detection, rejudge, and leaderboards.
- Expose support/admin operations to inspect, replay, correct, and annotate submission history.
...
- Untrusted code is strongly sandboxed.
- Verdicts are deterministic for fixed tests/environment.
- Queue latency remains acceptable during contests.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Submission record plus judge_runs; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Users | 50M |
| Submissions | 10M/day, 50x contest spikes |
| Languages | 20-50 runtimes |
| Tests | Private object storage |
| Workers | Autoscaled CPU/memory pools |
...
- Keep the online submission aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/submissions | problem_id, language, code_ref | Submit. |
| GET /v1/submissions/{id} | submission_id | Verdict. |
| POST /v1/admin/problems/{id}/rejudge | version, scope | Rejudge. |
| GET /v1/contests/{id}/leaderboard | cursor | Ranking. |
| GET /v1/submissions/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/submissions/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service JudgeService {
  rpc Create(CreateJudgeRequest) returns (Judge);
  rpc Get(GetJudgeRequest) returns (Judge);
  rpc Transition(TransitionJudgeRequest) returns (Judge);
  rpc Reconcile(ReconcileJudgeRequest) returns (ReconcileResult);
}
...
- judge.created.v1
- judge.validated.v1
- judge.state_changed.v1
- judge.committed.v1
- judge.failed.v1
- judge.reversed.v1
- judge.reconciliation_completed.v1
- judge.manual_review_required.v1
...
```text
Online Judge Like LeetCode Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Submission service -> Idempotency service -> Submission judging orchestrator -> State transition engine -> Problem service -> Leaderboard
        ...
        |       -> Judge scheduler / Sandbox worker / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Online Judge Like LeetCode service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Submission service -> Idempotency service -> Submission judging orchestrator -> State transition engine -> Problem service -> Leaderboard; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
4. **Async path:** Judge scheduler / Sandbox worker / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
- **Problem service**: statements, tags, tests, visibility.
- **Submission service**: code, language, status, history.
- **Judge scheduler**: priority queues by contest/language/resource.
- **Sandbox worker**: isolated compile/run.
- **Leaderboard**: accepted submissions and contest rules.
...
| Core domain services | Submission service, Idempotency service, Submission judging orchestrator, State transition engine, Problem service, Leaderboard | Own Online Judge Like LeetCode business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
| Async workers and integrations | Judge scheduler, Sandbox worker, Outbox/inbox workers, Admin/support console, Analytics/warehouse pipeline, Adapter manager | Consume committed events, call external systems, retry safely, update projections, run cleanup, and isolate slow dependencies from user-facing latency. |
...
- **Submission judging orchestrator**: owns submission state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed submission transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Online Judge Like LeetCode: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| problems | problem_id, visibility, limits, version | Problem. |
| test_cases | test_case_id, problem_id, input_ref, output_ref | Hidden tests. |
| submissions | submission_id, user_id, problem_id, language, state | Submission. |
| judge_runs | run_id, submission_id, test_case_id, verdict, resource_usage | Result. |
| worker_leases | lease_id, worker_id, submission_id, expires_at | Queue lease. |
...
| --- | --- | --- |
| Source of truth / primary store | PostgreSQL/CockroachDB for users, problems, submissions, contests, verdict metadata, and permissions; object storage for code bundles, test data, artifacts, and logs | submission and contest state needs transactional correctness, while payloads and test artifacts are large immutable blobs |
| Hot serving / cache | Redis for contest scoreboards, submission queues, rate limits, sessions, and hot problem metadata | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
| Event stream / outbox | Kafka/Pulsar/queue for submission lifecycle, sandbox execution events, plagiarism jobs, and notification fanout | decouples projections, notifications, analytics, search indexing, and recovery from the write path |
| Search / analytics | OpenSearch for problem/user search; ClickHouse for contest analytics and execution statistics | serves text/filter/OLAP queries without overloading transactional tables |
...
- Primary partition key: `contest_id/problem_id/user_id + time bucket; submission_id for execution lookup`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `submission_id/problem_id/contest_id` should be single-partition whenever possible.
...
- Hot partition mitigation: isolate popular contests, shard leaderboard updates, and keep sandbox logs off OLTP.
...
- Required secondary indexes: `user_id + created_at, problem_id + verdict, contest_id + rank_score, state + next_attempt_at`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for accepted submissions and final contest rankings | strong consistency for contest submissions, verdict commit, ranking tie-breakers, and permissions | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for live scoreboard caches with periodic authoritative recomputation | Eventual consistency for search, public stats, plagiarism analytics, notifications, and non-final leaderboard projections | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
1. Submission stores code and enqueues job.
2. Worker leases, fetches artifacts, compiles, runs sandboxed tests, uploads results.
3. Aggregator computes verdict and updates profile/leaderboard.
4. Rejudge creates new evaluation version without deleting history.
...
- Sandbox with microVM/container, seccomp, cgroups, read-only FS, no network.
- Hidden tests fetched with short-lived signed URLs.
- Contest freeze affects projection, not raw results.
- Source-of-truth boundary: Submission record plus judge_runs owns correctness; all low-latency read models are disposable projections.
...
- Protect hidden tests, user code, contest data, and admin tools.
- Detect plagiarism, spam submissions, resource abuse, sandbox escapes, and cheating.
...
- SLIs: submission mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do contests scale?
- How does rejudge work?
- Where exactly is submission source of truth stored?
...
- **Correctness boundary:** Submission record plus judge_runs is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `submit code -> queue judge -> sandbox run -> verdict -> ranking` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Problem service, Submission service, Judge scheduler, Sandbox worker, Leaderboard, Idempotency service; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `problems, test_cases, submissions, judge_runs, worker_leases, idempotency_keys` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `judge.created.v1, judge.validated.v1, judge.state_changed.v1, judge.committed.v1, judge.failed.v1, judge.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `submit code -> queue judge -> sandbox run -> verdict -> ranking`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `submit code -> queue judge -> sandbox run -> verdict -> ranking`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Submission judging orchestrator, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
