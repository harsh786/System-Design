# Design a recommendation system - System Design Deep Dive

**Problem #48**  
**Category:** ML/data  
**Primary pattern:** search geo  
**Deep-dive focus:** candidate generation, ranking, features, feedback loops
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the recommendation system service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own a recommendation system business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **candidate generation**: Generate broad candidates cheaply before expensive ranking.
- **ranking**: Separate candidate generation from ranking and keep a deterministic fallback ranker.
- **features**: Version feature definitions and prevent training/serving skew.
- **feedback loops**: Log impressions, clicks, skips, conversions, and negative feedback.
...
- **Hot path clarity:** start from `candidate generation` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `candidate generation`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `candidate generation`.
