# Design a load balancer - System Design Deep Dive

**Problem #5**  
**Category:** Infrastructure  
**Primary pattern:** core platform  
**Deep-dive focus:** L4/L7 routing, health checks, consistent hashing, draining

## 0. Interview Framing

A platform primitive where correctness, low latency, predictable scaling, and operability matter more than product breadth.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **L4/L7 routing**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Register backend targets and health checks.
- Route traffic using L4 or L7 policies.
- Drain unhealthy or deploying targets safely.
- Support sticky sessions only when required.
...
```http
GET /v1/backend-pools/{id}
Authorization: Bearer <token>
```

```http
GET /v1/backend-pools?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```

```http
PATCH /v1/backend-pools/{id}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/backend-pools/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/backend-pools/evaluate
Idempotency-Key: <uuid>
...
```http
GET /v1/backend-pools/{id}/metrics?from=2026-05-25T00:00:00Z&to=2026-05-25T01:00:00Z
```

```http
POST /v1/backend-pools/{id}/admin/quarantine
Idempotency-Key: <uuid>
...
```protobuf
service BackendPoolService {
  rpc CreateBackendPool(CreateBackendPoolRequest) returns (BackendPool);
  rpc GetBackendPool(GetBackendPoolRequest) returns (BackendPool);
  rpc UpdateBackendPool(UpdateBackendPoolRequest) returns (BackendPool);
  rpc EvaluateBackendPool(EvaluateBackendPoolRequest) returns (EvaluateBackendPoolResponse);
  rpc ListBackendPoolEvents(ListBackendPoolEventsRequest) returns (stream BackendPoolEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "backend.pools.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "backend-pools-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "backendPool_123",
...
- `backend.pools.created.v1`
- `backend.pools.updated.v1`
- `backend.pools.state_changed.v1`
- `backend.pools.deleted_or_expired.v1`
- `backend.pools.policy_denied.v1`
- `backend.pools.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the load balancer service boundary.
...
| Core domain services | Hot-path data-plane service, Control-plane API, Shard manager, Policy/config cache | Own a load balancer business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `BackendPoolController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `BackendPoolApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `BackendPoolDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `BackendPoolRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `BackendPoolPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `BackendPoolEventPublisher`: writes outbox records and publishes domain events to the broker.
- `BackendPoolReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface BackendPoolRepository {
    Optional<BackendPool> findById(BackendPoolId id, ReadConsistency consistency);
    Page<BackendPool> list(BackendPoolQuery query, Cursor cursor, int limit);
    BackendPool save(BackendPool aggregate, ExpectedVersion expectedVersion);
}

interface BackendPoolPolicyEvaluator {
    PolicyDecision canRead(Principal principal, BackendPool resource);
    PolicyDecision canMutate(Principal principal, BackendPool resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> BackendPool
  -> emits -> BackendPoolEvent
...
|---|---|
| `backend_pools` | `id, name, algorithm, health_policy_id, draining_timeout_sec, state` |
| `targets` | `id, pool_id, address, weight, zone, health_state, last_seen_at` |
| `health_checks` | `id, pool_id, protocol, path, interval_sec, timeout_ms, success_threshold, failure_threshold` |
| `routing_rules` | `id, tenant_id, backendpool_ref, state, version, created_at, updated_at` |
...
| --- | --- | --- |
| Source of truth / primary store | etcd/Consul/ZooKeeper/Raft-backed config store for upstream membership and load-balancing policy; PostgreSQL for inventory and audit history | traffic decisions need a consistent control plane and extremely fast local data-plane reads |
| Hot serving / cache | in-memory per-node routing tables, health state, and connection counters | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
- Primary partition key: `service_id + region + cluster`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `service_id / endpoint_id / config_version` should be single-partition whenever possible.
...
- Hot partition mitigation: keep the data plane local and precomputed; shard large services by region/cell and drain endpoints gradually.
...
- Required secondary indexes: `region + health_state, tenant_id + service_id, updated_at`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for config publication so split-brain routing policies do not occur | strong consistency for control-plane config versions and endpoint membership changes | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP-style local routing continues with last-known-good config during control-plane partitions | Eventual consistency for health telemetry, access logs, dashboards, and historical analytics | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **l4**: Make l4 explicit in the data model, APIs, scaling plan, and operational dashboards.
- **l7 routing**: Keep route matching deterministic, versioned, and observable with shadow/canary evaluation.
- **health checks**: Separate liveness, readiness, and deep dependency health checks to avoid false failover.
- **consistent hashing**: Use consistent hashing to reduce key movement during scale-out and node loss.
- **draining**: Support connection/request draining during deploys and autoscaling.
...
- **Hot path clarity:** start from `L4/L7 routing` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `backend.pools.created.v1, backend.pools.updated.v1, backend.pools.state_changed.v1, backend.pools.deleted_or_expired.v1, backend.pools.policy_denied.v1, backend.pools.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `L4/L7 routing`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `L4/L7 routing`.
