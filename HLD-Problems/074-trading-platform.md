# Design Trading Platform - System Design Deep Dive

**Problem #74**  
**Category:** Fintech/low latency  
**Primary pattern:** order gateway + risk + matching + market data  
**Deep-dive focus:** order book, pre-trade risk, matching, market data, clearing

## 0. Interview Framing

A trading platform needs deterministic order sequencing, low-latency matching, risk controls, market data fanout, and full audit.

Actors: traders, brokers, market makers, exchanges, clearing, surveillance. The highest-risk path is **order entry -> risk check -> sequenced matching -> execution report -> positions**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Accept buy/sell, cancel, replace, market/limit/stop, and time-in-force orders.
- Run pre-trade risk for funds, positions, price bands, fat-finger, and regulatory controls.
- Maintain order books per instrument and match by price-time priority.
- Emit execution reports, market data, drop copy, positions, clearing, and surveillance feeds.
...
- Per-instrument matching is deterministic and replayable.
- Slow consumers cannot block matching.
- Client order ID is idempotent per account/session.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Matching engine command log and execution reports; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Instruments | 100K symbols, hot symbols dominate |
| Orders | 100K-1M/sec peak venue |
| Market data | Millions msg/sec fanout |
| Audit | Full order/execution multi-year |
| Latency | Sub-ms to low-ms depending venue |
...
| --- | --- | --- |
| POST /v1/orders | account_id, instrument, side, price, qty, tif | Submit order. |
| DELETE /v1/orders/{id} | client_order_id, reason | Cancel order. |
| PATCH /v1/orders/{id} | new_price, new_qty | Replace order. |
| GET /v1/market-data/{symbol}/book | depth | Book snapshot. |
...
service TradeService {
  rpc Create(CreateTradeRequest) returns (Trade);
  rpc Get(GetTradeRequest) returns (Trade);
  rpc Transition(TransitionTradeRequest) returns (Trade);
  rpc Reconcile(ReconcileTradeRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "trade.state_changed.v1",
...
- trade.created.v1
- trade.validated.v1
- trade.state_changed.v1
- trade.committed.v1
- trade.failed.v1
- trade.reversed.v1
- trade.reconciliation_completed.v1
- trade.manual_review_required.v1
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Order gateway -> Matching engine -> Idempotency service -> Order lifecycle service -> State transition engine -> Risk engine
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Trading Platform service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Order gateway -> Matching engine -> Idempotency service -> Order lifecycle service -> State transition engine -> Risk engine; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
- **Order gateway**: sessions, protocol validation, throttles, idempotency.
- **Risk engine**: buying power, position, limits, kill switches.
- **Matching engine**: single-threaded per instrument/partition book.
- **Market data publisher**: top-of-book, depth, trades, snapshots.
- **Surveillance**: spoofing, layering, wash trade detection.
...
| Core domain services | Order gateway, Matching engine, Idempotency service, Order lifecycle service, State transition engine, Risk engine | Own Trading Platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Order lifecycle service**: owns order state machine, policy checks, outbox events, and compensation.
...
- Trading Platform: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| orders | order_id, account_id, instrument_id, side, price, qty, state | Order. |
| execution_reports | exec_id, order_id, trade_id, qty, price | Execution. |
| order_book_snapshots | instrument_id, sequence, snapshot_ref | Recovery snapshot. |
| positions | account_id, instrument_id, qty, avg_price | Position projection. |
| risk_limits | account_id, limit_type, value, status | Limits. |
...
1. Gateway validates session and idempotency, risk checks, then sends command to sequencer.
2. Matching engine applies commands in order, updates in-memory book, emits executions and market data.
3. Cancel/replace are sequenced like orders.
4. Recovery loads snapshot and replays command/event log.
...
- Do not call database on match path.
- Use price-time priority queues per price level.
- Risk reserves funds/positions before matching and releases on cancel/expire.
- Source-of-truth boundary: Matching engine command log and execution reports owns correctness; all low-latency read models are disposable projections.
...
- Protect accounts, orders, market data entitlements, and trading history.
- Detect spoofing, wash trades, self-trade abuse, credential theft, and fat-finger orders.
...
- How do you preserve price-time priority?
- How do you recover matching engine state?
...
- **Correctness boundary:** Matching engine command log and execution reports is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `order entry -> risk check -> sequenced matching -> execution report -> positions` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Order gateway, Risk engine, Matching engine, Market data publisher, Surveillance, Idempotency service; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `orders, execution_reports, order_book_snapshots, positions, risk_limits, idempotency_keys` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `trade.created.v1, trade.validated.v1, trade.state_changed.v1, trade.committed.v1, trade.failed.v1, trade.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `order entry -> risk check -> sequenced matching -> execution report -> positions`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `order entry -> risk check -> sequenced matching -> execution report -> positions`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Order lifecycle service, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
