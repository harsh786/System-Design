# Design Movie Ticket Booking Like BookMyShow - System Design Deep Dive

**Problem #69**  
**Category:** Booking/events  
**Primary pattern:** seat map + distributed seat lock + payment timeout  
**Deep-dive focus:** seat discovery, locking, fairness, ticket issuance

## 0. Interview Framing

Movie ticketing is a high-concurrency scarce-seat problem where hot shows must degrade into fair queues, not duplicate seat sales.

Actors: moviegoers, theaters, payment providers, scanners, admins. The highest-risk path is **show seat selection -> lock seats -> pay -> issue tickets without duplicate sale**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Browse movies/theaters/shows/formats/languages and seat maps.
- Lock selected seats for a short TTL and complete payment.
- Issue signed QR tickets and validate at theater scanners.
- Support cancellation/refund policy, seat blocks, quotas, and settlement.
...
- A show-seat can be SOLD once and LOCKED by at most one active lock.
- Lock expiry must release seats reliably.
- Ticket scan is one-time and auditable.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: show_seats table with conditional state transitions; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Shows | 1M/day |
| Seat map reads | 50-100x booking writes |
| Booking attempts | 5M/day, 50x blockbuster peaks |
| Lock TTL | 5-10 minutes |
| Retention | Seat/payment/scan audit multi-year |
...
| --- | --- | --- |
| GET /v1/movies/{movie_id}/shows | city, date, format | Show discovery. |
| POST /v1/seat-locks | show_id, seat_ids, user_id | Atomic all-or-nothing lock. |
| POST /v1/ticket-bookings | lock_id, payment_method_id | Commit seats and issue tickets. |
| POST /v1/tickets/{ticket_id}/scan | scanner_id, show_id | One-time ticket scan. |
| GET /v1/ticket-bookings/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/ticket-bookings/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service MovieTicketService {
  rpc Create(CreateMovieTicketRequest) returns (MovieTicket);
  rpc Get(GetMovieTicketRequest) returns (MovieTicket);
  rpc Transition(TransitionMovieTicketRequest) returns (MovieTicket);
  rpc Reconcile(ReconcileMovieTicketRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "movie_ticket.state_changed.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "movie_ticket-service",
...
- movie_ticket.created.v1
- movie_ticket.validated.v1
- movie_ticket.state_changed.v1
- movie_ticket.committed.v1
- movie_ticket.failed.v1
- movie_ticket.reversed.v1
- movie_ticket.reconciliation_completed.v1
- movie_ticket.manual_review_required.v1
...
```text
Movie Ticket Booking Like BookMyShow Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Seat inventory -> Idempotency service -> Seat booking orchestrator -> State transition engine -> Ticket service -> Queue/fairness
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Movie Ticket Booking Like BookMyShow service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Seat inventory -> Idempotency service -> Seat booking orchestrator -> State transition engine -> Ticket service -> Queue/fairness; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
3. **Query path:** read-heavy traffic is served by Catalog service; strong reads fall back to the canonical store when stale projections are unsafe.
...
- **Catalog service**: movies, theaters, screens, showtimes, formats.
- **Seat inventory**: owns show_seat state, locks, commits, expiry.
- **Queue/fairness**: protects hot releases from bots and floods.
- **Ticket service**: issues signed QR and validates scans.
...
| Core domain services | Seat inventory, Idempotency service, Seat booking orchestrator, State transition engine, Ticket service, Queue/fairness | Own Movie Ticket Booking Like BookMyShow business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Catalog service | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
...
- **Seat booking orchestrator**: owns booking state machine, policy checks, outbox events, and compensation.
...
- Movie Ticket Booking Like BookMyShow: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| shows | show_id, theater_id, screen_id, starts_at, status | Show schedule. |
| show_seats | show_id, seat_id, state, lock_id, version | Canonical seat state. |
| seat_locks | lock_id, show_id, user_id, expires_at, state | Seat lock. |
| tickets | ticket_id, booking_id, seat_id, qr_hash, scan_state | Issued ticket. |
...
1. Seat lock conditionally updates all requested seats or fails with conflicts.
2. Checkout captures payment, commits seats to SOLD, issues tickets, and notifies user.
3. Expiry worker releases stale locks and rebuilds seat-map projection.
...
- Atomic multi-seat lock is all-or-nothing.
- Seat map cache is never source of truth.
- Queue tokens and lock quotas prevent bot starvation.
- Source-of-truth boundary: show_seats table with conditional state transitions owns correctness; all low-latency read models are disposable projections.
...
- Protect payment data and ticket QR secrets.
- Detect bots, scripted locks, stolen cards, refund abuse, and duplicate scans.
...
- How do you lock adjacent seats atomically?
- What if payment succeeds after lock expiry?
- How do theater scans work offline?
...
- **Correctness boundary:** show_seats table with conditional state transitions is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `show seat selection -> lock seats -> pay -> issue tickets without duplicate sale` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Catalog service, Seat inventory, Queue/fairness, Ticket service, Idempotency service, Outbox/inbox workers; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `shows, show_seats, seat_locks, tickets, idempotency_keys, domain_events` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `movie_ticket.created.v1, movie_ticket.validated.v1, movie_ticket.state_changed.v1, movie_ticket.committed.v1, movie_ticket.failed.v1, movie_ticket.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `show seat selection -> lock seats -> pay -> issue tickets without duplicate sale`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `show seat selection -> lock seats -> pay -> issue tickets without duplicate sale`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Seat booking orchestrator, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
