# Design Google Docs collaborative editing - System Design Deep Dive

**Problem #18**  
**Category:** Collaboration  
**Primary pattern:** realtime collab  
**Deep-dive focus:** CRDT/OT, cursor presence, conflict resolution, snapshots
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **CRDT/OT**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Collaboratively edit documents with conflict resolution.
- Show cursor presence and comments.
- Persist snapshots and operation logs.
- Recover from reconnects without data loss.
...
```http
GET /v1/documents?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/documents/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
GET /v1/documents/{id}/sync?cursor=<device_cursor>&limit=100
```

```http
POST /v1/documents/{id}/read-receipts
Idempotency-Key: <uuid>
...
service DocumentService {
  ...
  rpc ListDocumentEvents(ListDocumentEventsRequest) returns (stream DocumentEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "documents.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "documents-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "document_123",
...
- `documents.created.v1`
- `documents.updated.v1`
- `documents.state_changed.v1`
- `documents.deleted_or_expired.v1`
- `documents.policy_denied.v1`
- `documents.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Google Docs collaborative editing service boundary.
...
| Core domain services | Message or collaboration service, Media/attachment service, Moderation and abuse service, Session registry | Own Google Docs collaborative editing business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `DocumentController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `DocumentApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `DocumentDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `DocumentRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `DocumentPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `DocumentEventPublisher`: writes outbox records and publishes domain events to the broker.
- `DocumentReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface DocumentRepository {
    Optional<Document> findById(DocumentId id, ReadConsistency consistency);
    Page<Document> list(DocumentQuery query, Cursor cursor, int limit);
    Document save(Document aggregate, ExpectedVersion expectedVersion);
}

interface DocumentPolicyEvaluator {
    PolicyDecision canRead(Principal principal, Document resource);
    PolicyDecision canMutate(Principal principal, Document resource, Action action);
}
...
| `document_ops` | `document_id, op_seq, actor_id, operation_json, base_version, created_at` |
| `snapshots` | `id, document_id, op_seq_to, object_ref, checksum, created_at` |
| `collaborators` | `document_id, user_id, role, cursor_state_ref, last_seen_at` |
...
| --- | --- | --- |
| Source of truth / primary store | CockroachDB/Spanner/FoundationDB-style transactional store for document metadata, ACLs, snapshots, and operation checkpoints; append-only operation log for CRDT/OT edits | collaboration needs durable ordered operations plus strongly consistent permissions and snapshot metadata |
| Hot serving / cache | Redis/WebSocket session registry for cursor presence and ephemeral collaboration state | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
- Primary partition key: `document_id + operation_seq; tenant_id for admin and ACL tables`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `document_id + version/operation_seq` should be single-partition whenever possible.
...
- Required secondary indexes: `owner_id + updated_at, collaborator_id + last_opened_at, snapshot_version`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for document operation acceptance and permission checks | convergent document state using CRDT/OT; strong consistency for ACLs and snapshot checkpoints | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for presence and cursor state | Eventual consistency for presence, cursor positions, search index, comments notifications, and analytics | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **crdt**: Pick CRDT for offline/multi-master friendliness or OT for central ordered collaboration; explain invariants.
- **ot**: Enforce quotas in the hot path and reconcile usage asynchronously for billing/reporting.
- **cursor presence**: Keep cursor presence ephemeral and separate from durable document edits.
...
- **Hot path clarity:** start from `CRDT/OT` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `documents.created.v1, documents.updated.v1, documents.state_changed.v1, documents.deleted_or_expired.v1, documents.policy_denied.v1, documents.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `CRDT/OT`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `CRDT/OT`.
