# Design Banking Ledger - System Design Deep Dive

**Problem #73**  
**Category:** Fintech/core banking  
**Primary pattern:** immutable double-entry journal + balances  
**Deep-dive focus:** chart of accounts, entries, snapshots, reversals, close, audit

## 0. Interview Framing

A banking ledger is the accounting source of truth. Correctness, immutability, deterministic replay, and audit beat convenience.

Actors: banking products, finance operators, auditors, regulators. The highest-risk path is **business transaction -> balanced journal -> immutable ledger -> balance snapshot -> reconciliation**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Post balanced journal transactions across customer, internal, suspense, fee, revenue, and settlement accounts.
- Support holds, settlement, reversal, adjustment, interest, fees, statements, close, and reconciliation.
- Expose balance queries, account statements, audit exports, and regulatory reports.
- Enforce maker-checker controls for manual adjustments.
- Expose support/admin operations to inspect, replay, correct, and annotate ledger_transaction history.
...
- Every committed transaction balances debits and credits per currency.
- Ledger entries are immutable; corrections are compensating entries.
- Historical as-of balances must be reproducible.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Immutable ledger_entries sequenced per ledger shard; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Accounts | 100M customer/internal |
| Transactions | 200M/day |
| Entries | 2-20 per transaction |
| Balance reads | High for payroll/events |
| Retention | Permanent or regulatory multi-year |
...
- Keep the online ledger_transaction aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/ledger-transactions | business_ref, entries, currency | Post balanced journal. |
| GET /v1/accounts/{id}/balance | as_of, consistency | Balance query. |
| POST /v1/ledger-transactions/{id}/reverse | reason | Compensating reversal. |
| POST /v1/close-batches | business_date | Daily close. |
| GET /v1/ledger-transactions/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/ledger-transactions/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service BankLedgerService {
  rpc Create(CreateBankLedgerRequest) returns (BankLedger);
  rpc Get(GetBankLedgerRequest) returns (BankLedger);
  rpc Transition(TransitionBankLedgerRequest) returns (BankLedger);
  rpc Reconcile(ReconcileBankLedgerRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "bank_ledger.state_changed.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "bank_ledger-service",
  "tenant_or_region_id": "region_123",
  "aggregate_id": "ledger_transaction_123",
...
- bank_ledger.created.v1
- bank_ledger.validated.v1
- bank_ledger.state_changed.v1
- bank_ledger.committed.v1
- bank_ledger.failed.v1
- bank_ledger.reversed.v1
- bank_ledger.reconciliation_completed.v1
- bank_ledger.manual_review_required.v1
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Journal service -> Idempotency service -> Journal posting service -> State transition engine -> Sequencer -> Balance projection
        ...
        |       -> Close/reconciliation / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager / Reconciliation worker
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Banking Ledger service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Journal service -> Idempotency service -> Journal posting service -> State transition engine -> Sequencer -> Balance projection; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
4. **Async path:** Close/reconciliation / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager / Reconciliation worker consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
- **Journal service**: validates balanced postings and appends immutable entries.
- **Sequencer**: assigns deterministic ledger sequence per shard.
- **Balance projection**: real-time and end-of-day balances.
- **Close/reconciliation**: daily close, suspense clearing, provider reconciliation.
...
| Core domain services | Journal service, Idempotency service, Journal posting service, State transition engine, Sequencer, Balance projection | Own Banking Ledger business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Journal posting service**: owns ledger_transaction state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed ledger_transaction transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Banking Ledger: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| accounts | account_id, account_type, currency, status | Ledger account. |
| ledger_transactions | txn_id, business_ref, state, posted_at | Transaction header. |
| ledger_entries | entry_id, txn_id, account_id, direction, amount, sequence | Immutable entry. |
| account_balances | account_id, ledger_sequence, available, posted | Projection. |
| close_batches | batch_id, business_date, state, totals_hash | Close control. |
...
1. Post validates accounts/currency, verifies balanced entries, appends atomically, and publishes sequence.
2. Balance read serves projection or computes as-of from snapshot plus entries.
3. Reversal posts inverse entries referencing original transaction.
4. Daily close freezes totals and reconciles suspense/provider accounts.
...
- Single writer/sequencer per ledger shard improves deterministic ordering.
- Hash chains/Merkle summaries detect tampering.
- Pending authorization holds may be outside posted ledger depending product.
- Source-of-truth boundary: Immutable ledger_entries sequenced per ledger shard owns correctness; all low-latency read models are disposable projections.
...
- Protect account identifiers, transaction history, operator actions, and reports.
- Detect unauthorized postings, suspicious adjustments, internal misuse, and reconciliation breaks.
...
- SLIs: ledger_transaction mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do you guarantee debit equals credit?
- Why immutable entries?
- How do you query historical balance?
- Where exactly is ledger_transaction source of truth stored?
...
- **Correctness boundary:** Immutable ledger_entries sequenced per ledger shard is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `business transaction -> balanced journal -> immutable ledger -> balance snapshot -> reconciliation` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Journal service, Sequencer, Balance projection, Close/reconciliation, Idempotency service, Outbox/inbox workers; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `accounts, ledger_transactions, ledger_entries, account_balances, close_batches, idempotency_keys` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `bank_ledger.created.v1, bank_ledger.validated.v1, bank_ledger.state_changed.v1, bank_ledger.committed.v1, bank_ledger.failed.v1, bank_ledger.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `business transaction -> balanced journal -> immutable ledger -> balance snapshot -> reconciliation`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `business transaction -> balanced journal -> immutable ledger -> balance snapshot -> reconciliation`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Journal posting service, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
