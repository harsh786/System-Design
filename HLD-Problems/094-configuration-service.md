# Design configuration service - System Design Deep Dive

**Problem #94**  
**Category:** Platform  
**Primary pattern:** infra security  
**Deep-dive focus:** versioning, watches, rollout, audit, consistency
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **versioning**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```http
GET /v1/configs?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/configs/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
service ConfigItemService {
  rpc CreateConfigItem(CreateConfigItemRequest) returns (ConfigItem);
  rpc GetConfigItem(GetConfigItemRequest) returns (ConfigItem);
  rpc UpdateConfigItem(UpdateConfigItemRequest) returns (ConfigItem);
  rpc EvaluateConfigItem(EvaluateConfigItemRequest) returns (EvaluateConfigItemResponse);
  rpc ListConfigItemEvents(ListConfigItemEventsRequest) returns (stream ConfigItemEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "configs.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "configs-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "configItem_123",
...
- `configs.created.v1`
- `configs.updated.v1`
- `configs.state_changed.v1`
- `configs.deleted_or_expired.v1`
- `configs.policy_denied.v1`
- `configs.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the configuration service boundary.
...
| Core domain services | Propagation service, Policy engine, Versioned config store, Data-plane evaluator or agent, SRE console | Own configuration service business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `ConfigItemController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `ConfigItemApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `ConfigItemDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `ConfigItemRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `ConfigItemPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `ConfigItemEventPublisher`: writes outbox records and publishes domain events to the broker.
- `ConfigItemReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface ConfigItemRepository {
    Optional<ConfigItem> findById(ConfigItemId id, ReadConsistency consistency);
    Page<ConfigItem> list(ConfigItemQuery query, Cursor cursor, int limit);
    ConfigItem save(ConfigItem aggregate, ExpectedVersion expectedVersion);
}

interface ConfigItemPolicyEvaluator {
    PolicyDecision canRead(Principal principal, ConfigItem resource);
    PolicyDecision canMutate(Principal principal, ConfigItem resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> ConfigItem
  -> emits -> ConfigItemEvent
...
|---|---|
| `config_keys` | `id, tenant_id, namespace, key, owner_team, schema_ref, state` |
| `config_versions` | `config_key_id, version, value_ref, checksum, created_by, created_at` |
| `watch_subscriptions` | `id, config_key_id, client_id, last_seen_version, expires_at` |
| `rollout_rules` | `id, config_key_id, version, segment_ref, rollout_percentage, state` |
...
- **watches**: Stream config changes with version ordering and periodic full resync.
...
- **Hot path clarity:** start from `versioning` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `configs.created.v1, configs.updated.v1, configs.state_changed.v1, configs.deleted_or_expired.v1, configs.policy_denied.v1, configs.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `versioning`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `versioning`.
