# Design real-time analytics dashboard - System Design Deep Dive

**Problem #85**  
**Category:** Data/realtime  
**Primary pattern:** data scheduling  
**Deep-dive focus:** Kafka, stream processing, OLAP store, freshness
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **Kafka**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the real-time analytics dashboard service boundary.
...
| Core domain services | Dispatcher/queue, State/checkpoint service, Control-plane API, Result serving API, Alerting and retry manager, Catalog/lineage service | Own real-time analytics dashboard business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **kafka**: Make kafka explicit in the data model, APIs, scaling plan, and operational dashboards.
- **stream processing**: Make stream processing explicit in the data model, APIs, scaling plan, and operational dashboards.
- **olap store**: Use OLAP/time-series stores for query serving and keep raw events in durable storage.
...
- **Hot path clarity:** start from `Kafka` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `Kafka`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `Kafka`.
