# Design metrics aggregation system - System Design Deep Dive

**Problem #88**  
**Category:** Observability/data  
**Primary pattern:** data scheduling  
**Deep-dive focus:** time-series ingestion, rollups, retention, alerts
...
```text
metrics aggregation system Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the metrics aggregation system service boundary.
...
| Core domain services | Dispatcher/queue, State/checkpoint service, Control-plane API, Result serving API, Alerting and retry manager, Catalog/lineage service | Own metrics aggregation system business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **time-series ingestion**: Shard by metric identity and time; compact into rollups.
- **rollups**: Precompute rollups to control query cost and retention.
- **retention**: Separate hot retention, compliance retention, and analytics retention.
- **alerts**: Evaluate alerts on reliable rollups and dedupe noisy signals.
...
- **Hot path clarity:** start from `time-series ingestion` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `time-series ingestion`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `time-series ingestion`.
