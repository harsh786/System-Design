# Design distributed cron / task scheduler - System Design Deep Dive

**Problem #81**  
**Category:** Scheduling  
**Primary pattern:** data scheduling  
**Deep-dive focus:** recurring jobs, distributed locks, leases, retries, misfires
...
```text
distributed cron / task scheduler Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the distributed cron / task scheduler service boundary.
...
| Core domain services | Dispatcher/queue, State/checkpoint service, Control-plane API, Result serving API, Alerting and retry manager, Catalog/lineage service | Own distributed cron / task scheduler business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **recurring jobs**: Store schedule definitions, next-fire time, and misfire policy explicitly.
- **distributed locks**: Use leases and fencing tokens around singleton job execution.
- **leases**: Expire leases safely and make workers prove ownership before committing side effects.
- **retries**: Use bounded retries with jitter, DLQs, poison-message handling, and replay tooling.
- **misfires**: Define catch-up, skip, or run-once behavior for missed schedules.
...
- **Hot path clarity:** start from `recurring jobs` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `recurring jobs`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `recurring jobs`.
