# Design data lakehouse platform - System Design Deep Dive

**Problem #86**  
**Category:** Data platform  
**Primary pattern:** data scheduling  
**Deep-dive focus:** S3, Iceberg/Hudi/Delta, Spark/Flink, catalog, governance
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **S3**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Register workloads, schedules, datasets, or devices.
- Dispatch execution or ingestion work reliably.
- Track state, checkpoints, retries, and backfills.
- Serve results with freshness and lineage metadata.
...
```http
GET /v1/workloads?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/workloads/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/workloads/runs/{run_id}/cancel
Idempotency-Key: <uuid>
...
```http
GET /v1/workloads/runs/{run_id}/logs?cursor=<cursor>&limit=1000
```
...
service WorkloadService {
  ...
  rpc EvaluateWorkload(EvaluateWorkloadRequest) returns (EvaluateWorkloadResponse);
  rpc ListWorkloadEvents(ListWorkloadEventsRequest) returns (stream WorkloadEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "workloads.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "workloads-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "workload_123",
...
- `workloads.created.v1`
- `workloads.updated.v1`
- `workloads.state_changed.v1`
- `workloads.deleted_or_expired.v1`
- `workloads.policy_denied.v1`
- `workloads.operation_failed.v1`
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |
        +--> Query / Serving Path
        |       -> Catalog/lineage service / Result serving API
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the data lakehouse platform service boundary.
...
3. **Query path:** read-heavy traffic is served by Catalog/lineage service / Result serving API; strong reads fall back to the canonical store when stale projections are unsafe.
4. **Async path:** Scheduler / Worker pools consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
| Core domain services | Dispatcher/queue, State/checkpoint service, Control-plane API, Result serving API, Alerting and retry manager, Catalog/lineage service | Own data lakehouse platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Catalog/lineage service, Result serving API | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
| Async workers and integrations | Scheduler, Worker pools | Consume committed events, call external systems, retry safely, update projections, run cleanup, and isolate slow dependencies from user-facing latency. |
...
- `WorkloadController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `WorkloadApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `WorkloadDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `WorkloadRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `WorkloadPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `WorkloadEventPublisher`: writes outbox records and publishes domain events to the broker.
- `WorkloadReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface WorkloadRepository {
    Optional<Workload> findById(WorkloadId id, ReadConsistency consistency);
    Page<Workload> list(WorkloadQuery query, Cursor cursor, int limit);
    Workload save(Workload aggregate, ExpectedVersion expectedVersion);
}

interface WorkloadPolicyEvaluator {
    PolicyDecision canRead(Principal principal, Workload resource);
    PolicyDecision canMutate(Principal principal, Workload resource, Action action);
}
...
| `tasks` | `id, run_id, task_key, dependencies, state, worker_id, lease_until, attempt` |
| `checkpoints` | `id, workload_id, run_id, offset_or_cursor, state_ref, created_at` |
| `lineage_events` | `id, source_ref, target_ref, transform_version, checksum, created_at` |

### Database Technology Choice

| Workload / Data | Recommended Database / Store | Why This Choice Fits |
| --- | --- | --- |
| Source of truth / primary store | PostgreSQL/CockroachDB for job/workflow metadata, leases, schedules, and control state; object storage/lakehouse for payloads, checkpoints, and historical data | control-plane state needs transactions and leases, while data payloads and analytics history scale better in object/columnar stores |
| Hot serving / cache | Redis for worker heartbeats, short leases, schedule wheels, and hot dashboard queries where correctness is backed by primary state | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
| Event stream / outbox | Kafka/Pulsar/Kinesis for ingestion, job events, metrics, backpressure, and replay | decouples projections, notifications, analytics, search indexing, and recovery from the write path |
| Search / analytics | ClickHouse/Druid/Pinot/TSDB for metrics and real-time analytics; OpenSearch for logs/job search | serves text/filter/OLAP queries without overloading transactional tables |
| Large immutable payloads | S3/GCS/Azure Blob with Iceberg/Hudi/Delta tables for durable datasets, reports, checkpoints, and exports | large or immutable data is cheaper, durable, and easier to lifecycle outside OLTP rows |
...
- Primary partition key: `tenant_id + job_id/workflow_id/device_id/metric_series; time bucket for events and metrics`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `job_id/workflow_id/run_id/series_id` should be single-partition whenever possible.
...
- Hot partition mitigation: partition high-cardinality streams, isolate noisy tenants, shard scheduler ownership, and use backpressure per topic/source.
...
- Required secondary indexes: `tenant_id + state + next_run_at, worker_id + heartbeat_at, source_id + event_time, state + updated_at`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for scheduler/workflow ownership and exactly-once-like checkpoint boundaries | strong consistency for schedule ownership, leases, workflow state transitions, and checkpoint commits | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for metrics/query serving where late data and retractions are handled | Eventual consistency for dashboards, rollups, search, reports, ML features, and long-range analytics | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **s3**: Make s3 explicit in the data model, APIs, scaling plan, and operational dashboards.
- **iceberg**: Make iceberg explicit in the data model, APIs, scaling plan, and operational dashboards.
- **hudi**: Make hudi explicit in the data model, APIs, scaling plan, and operational dashboards.
- **delta**: Make delta explicit in the data model, APIs, scaling plan, and operational dashboards.
- **spark**: Use Spark for batch and Flink or equivalent for stateful streaming where low latency matters.
- **catalog**: Centralize table/schema ownership, discovery, and access policy.
- **governance**: Apply data classification, lineage, retention, and access review.
...
- **Hot path clarity:** start from `S3` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `workloads.created.v1, workloads.updated.v1, workloads.state_changed.v1, workloads.deleted_or_expired.v1, workloads.policy_denied.v1, workloads.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `S3`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `S3`.
