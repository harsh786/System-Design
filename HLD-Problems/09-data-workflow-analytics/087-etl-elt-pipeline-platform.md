# Design ETL/ELT pipeline platform - System Design Deep Dive

**Problem #87**  
**Category:** Data engineering  
**Primary pattern:** data scheduling  
**Deep-dive focus:** connectors, transformations, lineage, backfill, data quality
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **connectors**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
ETL/ELT pipeline platform Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the ETL/ELT pipeline platform service boundary.
...
| Core domain services | Dispatcher/queue, State/checkpoint service, Control-plane API, Result serving API, Alerting and retry manager, Catalog/lineage service | Own ETL/ELT pipeline platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **connectors**: Sandbox connectors and manage schema drift, retries, and credentials.
- **transformations**: Version transformations and make output partitions reproducible.
- **lineage**: Track source inputs, transformations, versions, and output checksums.
- **backfill**: Run backfills with separate capacity and idempotent output partitions.
- **data quality**: Run freshness, volume, schema, and semantic checks before publishing.
...
- **Hot path clarity:** start from `connectors` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `connectors`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `connectors`.
