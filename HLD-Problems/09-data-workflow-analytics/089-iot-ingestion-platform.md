# Design IoT ingestion platform - System Design Deep Dive

**Problem #89**  
**Category:** IoT/data  
**Primary pattern:** data scheduling  
**Deep-dive focus:** device identity, MQTT, backpressure, time-series storage
...
```text
IoT ingestion platform Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the IoT ingestion platform service boundary.
...
| Core domain services | Dispatcher/queue, State/checkpoint service, Control-plane API, Result serving API, Alerting and retry manager, Catalog/lineage service | Own IoT ingestion platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **device identity**: Provision device credentials and rotate/revoke them safely.
- **mqtt**: Use MQTT brokers for constrained devices and bridge to durable streams.
- **backpressure**: Apply producer throttles, drop policies for non-critical events, and visible lag metrics.
- **time-series storage**: Store raw hot data and downsampled long-term data separately.
...
- **Hot path clarity:** start from `device identity` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `device identity`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `device identity`.
