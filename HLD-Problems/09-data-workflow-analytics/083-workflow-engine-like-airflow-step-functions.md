# Design workflow engine like Airflow / Step Functions - System Design Deep Dive

**Problem #83**  
**Category:** Orchestration  
**Primary pattern:** data scheduling  
**Deep-dive focus:** DAGs, state, retries, idempotency, backfills
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **DAGs**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
workflow engine like Airflow / Step Functions Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the workflow engine like Airflow / Step Functions service boundary.
...
| Core domain services | Dispatcher/queue, State/checkpoint service, Control-plane API, Result serving API, Alerting and retry manager, Catalog/lineage service | Own workflow engine like Airflow / Step Functions business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **dags**: Persist DAG definitions, task states, dependencies, retries, and backfill windows.
- **state**: Make allowed transitions explicit and reject invalid or duplicate commands.
...
- **backfills**: Run backfills with separate capacity and idempotent output partitions.
...
- **Hot path clarity:** start from `DAGs` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `DAGs`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `DAGs`.
