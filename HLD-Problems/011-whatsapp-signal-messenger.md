# Design WhatsApp / Signal messenger - System Design Deep Dive

**Problem #11**  
**Category:** Realtime/security  
**Primary pattern:** realtime collab  
**Deep-dive focus:** E2E encryption, device sync, offline delivery, media, receipts
...
```text
WhatsApp / Signal messenger Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the WhatsApp / Signal messenger service boundary.
...
| Core domain services | Message or collaboration service, Media/attachment service, Session registry | Own WhatsApp / Signal messenger business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **e2e encryption**: Keep plaintext off servers where possible; model key exchange, multi-device keys, and abuse-report exceptions.
- **device sync**: Use per-device queues and cursor-based sync so reconnects are deterministic.
- **offline delivery**: Persist undelivered messages/events and expire them by product policy.
- **media**: Move media to object storage/CDN and pass references through the core transaction path.
- **receipts**: Make delivery/read receipts lightweight events with per-recipient state.
...
- **Hot path clarity:** start from `E2E encryption` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `E2E encryption`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `E2E encryption`.
