# Design WebSocket presence service - System Design Deep Dive

**Problem #15**  
**Category:** Realtime infra  
**Primary pattern:** realtime collab  
**Deep-dive focus:** heartbeats, connection registry, fanout, stale connection cleanup
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **heartbeats**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Track online, idle, away, and offline states.
- Expire stale connections using heartbeats.
- Fan out presence changes to subscribed users.
- Protect the system from heartbeat storms.
...
```http
GET /v1/presence-sessions/{id}
Authorization: Bearer <token>
```

```http
GET /v1/presence-sessions?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```

```http
PATCH /v1/presence-sessions/{id}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/presence-sessions/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/presence-sessions/{id}/messages
Idempotency-Key: <uuid>
...
```http
GET /v1/presence-sessions/{id}/sync?cursor=<device_cursor>&limit=100
```

```http
POST /v1/presence-sessions/{id}/read-receipts
Idempotency-Key: <uuid>
...
```protobuf
service PresenceSessionService {
  rpc CreatePresenceSession(CreatePresenceSessionRequest) returns (PresenceSession);
  rpc GetPresenceSession(GetPresenceSessionRequest) returns (PresenceSession);
  rpc UpdatePresenceSession(UpdatePresenceSessionRequest) returns (PresenceSession);
  rpc EvaluatePresenceSession(EvaluatePresenceSessionRequest) returns (EvaluatePresenceSessionResponse);
  rpc ListPresenceSessionEvents(ListPresenceSessionEventsRequest) returns (stream PresenceSessionEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "presence.sessions.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "presence-sessions-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "presenceSession_123",
...
- `presence.sessions.created.v1`
- `presence.sessions.updated.v1`
- `presence.sessions.state_changed.v1`
- `presence.sessions.deleted_or_expired.v1`
- `presence.sessions.policy_denied.v1`
- `presence.sessions.operation_failed.v1`
...
```text
WebSocket presence service Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the WebSocket presence service boundary.
...
| Core domain services | Message or collaboration service, Media/attachment service, Moderation and abuse service, Session registry | Own WebSocket presence service business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `PresenceSessionController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `PresenceSessionApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `PresenceSessionDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `PresenceSessionRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `PresenceSessionPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `PresenceSessionEventPublisher`: writes outbox records and publishes domain events to the broker.
- `PresenceSessionReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface PresenceSessionRepository {
    Optional<PresenceSession> findById(PresenceSessionId id, ReadConsistency consistency);
    Page<PresenceSession> list(PresenceSessionQuery query, Cursor cursor, int limit);
    PresenceSession save(PresenceSession aggregate, ExpectedVersion expectedVersion);
}

interface PresenceSessionPolicyEvaluator {
    PolicyDecision canRead(Principal principal, PresenceSession resource);
    PolicyDecision canMutate(Principal principal, PresenceSession resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> PresenceSession
  -> emits -> PresenceSessionEvent
...
| `subscriptions` | `id, tenant_id, presencesession_ref, state, version, created_at, updated_at` |
| `heartbeats` | `id, tenant_id, presencesession_ref, state, version, created_at, updated_at` |
| `fanout_events` | `id, tenant_id, presencesession_ref, state, version, created_at, updated_at` |
...
- **heartbeats**: Make heartbeats explicit in the data model, APIs, scaling plan, and operational dashboards.
- **connection registry**: Make connection registry explicit in the data model, APIs, scaling plan, and operational dashboards.
- **fanout**: Choose fanout-on-write, fanout-on-read, or hybrid based on follower count and freshness needs.
- **stale connection cleanup**: Expire sessions with TTL and heartbeat miss thresholds; clean server maps on disconnect.
...
- **Hot path clarity:** start from `heartbeats` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `presence.sessions.created.v1, presence.sessions.updated.v1, presence.sessions.state_changed.v1, presence.sessions.deleted_or_expired.v1, presence.sessions.policy_denied.v1, presence.sessions.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `heartbeats`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `heartbeats`.
