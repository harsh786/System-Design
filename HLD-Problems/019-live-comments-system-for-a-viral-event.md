# Design a live comments system for a viral event - System Design Deep Dive

**Problem #19**  
**Category:** Realtime/social  
**Primary pattern:** realtime collab  
**Deep-dive focus:** ordering, moderation, backpressure, websocket fanout
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **ordering**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Create and manage a live comments system for a viral event domain objects.
- Serve core reads and writes with predictable latency.
- Emit audit and analytics events for every important state transition.
- Expose operational controls for administrators and SREs.
...
```http
POST /v1/a-live-comments-system-for-a-viral-event
...
```http
GET /v1/a-live-comments-system-for-a-viral-event/{id}
Authorization: Bearer <token>
```

```http
GET /v1/a-live-comments-system-for-a-viral-event?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```

```http
PATCH /v1/a-live-comments-system-for-a-viral-event/{id}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/a-live-comments-system-for-a-viral-event/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/a-live-comments-system-for-a-viral-event/{id}/messages
Idempotency-Key: <uuid>
...
```http
GET /v1/a-live-comments-system-for-a-viral-event/{id}/sync?cursor=<device_cursor>&limit=100
```

```http
POST /v1/a-live-comments-system-for-a-viral-event/{id}/read-receipts
Idempotency-Key: <uuid>
...
{
  "event_id": "evt_01H...",
  "event_type": "a.live.comments.system.for.a.viral.event.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "a-live-comments-system-for-a-viral-event-service",
...
- `a.live.comments.system.for.a.viral.event.created.v1`
- `a.live.comments.system.for.a.viral.event.updated.v1`
- `a.live.comments.system.for.a.viral.event.state_changed.v1`
- `a.live.comments.system.for.a.viral.event.deleted_or_expired.v1`
- `a.live.comments.system.for.a.viral.event.policy_denied.v1`
- `a.live.comments.system.for.a.viral.event.operation_failed.v1`
...
```text
a live comments system for a viral event Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the live comments system for a viral event service boundary.
...
| Core domain services | Message or collaboration service, Media/attachment service, Moderation and abuse service, Session registry | Own a live comments system for a viral event business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **websocket fanout**: Choose fanout-on-write, fanout-on-read, or hybrid based on follower count and freshness needs.
...
- **Hot path clarity:** start from `ordering` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `a.live.comments.system.for.a.viral.event.created.v1, a.live.comments.system.for.a.viral.event.updated.v1, a.live.comments.system.for.a.viral.event.state_changed.v1, a.live.comments.system.for.a.viral.event.deleted_or_expired.v1, a.live.comments.system.for.a.viral.event.policy_denied.v1, a.live.comments.system.for.a.viral.event.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `ordering`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `ordering`.
