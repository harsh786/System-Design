# Design backup and restore platform - System Design Deep Dive

**Problem #55**  
**Category:** Reliability/storage  
**Primary pattern:** storage sync  
**Deep-dive focus:** snapshots, retention, restore tests, encryption, ransomware recovery
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **snapshots**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
backup and restore platform Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the backup and restore platform service boundary.
...
3. **Query path:** read-heavy traffic is served by Metadata service; strong reads fall back to the canonical store when stale projections are unsafe.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own backup and restore platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Metadata service | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
...
- **snapshots**: Represent point-in-time snapshots with incremental manifests and retention policy.
- **retention**: Separate hot retention, compliance retention, and analytics retention.
- **restore tests**: Continuously test restores, not just backups.
- **encryption**: Encrypt in transit and at rest; isolate keys by tenant, region, and sensitivity.
- **ransomware recovery**: Use immutable backups, delayed deletion, anomaly detection, and break-glass restore.
...
- **Hot path clarity:** start from `snapshots` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `snapshots`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `snapshots`.
