# Design object storage like S3 - System Design Deep Dive

**Problem #51**  
**Category:** Storage  
**Primary pattern:** storage sync  
**Deep-dive focus:** buckets, object metadata, durability, erasure coding, multipart upload

## 0. Interview Framing

A durable storage system where metadata correctness, object durability, sync semantics, lifecycle management, and access control are central.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **buckets**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Upload, download, version, and delete stored objects.
- Enforce namespace, sharing, and retention policies.
- Sync changes across clients or replicas.
- Repair, tier, and audit stored data.
...
- strong durability and metadata correctness.
- resumable upload/download for large files.
- fine-grained authorization and share revocation.
- eventual sync convergence across devices.
- auditable retention and deletion.
...
- Do not make physical deletion the only way to enforce access revocation.
- Do not trust client sync state without server-side version checks.
...
|---|---|
| Users/accounts | 100M users or 1M enterprise tenants |
| Objects/files | 10B+ objects |
| Stored data | 10+ PB with tiered retention |
| Operations | 100K read QPS, 30K write QPS peak |
| Durability target | 11 nines logical durability with continuous repair |
...
```http
GET /v1/objects?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/objects/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
PUT /v1/objects/{id}/parts/{part_number}
Content-Range: bytes 0-1048575/10737418240
```
...
service StoredObjectService {
  ...
  rpc GetStoredObject(GetStoredObjectRequest) returns (StoredObject);
  rpc UpdateStoredObject(UpdateStoredObjectRequest) returns (StoredObject);
  rpc EvaluateStoredObject(EvaluateStoredObjectRequest) returns (EvaluateStoredObjectResponse);
  rpc ListStoredObjectEvents(ListStoredObjectEventsRequest) returns (stream StoredObjectEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "objects.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "objects-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "storedObject_123",
...
- `objects.created.v1`
- `objects.updated.v1`
- `objects.state_changed.v1`
- `objects.deleted_or_expired.v1`
- `objects.policy_denied.v1`
- `objects.operation_failed.v1`
...
```text
object storage like S3 Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Upload/download gateway -> Metadata service -> Chunk/object service -> Sync/change service -> Permission service -> Virus/DLP scanning service
        |       -> Source-of-Truth Write + Transactional Outbox
        |
        +--> Query / Serving Path
        |       -> Metadata service / Search indexer
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the object storage like S3 service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Upload/download gateway -> Metadata service -> Chunk/object service -> Sync/change service -> Permission service -> Virus/DLP scanning service; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
3. **Query path:** read-heavy traffic is served by Metadata service / Search indexer; strong reads fall back to the canonical store when stale projections are unsafe.
4. **Async path:** Search indexer consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
- **Upload/download gateway**: Creates upload sessions, validates chunks/checksums, writes originals to object storage, records metadata, and emits processing events.
...
- **Chunk/object service**: Stores large binary payloads outside OLTP, performs validation/scanning/transcoding where needed, and serves via signed URLs/CDN.
- **Sync/change service**: Replicates changes across regions/stores/providers, tracks lag/conflicts, and supports replay/recovery.
- **Permission service**: Evaluates ACL/RBAC/ABAC decisions, owns sharing grants, inheritance, revocation, and audit evidence for access changes.
- **Lifecycle manager**: Applies TTL, archival, deletion, legal hold, retention, and storage-tier transitions with audit and replay safety.
- **Virus/DLP scanning service**: Scans files/content for malware, policy violations, sensitive data, and quarantines unsafe artifacts before broad exposure.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own object storage like S3 business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Metadata service, Search indexer | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
| Async workers and integrations | Search indexer | Consume committed events, call external systems, retry safely, update projections, run cleanup, and isolate slow dependencies from user-facing latency. |
...
- Metadata DB for namespaces, versions, ACLs, and object pointers.
- Chunk/object storage with replication or erasure coding.
- Change log for sync and index updates.
- Search index for metadata/content.
- Lifecycle catalog for tiering and deletion.

## 6. Low-Level Design

### Core Modules

- `StoredObjectController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `StoredObjectApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `StoredObjectDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `StoredObjectRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `StoredObjectPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `StoredObjectEventPublisher`: writes outbox records and publishes domain events to the broker.
- `StoredObjectReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface StoredObjectRepository {
    Optional<StoredObject> findById(StoredObjectId id, ReadConsistency consistency);
    Page<StoredObject> list(StoredObjectQuery query, Cursor cursor, int limit);
    StoredObject save(StoredObject aggregate, ExpectedVersion expectedVersion);
}

interface StoredObjectPolicyEvaluator {
    PolicyDecision canRead(Principal principal, StoredObject resource);
    PolicyDecision canMutate(Principal principal, StoredObject resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> StoredObject
  -> emits -> StoredObjectEvent
...
|---|---|
| `namespaces` | `id, tenant_id, parent_id, name, acl_policy_id, quota_bytes, created_at` |
| `objects` | `id, namespace_id, name, current_version_id, size_bytes, checksum, storage_class, state` |
| `object_versions` | `id, object_id, version_num, object_ref, size_bytes, checksum, created_by, created_at` |
| `chunks` | `id, object_version_id, chunk_index, hash, size_bytes, storage_nodes, state` |
| `access_grants` | `id, resource_id, principal_id, role, expires_at, revoked_at, created_at` |

### Database Technology Choice

| Workload / Data | Recommended Database / Store | Why This Choice Fits |
| --- | --- | --- |
| Source of truth / primary store | strongly consistent metadata store such as Spanner/CockroachDB/FoundationDB/PostgreSQL for namespaces, object metadata, versions, ACLs, and leases; distributed object/chunk store for data blocks | metadata correctness controls ownership, visibility, and versioning, while chunks/blobs need cheap durable replicated storage |
| Hot serving / cache | Redis/local metadata cache for hot directory listings, signed upload sessions, chunk maps, and sync cursors | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
| Event stream / outbox | Kafka/Pulsar for sync events, lifecycle actions, indexing, audit, and repair workflows | decouples projections, notifications, analytics, search indexing, and recovery from the write path |
| Search / analytics | OpenSearch for metadata search; ClickHouse/warehouse for storage analytics and billing | serves text/filter/OLAP queries without overloading transactional tables |
| Large immutable payloads | erasure-coded object storage/distributed file system for chunks, versions, backups, and immutable snapshots | large or immutable data is cheaper, durable, and easier to lifecycle outside OLTP rows |
...
- Primary partition key: `bucket_id/object_key hash, namespace_id, or file_id; time bucket for audit/lifecycle events`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `bucket + object_key/version_id or file_id` should be single-partition whenever possible.
...
- Hot partition mitigation: split hot directories/buckets, use prefix hashing, cache metadata, and move large scans to offline jobs.
...
- Required secondary indexes: `owner_id + path, parent_id + name, checksum, lifecycle_state + transition_at, acl_subject`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for metadata and namespace correctness | strong consistency for namespace mutations, ACLs, version pointers, leases, and delete markers | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for derived previews, search, sync hints, and cached listings | Eventual consistency for sync notifications, search index, thumbnails/previews, analytics, and lifecycle transitions | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
For this design, use S3/GCS/blob storage for payloads, Kafka for metadata/lifecycle events, Debezium/CDC for search and sync projections, batch jobs for lifecycle/compaction/repair, Iceberg tables for storage inventory, and Athena/Trino for audit queries.
...
- **buckets**: Model bucket-level ownership, policy, location, versioning, and lifecycle.
- **object metadata**: Keep metadata strongly consistent enough for authorization and lifecycle.
- **durability**: Use replication, erasure coding, scrubbing, and repair loops.
- **erasure coding**: Use erasure coding for cold large objects and replication for hot/small objects.
- **multipart upload**: Track parts, checksums, and commit state to support resumability.
...
- raw and replicated storage bytes.
- PUT/GET request volume.
- egress and CDN usage.
- indexing and metadata DB capacity.
- lifecycle scans and repair jobs.
...
- **Hot path clarity:** start from `buckets` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Lifecycle manager; avoid vague boxes that do not own data or decisions.
...
- **Event model:** use `objects.created.v1, objects.updated.v1, objects.state_changed.v1, objects.deleted_or_expired.v1, objects.policy_denied.v1, objects.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `buckets`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `buckets`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Lifecycle manager.
