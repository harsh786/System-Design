# Design a document versioning system - System Design Deep Dive

**Problem #58**  
**Category:** Collaboration/storage  
**Primary pattern:** storage sync  
**Deep-dive focus:** immutable versions, diffs, conflict resolution, retention
...
```text
a document versioning system Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the document versioning system service boundary.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own a document versioning system business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **immutable versions**: Store immutable version records and move mutable pointers atomically.
- **diffs**: Store diffs for efficient history but keep reconstructable snapshots.
...
- **Hot path clarity:** start from `immutable versions` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `immutable versions`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `immutable versions`.
