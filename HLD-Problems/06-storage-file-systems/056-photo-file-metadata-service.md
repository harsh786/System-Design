# Design a photo/file metadata service - System Design Deep Dive

**Problem #56**  
**Category:** Storage metadata  
**Primary pattern:** storage sync  
**Deep-dive focus:** metadata indexing, ACLs, search, versioning
...
```http
POST /v1/search-documents/upload-sessions
Idempotency-Key: <uuid>
...
```http
PUT /v1/search-documents/{id}/parts/{part_number}
Content-Range: bytes 0-1048575/10737418240
```

```http
POST /v1/search-documents/{id}/complete
Idempotency-Key: <uuid>
...
```text
a photo/file metadata service Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the photo/file metadata service boundary.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own a photo/file metadata service business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **metadata indexing**: Keep canonical metadata strongly consistent enough for rights, deletion, and discovery.
- **acls**: Store ACLs as first-class versioned records and audit every permission change.
...
- **Hot path clarity:** start from `metadata indexing` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `metadata indexing`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `metadata indexing`.
