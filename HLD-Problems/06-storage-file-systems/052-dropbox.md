# Design Dropbox - System Design Deep Dive

**Problem #52**  
**Category:** File sync  
**Primary pattern:** storage sync  
**Deep-dive focus:** block sync, dedupe, conflict resolution, offline clients
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **block sync**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Dropbox service boundary.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own Dropbox business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **block sync**: Use content-defined chunking and hashes to upload only changed blocks.
- **dedupe**: Deduplicate alerts by fingerprint and suppress during maintenance.
- **conflict resolution**: Expose deterministic conflict copies or merge rules; never silently lose edits.
- **offline clients**: Use change cursors and idempotent sync operations.
...
- **Hot path clarity:** start from `block sync` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `block sync`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `block sync`.
