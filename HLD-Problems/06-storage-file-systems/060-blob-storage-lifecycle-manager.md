# Design a blob storage lifecycle manager - System Design Deep Dive

**Problem #60**  
**Category:** Storage/cost  
**Primary pattern:** storage sync  
**Deep-dive focus:** tiering, TTL, archival, deletion, compliance holds
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **tiering**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
a blob storage lifecycle manager Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the blob storage lifecycle manager service boundary.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own a blob storage lifecycle manager business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **tiering**: Automate movement across hot/warm/cold/archive tiers by policy and access pattern.
- **ttl**: Represent expiration as indexed metadata plus async cleanup; do not rely only on physical deletion for correctness.
- **archival**: Model restore latency and retrieval cost in API contracts.
- **deletion**: Use tombstones, legal holds, delayed physical deletion, and audit logs.
- **compliance holds**: Make legal/compliance holds override lifecycle deletion.
...
- **Hot path clarity:** start from `tiering` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `tiering`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `tiering`.
