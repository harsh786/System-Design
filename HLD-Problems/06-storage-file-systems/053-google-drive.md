# Design Google Drive - System Design Deep Dive

**Problem #53**  
**Category:** File storage  
**Primary pattern:** storage sync  
**Deep-dive focus:** permissions, sharing, versioning, sync, search
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **permissions**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Google Drive service boundary.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own Google Drive business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **permissions**: Cache permissions carefully and invalidate on share/revoke changes.
- **sharing**: Use explicit ACL/share-link records and support revocation.
- **versioning**: Version mutable assets/configs and keep rollback paths explicit.
- **sync**: Use per-device queues and cursor-based sync so reconnects are deterministic.
...
- **Hot path clarity:** start from `permissions` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `permissions`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `permissions`.
