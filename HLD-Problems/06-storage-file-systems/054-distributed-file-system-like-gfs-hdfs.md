# Design a distributed file system like GFS/HDFS - System Design Deep Dive

**Problem #54**  
**Category:** Storage infra  
**Primary pattern:** storage sync  
**Deep-dive focus:** master metadata, chunk servers, replication, consistency
...
```text
a distributed file system like GFS/HDFS Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the distributed file system like GFS/HDFS service boundary.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own a distributed file system like GFS/HDFS business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **master metadata**: Keep canonical metadata strongly consistent enough for rights, deletion, and discovery.
- **chunk servers**: Separate metadata master from chunk data and repair under-replicated chunks.
...
- **Hot path clarity:** start from `master metadata` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `master metadata`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `master metadata`.
