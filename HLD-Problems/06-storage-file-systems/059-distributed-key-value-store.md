# Design a distributed key-value store - System Design Deep Dive

**Problem #59**  
**Category:** Storage infra  
**Primary pattern:** storage sync  
**Deep-dive focus:** consistent hashing, replication, quorum, vector clocks
...
```text
a distributed key-value store Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the distributed key-value store service boundary.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own a distributed key-value store business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **consistent hashing**: Use consistent hashing to reduce key movement during scale-out and node loss.
- **replication**: Use replication for availability, read scale, and durability; state the consistency trade-off.
- **quorum**: Use read/write quorum trade-offs explicitly for latency vs consistency.
- **vector clocks**: Use vector clocks or version vectors to detect concurrent writes where last-write-wins is unsafe.
...
- **Hot path clarity:** start from `consistent hashing` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `consistent hashing`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `consistent hashing`.
