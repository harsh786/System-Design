# Design Zoom / video conferencing - System Design Deep Dive

**Problem #17**  
**Category:** Realtime media  
**Primary pattern:** realtime collab  
**Deep-dive focus:** SFU vs MCU, WebRTC, screen share, recording, bandwidth adaptation
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **SFU vs MCU**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Create meetings and manage participants.
- Route audio/video via SFU clusters.
- Support screen share, recording, and chat.
- Adapt media quality to bandwidth.
...
```http
GET /v1/meetings?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/meetings/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
GET /v1/meetings/{id}/sync?cursor=<device_cursor>&limit=100
```

```http
POST /v1/meetings/{id}/read-receipts
Idempotency-Key: <uuid>
...
service MeetingService {
  ...
  rpc EvaluateMeeting(EvaluateMeetingRequest) returns (EvaluateMeetingResponse);
  rpc ListMeetingEvents(ListMeetingEventsRequest) returns (stream MeetingEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "meetings.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "meetings-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "meeting_123",
...
- `meetings.created.v1`
- `meetings.updated.v1`
- `meetings.state_changed.v1`
- `meetings.deleted_or_expired.v1`
- `meetings.policy_denied.v1`
- `meetings.operation_failed.v1`
...
WebSocket gateway fleet
        ...
        |       -> Message or collaboration service -> Media/attachment service -> Session registry
        ...
        |       -> Fanout workers / Moderation and abuse service / Notification service / Search indexer
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Zoom / video conferencing service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Message or collaboration service -> Media/attachment service -> Session registry; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
3. **Query path:** read-heavy traffic is served by Search indexer; strong reads fall back to the canonical store when stale projections are unsafe.
4. **Async path:** Fanout workers / Moderation and abuse service / Notification service / Search indexer consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
| Core domain services | Message or collaboration service, Media/attachment service, Session registry | Own Zoom / video conferencing business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Search indexer | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
| Async workers and integrations | Fanout workers, Moderation and abuse service, Notification service, Search indexer | Consume committed events, call external systems, retry safely, update projections, run cleanup, and isolate slow dependencies from user-facing latency. |
...
- `MeetingController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `MeetingApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `MeetingDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `MeetingRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `MeetingPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `MeetingEventPublisher`: writes outbox records and publishes domain events to the broker.
- `MeetingReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface MeetingRepository {
    Optional<Meeting> findById(MeetingId id, ReadConsistency consistency);
    Page<Meeting> list(MeetingQuery query, Cursor cursor, int limit);
    Meeting save(Meeting aggregate, ExpectedVersion expectedVersion);
}

interface MeetingPolicyEvaluator {
    PolicyDecision canRead(Principal principal, Meeting resource);
    PolicyDecision canMutate(Principal principal, Meeting resource, Action action);
}
...
|---|---|
| `meetings` | `id, tenant_id, host_id, region, state, started_at, ended_at, recording_policy` |
| `participants` | `meeting_id, user_id, device_id, role, joined_at, left_at, media_state` |
| `media_sessions` | `meeting_id, participant_id, sfu_id, audio_ssrc, video_ssrc, quality_stats_ref` |
| `recordings` | `id, meeting_id, object_ref, transcript_ref, state, created_at` |
...
- **sfu vs mcu**: Prefer SFU for scalable multiparty video; use MCU only when server-side compositing is required.
- **webrtc**: Plan signaling, ICE/STUN/TURN, NAT traversal, codec negotiation, and QoS telemetry.
- **screen share**: Treat screen share as a separate media track with higher readability constraints.
- **recording**: Record asynchronously from media streams and process into durable assets.
- **bandwidth adaptation**: Use simulcast/SVC and receiver-side bandwidth estimation.
...
- **Hot path clarity:** start from `SFU vs MCU` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `meetings.created.v1, meetings.updated.v1, meetings.state_changed.v1, meetings.deleted_or_expired.v1, meetings.policy_denied.v1, meetings.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `SFU vs MCU`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `SFU vs MCU`.
