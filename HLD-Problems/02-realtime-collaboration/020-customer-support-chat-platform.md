# Design a customer-support chat platform - System Design Deep Dive

**Problem #20**  
**Category:** SaaS/realtime  
**Primary pattern:** realtime collab  
**Deep-dive focus:** routing agents, queues, transcripts, SLAs, escalation
...
```text
a customer-support chat platform Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the customer-support chat platform service boundary.
...
| Core domain services | Message or collaboration service, Media/attachment service, Moderation and abuse service, Session registry | Own a customer-support chat platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **routing agents**: Match work to agents using skills, availability, priority, SLA, and fairness.
- **queues**: Expose wait time, priority, assignment, and escalation state as first-class data.
- **transcripts**: Store transcripts immutably with privacy controls and retention policy.
- **slas**: Represent SLA deadlines explicitly and alert before breach.
...
- **Hot path clarity:** start from `routing agents` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `routing agents`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `routing agents`.
