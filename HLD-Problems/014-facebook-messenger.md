# Design Facebook Messenger - System Design Deep Dive

**Problem #14**  
**Category:** Realtime/social  
**Primary pattern:** realtime collab  
**Deep-dive focus:** group chat, online status, media sharing, delivery guarantees
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **group chat**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Facebook Messenger service boundary.
...
| Core domain services | Message or collaboration service, Media/attachment service, Session registry | Own Facebook Messenger business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **group chat**: Use conversation partitions and fanout workers; avoid O(N) synchronous writes for large groups.
- **online status**: Use heartbeats and last-seen timestamps with anti-flapping dampening.
- **media sharing**: Move media to object storage/CDN and pass references through the core transaction path.
- **delivery guarantees**: Track delivery state across email, storage, and web download targets.
...
- **Hot path clarity:** start from `group chat` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `group chat`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `group chat`.
