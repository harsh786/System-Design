# Design Parking Lot / Smart Parking Marketplace - System Design Deep Dive

**Problem #111**  
**Category:** Marketplace/IoT/geo  
**Primary pattern:** spot inventory + reservation + sensor integration  
**Deep-dive focus:** spot discovery, reservations, sensors, gates, settlement

## 0. Interview Framing

Smart parking combines geo discovery, scarce reservations, IoT occupancy, access control, dynamic pricing, and settlement.

Actors: drivers, lot operators, sensors, gates, enforcement, support. The highest-risk path is **search lot -> reserve spot -> enter -> exit/pay -> settle operator**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Drivers search lots by location/time/vehicle/price/EV/covered/accessibility and reserve/start sessions.
- Operators manage lots, spots, rates, blackout dates, sensors, gates, permits, and enforcement.
- Sensors/gates report occupancy, entry, exit, plate, and payment status.
- Support dynamic pricing, overstay fees, cancellation, refunds, validation codes, and settlement.
- Expose support/admin operations to inspect, replay, correct, and annotate parking_session history.
...
- Reservation and sensor state conflicts are resolved by explicit policy.
- Capacity is not oversold beyond configured tolerance.
- Gate access works during brief cloud outages.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Reservation/session records plus lot capacity buckets; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Lots | 100K lots, millions spots |
| Search | High geo reads |
| Reservations | 5M/day city events |
| Sensor events | Millions/minute large deployments |
| Retention | Sessions/payment/enforcement multi-year |
...
- Keep the online parking_session aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| GET /v1/parking/search | lat, lon, time_range, vehicle | Find lots. |
| POST /v1/parking/reservations | lot_id, time_range, vehicle_plate | Reserve. |
| POST /v1/parking/sessions/{id}/entry | access_token, gate_id | Start session. |
| POST /v1/parking/sessions/{id}/exit | gate_id, sensor_evidence | End/pay. |
| GET /v1/parking-sessions/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/parking-sessions/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service ParkingService {
  rpc Create(CreateParkingRequest) returns (Parking);
  rpc Get(GetParkingRequest) returns (Parking);
  rpc Transition(TransitionParkingRequest) returns (Parking);
  rpc Reconcile(ReconcileParkingRequest) returns (ReconcileResult);
}
...
- parking.created.v1
- parking.validated.v1
- parking.state_changed.v1
- parking.committed.v1
- parking.failed.v1
- parking.reversed.v1
- parking.reconciliation_completed.v1
- parking.manual_review_required.v1
...
```text
Parking Lot / Smart Parking Marketplace Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Inventory service -> Sensor ingestion -> Idempotency service -> Parking session orchestrator -> State transition engine -> Lot service
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Parking Lot / Smart Parking Marketplace service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Inventory service -> Sensor ingestion -> Idempotency service -> Parking session orchestrator -> State transition engine -> Lot service; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
- **Lot service**: lot metadata, spot types, rates, rules.
- **Inventory service**: reservable capacity by lot/zone/time.
- **Sensor ingestion**: Creates upload sessions, validates chunks/checksums, writes originals to object storage, records metadata, and emits processing events.
- **Access service**: QR/license/access tokens and validation.
- **Settlement**: operator payout, fees, taxes, penalties.
...
| Core domain services | Inventory service, Sensor ingestion, Idempotency service, Parking session orchestrator, State transition engine, Lot service | Own Parking Lot / Smart Parking Marketplace business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Parking session orchestrator**: owns parking_session state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed parking_session transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Parking Lot / Smart Parking Marketplace: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| parking_lots | lot_id, operator_id, location, capacity | Lot. |
| parking_spots | spot_id, lot_id, type, sensor_id, state | Spot. |
| rates | rate_id, lot_id, time_window, price_rule | Rate. |
| reservations | reservation_id, user_id, lot_id, time_range, state | Reservation. |
| parking_sessions | session_id, reservation_id, plate, entry_at, exit_at | Session. |
| sensor_events | event_id, sensor_id, event_type, confidence | Telemetry. |
...
1. Search ranks lots by geo, availability, vehicle constraints, and price.
2. Reservation conditionally reserves capacity and issues access token.
3. Entry validates token/plate and starts session.
4. Exit computes final charge/overstay/refund and settlement.
...
- Capacity buckets are enough for unassigned lots; spot-level state for assigned parking.
- Sensor confidence should not automatically override paid reservations.
- Offline gates validate signed short-lived tokens.
- Source-of-truth boundary: Reservation/session records plus lot capacity buckets owns correctness; all low-latency read models are disposable projections.
...
- Protect plates, location history, payment data, and operator financials.
- Detect fake reservations, plate spoofing, sensor tampering, gate abuse, refund fraud.
...
- SLIs: parking_session mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do sensor occupancy and reservations reconcile?
- How do gates work offline?
- How do you avoid overselling?
- Where exactly is parking_session source of truth stored?
...
- **Correctness boundary:** Reservation/session records plus lot capacity buckets is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `search lot -> reserve spot -> enter -> exit/pay -> settle operator` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Lot service, Inventory service, Sensor ingestion, Access service, Settlement, Idempotency service; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `parking_lots, parking_spots, rates, reservations, parking_sessions, sensor_events` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `parking.created.v1, parking.validated.v1, parking.state_changed.v1, parking.committed.v1, parking.failed.v1, parking.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `search lot -> reserve spot -> enter -> exit/pay -> settle operator`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `search lot -> reserve spot -> enter -> exit/pay -> settle operator`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Parking session orchestrator, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
