# Design a notification system - System Design Deep Dive

**Problem #16**  
**Category:** Product infra  
**Primary pattern:** realtime collab  
**Deep-dive focus:** email/SMS/push, preferences, templates, retries, priority
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **email/SMS/push**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Send email, SMS, push, and in-app notifications.
- Apply user preferences and quiet hours.
- Retry transient failures and route permanent failures to DLQ.
- Support priority and transactional vs marketing separation.
...
```http
GET /v1/notifications?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/notifications/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/notifications/{id}/messages
Idempotency-Key: <uuid>
...
```http
GET /v1/notifications/{id}/sync?cursor=<device_cursor>&limit=100
```

```http
POST /v1/notifications/{id}/read-receipts
Idempotency-Key: <uuid>
...
service NotificationService {
  ...
  rpc EvaluateNotification(EvaluateNotificationRequest) returns (EvaluateNotificationResponse);
  rpc ListNotificationEvents(ListNotificationEventsRequest) returns (stream NotificationEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "notifications.updated.v1",
...
- `notifications.created.v1`
- `notifications.updated.v1`
- `notifications.state_changed.v1`
- `notifications.deleted_or_expired.v1`
- `notifications.policy_denied.v1`
- `notifications.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the notification system service boundary.
...
| Core domain services | Message or collaboration service, Media/attachment service, Moderation and abuse service, Session registry | Own a notification system business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `NotificationController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `NotificationApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `NotificationDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `NotificationRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `NotificationPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `NotificationEventPublisher`: writes outbox records and publishes domain events to the broker.
- `NotificationReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface NotificationRepository {
    Optional<Notification> findById(NotificationId id, ReadConsistency consistency);
    Page<Notification> list(NotificationQuery query, Cursor cursor, int limit);
    Notification save(Notification aggregate, ExpectedVersion expectedVersion);
}

interface NotificationPolicyEvaluator {
    PolicyDecision canRead(Principal principal, Notification resource);
    PolicyDecision canMutate(Principal principal, Notification resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> Notification
...
|---|---|
| `notifications` | `id, user_id, channel, template_id, priority, dedupe_key, state, scheduled_at` |
| `templates` | `id, name, version, locale, body_ref, required_vars, state` |
| `preferences` | `user_id, channel, topic, enabled, quiet_hours, updated_at` |
| `delivery_attempts` | `notification_id, provider, attempt, status, provider_message_id, error_code, created_at` |
...
- **email**: Abstract providers behind channel adapters with provider failover and per-channel rate limits.
- **preferences**: Evaluate preferences before enqueueing expensive provider work.
...
- **Hot path clarity:** start from `email/SMS/push` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `notifications.created.v1, notifications.updated.v1, notifications.state_changed.v1, notifications.deleted_or_expired.v1, notifications.policy_denied.v1, notifications.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `email/SMS/push`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `email/SMS/push`.
