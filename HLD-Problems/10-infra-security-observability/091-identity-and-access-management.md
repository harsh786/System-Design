# Design identity and access management - System Design Deep Dive

**Problem #91**  
**Category:** Security  
**Primary pattern:** infra security  
**Deep-dive focus:** auth, federation, RBAC/ABAC, sessions, audit
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **auth**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```http
GET /v1/identities?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/identities/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
service IdentityService {
  ...
  rpc EvaluateIdentity(EvaluateIdentityRequest) returns (EvaluateIdentityResponse);
  rpc ListIdentityEvents(ListIdentityEventsRequest) returns (stream IdentityEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "identities.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "identities-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "identity_123",
...
- `identities.created.v1`
- `identities.updated.v1`
- `identities.state_changed.v1`
- `identities.deleted_or_expired.v1`
- `identities.policy_denied.v1`
- `identities.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the identity and access management service boundary.
...
| Core domain services | Propagation service, Policy engine, Versioned config store, Data-plane evaluator or agent, SRE console | Own identity and access management business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `IdentityController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `IdentityApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `IdentityDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `IdentityRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `IdentityPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `IdentityEventPublisher`: writes outbox records and publishes domain events to the broker.
- `IdentityReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface IdentityRepository {
    Optional<Identity> findById(IdentityId id, ReadConsistency consistency);
    Page<Identity> list(IdentityQuery query, Cursor cursor, int limit);
    Identity save(Identity aggregate, ExpectedVersion expectedVersion);
}

interface IdentityPolicyEvaluator {
    PolicyDecision canRead(Principal principal, Identity resource);
    PolicyDecision canMutate(Principal principal, Identity resource, Action action);
}
...
|---|---|
| `identities` | `id, tenant_id, subject_type, external_id, profile_ref, state, created_at` |
| `groups` | `id, tenant_id, name, description, state, created_at` |
| `roles` | `id, tenant_id, name, permissions_ref, version, state` |
| `policies` | `id, tenant_id, policy_type, body_json, version, state, created_by, created_at` |
| `sessions` | `id, identity_id, device_id, issued_at, expires_at, revoked_at, risk_score` |
...
- **federation**: Support OIDC/SAML federation with tenant-specific policies and safe metadata updates.
- **rbac**: Use RBAC for coarse roles and ABAC/policy engine for contextual decisions.
...
- **Hot path clarity:** start from `auth` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `identities.created.v1, identities.updated.v1, identities.state_changed.v1, identities.deleted_or_expired.v1, identities.policy_denied.v1, identities.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `auth`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `auth`.
