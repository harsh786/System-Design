# Design metrics monitoring system like Datadog - System Design Deep Dive

**Problem #96**  
**Category:** Observability  
**Primary pattern:** infra security  
**Deep-dive focus:** ingestion, aggregation, storage, dashboards, alerting
...
```http
GET /v1/metrics?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/metrics/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```protobuf
service MetricSeriesService {
  rpc CreateMetricSeries(CreateMetricSeriesRequest) returns (MetricSeries);
  rpc GetMetricSeries(GetMetricSeriesRequest) returns (MetricSeries);
  rpc UpdateMetricSeries(UpdateMetricSeriesRequest) returns (MetricSeries);
  rpc EvaluateMetricSeries(EvaluateMetricSeriesRequest) returns (EvaluateMetricSeriesResponse);
  rpc ListMetricSeriesEvents(ListMetricSeriesEventsRequest) returns (stream MetricSeriesEvent);
}
```
...
- `metrics.created.v1`
- `metrics.updated.v1`
- `metrics.state_changed.v1`
- `metrics.deleted_or_expired.v1`
- `metrics.policy_denied.v1`
- `metrics.operation_failed.v1`
...
```text
metrics monitoring system like Datadog Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the metrics monitoring system like Datadog service boundary.
...
| Core domain services | Propagation service, Policy engine, Versioned config store, Data-plane evaluator or agent, SRE console | Own metrics monitoring system like Datadog business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `MetricSeriesController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `MetricSeriesApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `MetricSeriesDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `MetricSeriesRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `MetricSeriesPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `MetricSeriesEventPublisher`: writes outbox records and publishes domain events to the broker.
- `MetricSeriesReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface MetricSeriesRepository {
    Optional<MetricSeries> findById(MetricSeriesId id, ReadConsistency consistency);
    Page<MetricSeries> list(MetricSeriesQuery query, Cursor cursor, int limit);
    MetricSeries save(MetricSeries aggregate, ExpectedVersion expectedVersion);
}

interface MetricSeriesPolicyEvaluator {
    PolicyDecision canRead(Principal principal, MetricSeries resource);
    PolicyDecision canMutate(Principal principal, MetricSeries resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> MetricSeries
  -> emits -> MetricSeriesEvent
...
|---|---|
| `metric_series` | `id, tenant_id, metric_name, tag_set_hash, type, retention_policy, created_at` |
| `metric_points` | `series_id, ts_bucket, timestamp, value, tags_ref` |
| `rollups` | `series_id, granularity, window_start, count, sum, min, max, p95, p99` |
| `dashboards` | `id, tenant_id, name, layout_ref, owner_id, updated_at` |
| `alert_rules` | `id, tenant_id, query_ref, threshold, window, severity, state` |
| `incidents` | `id, alert_rule_id, fingerprint, state, opened_at, resolved_at` |
...
- **aggregation**: Make aggregation explicit in the data model, APIs, scaling plan, and operational dashboards.
- **storage**: Separate metadata from immutable content blobs and keep render/caching decisions out of the write path.
- **dashboards**: Make dashboards explicit in the data model, APIs, scaling plan, and operational dashboards.
- **alerting**: Make alerting explicit in the data model, APIs, scaling plan, and operational dashboards.
...
- **Event model:** use `metrics.created.v1, metrics.updated.v1, metrics.state_changed.v1, metrics.deleted_or_expired.v1, metrics.policy_denied.v1, metrics.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
