# Design Twitch live streaming - System Design Deep Dive

**Problem #33**  
**Category:** Live media  
**Primary pattern:** media content  
**Deep-dive focus:** RTMP ingest, HLS/DASH, low latency, chat scale
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **RTMP ingest**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
Twitch live streaming Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Twitch live streaming service boundary.
...
| Core domain services | Upload/ingest service, Metadata service | Own Twitch live streaming business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **rtmp ingest**: Terminate ingest regionally and publish to transcode/packaging pipelines.
- **hls**: Package media into segment manifests and tune segment length for latency/cost trade-offs.
- **low latency**: Use low-latency HLS/DASH/WebRTC paths depending on delay target.
- **chat scale**: Keep stream chat separate from media path and throttle during spikes.
...
- **Hot path clarity:** start from `RTMP ingest` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `RTMP ingest`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `RTMP ingest`.
