# Design Instagram Stories / ephemeral media - System Design Deep Dive

**Problem #37**  
**Category:** Social/media  
**Primary pattern:** media content  
**Deep-dive focus:** TTL, media processing, viewer receipts, ranking
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **TTL**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```http
POST /v1/social-objects/upload-sessions
Idempotency-Key: <uuid>
...
```http
POST /v1/social-objects/{id}/processing-jobs
Idempotency-Key: <uuid>
...
```http
GET /v1/social-objects/{id}/playback-manifest?device=mobile&network=wifi
```
...
```text
Instagram Stories / ephemeral media Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Instagram Stories / ephemeral media service boundary.
...
| Core domain services | Upload/ingest service, Metadata service | Own Instagram Stories / ephemeral media business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **media processing**: Move media to object storage/CDN and pass references through the core transaction path.
- **viewer receipts**: Record receipts asynchronously and aggregate for read views.
...
- **Hot path clarity:** start from `TTL` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `TTL`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `TTL`.
