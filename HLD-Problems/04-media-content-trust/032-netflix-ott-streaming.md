# Design Netflix / OTT streaming - System Design Deep Dive

**Problem #32**  
**Category:** Media/video  
**Primary pattern:** media content  
**Deep-dive focus:** catalog, playback, DRM, regional CDN, resilience
...
```text
Netflix / OTT streaming Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Netflix / OTT streaming service boundary.
...
| Core domain services | Upload/ingest service, Metadata service | Own Netflix / OTT streaming business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **playback**: Optimize startup latency, DRM/license fetch, manifest generation, and CDN locality.
- **drm**: Separate license issuance from media serving and log access for audits.
- **regional cdn**: Place origins and caches near users while enforcing content rights and residency.
- **resilience**: Support fallback bitrates, alternate CDNs, and origin failover.
