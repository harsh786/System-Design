# Design YouTube - System Design Deep Dive

**Problem #31**  
**Category:** Media/video  
**Primary pattern:** media content  
**Deep-dive focus:** upload, transcoding, CDN, metadata, recommendations
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **upload**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the YouTube service boundary.
...
| Core domain services | Upload/ingest service, Metadata service | Own YouTube business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **upload**: Use resumable direct upload with checksum verification and async processing.
- **transcoding**: Run priority-aware distributed jobs and store idempotent job state.
...
- **Hot path clarity:** start from `upload` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `upload`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `upload`.
