# Design Google Photos - System Design Deep Dive

**Problem #36**  
**Category:** Media/ML  
**Primary pattern:** media content  
**Deep-dive focus:** dedupe, thumbnails, metadata search, albums, sharing
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **dedupe**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Google Photos service boundary.
...
| Core domain services | Upload/ingest service, Metadata service | Own Google Photos business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **thumbnails**: Generate deterministic derivatives and avoid blocking upload completion on every derivative.
- **metadata search**: Index EXIF, labels, albums, faces where allowed, and sharing ACLs.
- **albums**: Treat albums as metadata collections referencing immutable media objects.
...
- **Hot path clarity:** start from `dedupe` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `dedupe`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `dedupe`.
