# Design Spotify - System Design Deep Dive

**Problem #34**  
**Category:** Media/audio  
**Primary pattern:** media content  
**Deep-dive focus:** audio streaming, playlists, offline mode, recommendations
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Spotify service boundary.
...
| Core domain services | Upload/ingest service, Metadata service | Own Spotify business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **audio streaming**: Optimize small-object segment caching and offline license windows.
- **playlists**: Model playlist mutations as ordered, idempotent operations.
- **offline mode**: Use device-bound encrypted downloads with revocation and expiry.
...
- **Hot path clarity:** start from `audio streaming` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `audio streaming`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `audio streaming`.
