# Design Auction / Bidding Platform - System Design Deep Dive

**Problem #114**  
**Category:** Marketplace/commerce  
**Primary pattern:** auction state machine + bid ordering  
**Deep-dive focus:** bid validation, proxy bids, anti-sniping, payment, payout

## 0. Interview Framing

Auctions need strict per-auction bid ordering, transparent rules, anti-sniping, reserve prices, payment collection, and seller payout.

Actors: buyers, sellers, admins, payment providers. The highest-risk path is **bid -> validate/order -> update winner -> close -> payment/payout**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Sellers create auctions with item, reserve, start price, increment, duration, shipping, and rules.
- Buyers place bids, max/proxy bids, watch auctions, and pay if they win.
- System closes auctions, determines winner, collects payment, handles non-payment/disputes/refunds/payout.
- Admins moderate listings, suspicious bidding, and policy violations.
- Expose support/admin operations to inspect, replay, correct, and annotate bid history.
...
- Per-auction bid ordering is deterministic.
- Bid accepted only if state, amount, increment, and eligibility pass.
- Auction close happens exactly once.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Per-auction bid log and auction state; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Auctions | 10M active |
| Bids | 100M/day, hot skew |
| Watchers | Millions notifications |
| Close jobs | Time-bucket spikes |
| Retention | Bid/payment/dispute multi-year |
...
- Keep the online bid aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/auctions | item, reserve, start_price, duration | Create auction. |
| POST /v1/auctions/{id}/bids | amount, max_proxy_amount | Place bid. |
| POST /v1/auctions/{id}/close | expected_version | Finalize. |
| POST /v1/auction-payments | auction_id, payment_method | Winner payment. |
| GET /v1/bids/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/bids/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service AuctionService {
  rpc Create(CreateAuctionRequest) returns (Auction);
  rpc Get(GetAuctionRequest) returns (Auction);
  rpc Transition(TransitionAuctionRequest) returns (Auction);
  rpc Reconcile(ReconcileAuctionRequest) returns (ReconcileResult);
}
...
- auction.created.v1
- auction.validated.v1
- auction.state_changed.v1
- auction.committed.v1
- auction.failed.v1
- auction.reversed.v1
- auction.reconciliation_completed.v1
- auction.manual_review_required.v1
...
```text
Auction / Bidding Platform Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Auction service -> Bid service -> Payment/payout -> Idempotency service -> Auction bid service -> State transition engine
        ...
        |       -> Notification service / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager / Reconciliation worker
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Auction / Bidding Platform service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Auction service -> Bid service -> Payment/payout -> Idempotency service -> Auction bid service -> State transition engine; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
4. **Async path:** Notification service / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager / Reconciliation worker consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
- **Auction service**: listing, timing, rules, reserve, state.
- **Bid service**: validation, sequencing, current winner/price.
- **Proxy engine**: max bids and increments.
- **Notification service**: outbid/watched/won/lost.
- **Payment/payout**: winner payment and seller payout.
...
| Core domain services | Auction service, Bid service, Payment/payout, Idempotency service, Auction bid service, State transition engine | Own Auction / Bidding Platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Auction bid service**: owns bid state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed bid transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Auction / Bidding Platform: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| auctions | auction_id, seller_id, state, starts_at, ends_at | Auction. |
| bids | bid_id, auction_id, bidder_id, amount, sequence | Bid. |
| proxy_bids | auction_id, bidder_id, max_amount | Proxy. |
| watchers | auction_id, user_id, pref | Watch. |
| auction_closes | auction_id, close_job_id, state | Close. |
...
1. Bid validates user/auction/time/amount/risk and sequencer applies it.
2. Proxy engine compares max bids and updates visible price.
3. Anti-sniping extends end time for last-minute valid bids.
4. Close finalizes winner/reserve outcome and starts payment/payout.
...
- Single writer per hot auction preserves ordering.
- Bid log is source of truth; current winner is projection.
- Close is fenced state transition, not just timer.
- Source-of-truth boundary: Per-auction bid log and auction state owns correctness; all low-latency read models are disposable projections.
...
- Protect bidder identity, payment data, seller info, and fraud signals.
- Detect shill bidding, bid shielding, non-paying winners, counterfeit listings, ATO.
...
- SLIs: bid mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do you order concurrent bids?
- How does proxy bidding work?
- How do close races work?
- Where exactly is bid source of truth stored?
...
- **Correctness boundary:** Per-auction bid log and auction state is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `bid -> validate/order -> update winner -> close -> payment/payout` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Auction service, Bid service, Proxy engine, Notification service, Payment/payout, Idempotency service; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `auctions, bids, proxy_bids, watchers, auction_closes, idempotency_keys` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `auction.created.v1, auction.validated.v1, auction.state_changed.v1, auction.committed.v1, auction.failed.v1, auction.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `bid -> validate/order -> update winner -> close -> payment/payout`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `bid -> validate/order -> update winner -> close -> payment/payout`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Auction bid service, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
