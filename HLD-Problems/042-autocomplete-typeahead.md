# Design autocomplete / typeahead - System Design Deep Dive

**Problem #42**  
**Category:** Search  
**Primary pattern:** search geo  
**Deep-dive focus:** prefix index, trie, ranking, personalization, latency
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the autocomplete / typeahead service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own autocomplete / typeahead business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **prefix index**: Use trie/FST/prefix tables for fast top-k prefix lookups.
- **trie**: Compact tries/FSTs and cache hot prefixes in memory.
...
- **latency**: Budget latency per dependency and define asynchronous fallback for non-critical checks.
...
- **Hot path clarity:** start from `prefix index` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `prefix index`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `prefix index`.
