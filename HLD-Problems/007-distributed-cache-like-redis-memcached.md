# Design a distributed cache like Redis/Memcached - System Design Deep Dive

**Problem #7**  
**Category:** Storage/platform  
**Primary pattern:** core platform  
**Deep-dive focus:** sharding, replication, eviction, hot keys, cache stampede

## 0. Interview Framing

A platform primitive where correctness, low latency, predictable scaling, and operability matter more than product breadth.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **sharding**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Store and fetch low-latency key/value data.
- Support TTL, eviction, and sharding.
- Handle hot keys and cache stampedes.
- Expose cluster health and memory pressure controls.
...
```http
GET /v1/cache-keys/{id}
Authorization: Bearer <token>
```

```http
GET /v1/cache-keys?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```

```http
PATCH /v1/cache-keys/{id}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/cache-keys/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
GET /v1/cache-keys/{id}/metrics?from=2026-05-25T00:00:00Z&to=2026-05-25T01:00:00Z
```

```http
POST /v1/cache-keys/{id}/admin/quarantine
Idempotency-Key: <uuid>
...
service CacheKeyService {
  rpc CreateCacheKey(CreateCacheKeyRequest) returns (CacheKey);
  rpc GetCacheKey(GetCacheKeyRequest) returns (CacheKey);
  rpc UpdateCacheKey(UpdateCacheKeyRequest) returns (CacheKey);
  rpc EvaluateCacheKey(EvaluateCacheKeyRequest) returns (EvaluateCacheKeyResponse);
  rpc ListCacheKeyEvents(ListCacheKeyEventsRequest) returns (stream CacheKeyEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "cache.keys.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "cache-keys-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "cacheKey_123",
...
- `cache.keys.created.v1`
- `cache.keys.updated.v1`
- `cache.keys.state_changed.v1`
- `cache.keys.deleted_or_expired.v1`
- `cache.keys.policy_denied.v1`
- `cache.keys.operation_failed.v1`
...
```text
a distributed cache like Redis/Memcached Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the distributed cache like Redis/Memcached service boundary.
...
| Core domain services | Hot-path data-plane service, Control-plane API, Shard manager, Policy/config cache | Own a distributed cache like Redis/Memcached business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `CacheKeyController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `CacheKeyApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `CacheKeyDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `CacheKeyRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `CacheKeyPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `CacheKeyEventPublisher`: writes outbox records and publishes domain events to the broker.
- `CacheKeyReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface CacheKeyRepository {
    Optional<CacheKey> findById(CacheKeyId id, ReadConsistency consistency);
    Page<CacheKey> list(CacheKeyQuery query, Cursor cursor, int limit);
    CacheKey save(CacheKey aggregate, ExpectedVersion expectedVersion);
}

interface CacheKeyPolicyEvaluator {
    PolicyDecision canRead(Principal principal, CacheKey resource);
    PolicyDecision canMutate(Principal principal, CacheKey resource, Action action);
}
...
|---|---|
| `cache_namespaces` | `id, tenant_id, name, eviction_policy, max_memory_mb, replication_factor` |
| `shards` | `id, namespace_id, hash_range_start, hash_range_end, leader_node, state` |
| `replicas` | `id, shard_id, node_id, role, lag_ms, state` |
| `eviction_events` | `id, tenant_id, cachekey_ref, state, version, created_at, updated_at` |
...
| --- | --- | --- |
| Source of truth / primary store | in-memory sharded KV engine with optional RocksDB/AOF snapshots; Raft/etcd-style metadata store for cluster membership and slot ownership | cache data optimizes latency and may be disposable, while membership and slot assignment require coordination |
| Hot serving / cache | the cache cluster itself, with local client-side caching for stable keys | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
| Event stream / outbox | append-only replication log or gossip/stream replication for replicas and failover | decouples projections, notifications, analytics, search indexing, and recovery from the write path |
| Search / analytics | metrics TSDB/ClickHouse for cache observability, not for key lookup | serves text/filter/OLAP queries without overloading transactional tables |
| Large immutable payloads | object storage for snapshots/backups when persistence is enabled | large or immutable data is cheaper, durable, and easier to lifecycle outside OLTP rows |
...
- Primary partition key: `hash slot / key hash`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `cache key` should be single-partition whenever possible.
...
- Hot partition mitigation: detect hot keys, use client request coalescing, key splitting, local near-cache, and admission control.
...
- Required secondary indexes: `no secondary indexes on the hot path; maintain optional keyspace samples offline`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for ownership metadata and failover fencing | best-effort or tunable consistency for values; strong consistency only for cluster metadata/failover fencing | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for cached values because the authoritative store is elsewhere | Eventual consistency for replica values, metrics, keyspace scans, and eviction telemetry | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **sharding**: Shard by a stable high-cardinality key and maintain a resharding plan before the first hot shard.
- **replication**: Use replication for availability, read scale, and durability; state the consistency trade-off.
- **eviction**: Pick eviction policy by access pattern and protect critical keys from accidental eviction.
- **hot keys**: Use request coalescing, key splitting, local caches, and write buffering for hot objects.
- **cache stampede**: Use single-flight, soft TTL, jitter, and stale-while-revalidate.
...
- **Hot path clarity:** start from `sharding` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `cache.keys.created.v1, cache.keys.updated.v1, cache.keys.state_changed.v1, cache.keys.deleted_or_expired.v1, cache.keys.policy_denied.v1, cache.keys.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `sharding`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `sharding`.
