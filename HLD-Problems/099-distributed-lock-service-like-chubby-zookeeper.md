# Design distributed lock service like Chubby/ZooKeeper - System Design Deep Dive

**Problem #99**  
**Category:** Coordination  
**Primary pattern:** infra security  
**Deep-dive focus:** consensus, leases, fencing tokens, sessions
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **consensus**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```http
GET /v1/locks?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/locks/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
service DistributedLockService {
  rpc CreateDistributedLock(CreateDistributedLockRequest) returns (DistributedLock);
  rpc GetDistributedLock(GetDistributedLockRequest) returns (DistributedLock);
  rpc UpdateDistributedLock(UpdateDistributedLockRequest) returns (DistributedLock);
  rpc EvaluateDistributedLock(EvaluateDistributedLockRequest) returns (EvaluateDistributedLockResponse);
  rpc ListDistributedLockEvents(ListDistributedLockEventsRequest) returns (stream DistributedLockEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "locks.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "locks-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "distributedLock_123",
...
- `locks.created.v1`
- `locks.updated.v1`
- `locks.state_changed.v1`
- `locks.deleted_or_expired.v1`
- `locks.policy_denied.v1`
- `locks.operation_failed.v1`
...
```text
distributed lock service like Chubby/ZooKeeper Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the distributed lock service like Chubby/ZooKeeper service boundary.
...
| Core domain services | Propagation service, Policy engine, Versioned config store, Data-plane evaluator or agent, SRE console | Own distributed lock service like Chubby/ZooKeeper business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `DistributedLockController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `DistributedLockApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `DistributedLockDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `DistributedLockRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `DistributedLockPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `DistributedLockEventPublisher`: writes outbox records and publishes domain events to the broker.
- `DistributedLockReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface DistributedLockRepository {
    Optional<DistributedLock> findById(DistributedLockId id, ReadConsistency consistency);
    Page<DistributedLock> list(DistributedLockQuery query, Cursor cursor, int limit);
    DistributedLock save(DistributedLock aggregate, ExpectedVersion expectedVersion);
}

interface DistributedLockPolicyEvaluator {
    PolicyDecision canRead(Principal principal, DistributedLock resource);
    PolicyDecision canMutate(Principal principal, DistributedLock resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> DistributedLock
  -> emits -> DistributedLockEvent
...
|---|---|
| `lock_namespaces` | `id, tenant_id, name, quorum_group, policy_ref, state` |
| `lock_leases` | `id, namespace_id, lock_key, owner_session_id, fencing_token, expires_at, state` |
| `sessions` | `id, client_id, namespace_id, lease_until, last_heartbeat_at, state` |
| `fencing_tokens` | `namespace_id, lock_key, current_token, updated_at` |
| `consensus_log` | `index, term, command_type, command_ref, committed_at` |
...
- **consensus**: Use consensus for small critical metadata, not high-throughput data movement.
- **leases**: Expire leases safely and make workers prove ownership before committing side effects.
- **fencing tokens**: Issue monotonically increasing fencing tokens to prevent stale owners from committing writes.
- **sessions**: Expire sessions through leases and heartbeat failures.
...
- **Hot path clarity:** start from `consensus` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `locks.created.v1, locks.updated.v1, locks.state_changed.v1, locks.deleted_or_expired.v1, locks.policy_denied.v1, locks.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `consensus`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `consensus`.
