# Design secrets management platform - System Design Deep Dive

**Problem #100**  
**Category:** Security/platform  
**Primary pattern:** infra security  
**Deep-dive focus:** encryption, rotation, access policy, audit, break-glass
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **encryption**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```http
GET /v1/secrets?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/secrets/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
service SecretService {
  ...
  rpc EvaluateSecret(EvaluateSecretRequest) returns (EvaluateSecretResponse);
  rpc ListSecretEvents(ListSecretEventsRequest) returns (stream SecretEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "secrets.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "secrets-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "secret_123",
...
- `secrets.created.v1`
- `secrets.updated.v1`
- `secrets.state_changed.v1`
- `secrets.deleted_or_expired.v1`
- `secrets.policy_denied.v1`
- `secrets.operation_failed.v1`
...
```text
secrets management platform Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the secrets management platform service boundary.
...
| Core domain services | Propagation service, Policy engine, Versioned config store, Data-plane evaluator or agent, SRE console | Own secrets management platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `SecretController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `SecretApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `SecretDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `SecretRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `SecretPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `SecretEventPublisher`: writes outbox records and publishes domain events to the broker.
- `SecretReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface SecretRepository {
    Optional<Secret> findById(SecretId id, ReadConsistency consistency);
    Page<Secret> list(SecretQuery query, Cursor cursor, int limit);
    Secret save(Secret aggregate, ExpectedVersion expectedVersion);
}

interface SecretPolicyEvaluator {
    PolicyDecision canRead(Principal principal, Secret resource);
    PolicyDecision canMutate(Principal principal, Secret resource, Action action);
}
...
|---|---|
| `secrets` | `id, tenant_id, path, owner_team, current_version, kms_key_ref, state, created_at` |
| `secret_versions` | `secret_id, version, ciphertext_ref, checksum, created_by, created_at, expires_at` |
| `secret_access_policies` | `id, secret_id, principal_ref, actions, conditions_json, expires_at, state` |
| `rotation_jobs` | `id, secret_id, target_version, strategy, state, next_run_at, last_error` |
...
- **rotation**: Automate secret/key rotation and support dual-read/write during transition.
- **access policy**: Enforce least privilege with context, approvals, and just-in-time access.
- **audit**: Store immutable actor/action/resource/outcome logs and make them searchable.
- **break-glass**: Require strong audit, time limits, approvals, and post-use review for emergency access.
...
- **Hot path clarity:** start from `encryption` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `secrets.created.v1, secrets.updated.v1, secrets.state_changed.v1, secrets.deleted_or_expired.v1, secrets.policy_denied.v1, secrets.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `encryption`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `encryption`.
