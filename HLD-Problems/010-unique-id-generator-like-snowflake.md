# Design a unique ID generator like Snowflake - System Design Deep Dive

**Problem #10**  
**Category:** Platform  
**Primary pattern:** core platform  
**Deep-dive focus:** clock drift, sequence, region/worker IDs, monotonicity

## 0. Interview Framing

A platform primitive where correctness, low latency, predictable scaling, and operability matter more than product breadth.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **clock drift**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Generate unique sortable IDs at high throughput.
- Avoid collisions across regions and workers.
- Handle clock drift and worker restarts.
- Expose ID metadata for debugging.
...
```http
GET /v1/ids?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
GET /v1/ids/{id}/metrics?from=2026-05-25T00:00:00Z&to=2026-05-25T01:00:00Z
```

```http
POST /v1/ids/{id}/admin/quarantine
Idempotency-Key: <uuid>
...
```protobuf
service GeneratedIdService {
  rpc CreateGeneratedId(CreateGeneratedIdRequest) returns (GeneratedId);
  rpc GetGeneratedId(GetGeneratedIdRequest) returns (GeneratedId);
  rpc UpdateGeneratedId(UpdateGeneratedIdRequest) returns (GeneratedId);
  rpc EvaluateGeneratedId(EvaluateGeneratedIdRequest) returns (EvaluateGeneratedIdResponse);
  rpc ListGeneratedIdEvents(ListGeneratedIdEventsRequest) returns (stream GeneratedIdEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "ids.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "ids-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "generatedId_123",
...
- `ids.created.v1`
- `ids.updated.v1`
- `ids.state_changed.v1`
- `ids.deleted_or_expired.v1`
- `ids.policy_denied.v1`
- `ids.operation_failed.v1`
...
```text
a unique ID generator like Snowflake Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the unique ID generator like Snowflake service boundary.
...
| Core domain services | Hot-path data-plane service, Control-plane API, Shard manager, Policy/config cache | Own a unique ID generator like Snowflake business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `GeneratedIdController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `GeneratedIdApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `GeneratedIdDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `GeneratedIdRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `GeneratedIdPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `GeneratedIdEventPublisher`: writes outbox records and publishes domain events to the broker.
- `GeneratedIdReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface GeneratedIdRepository {
    Optional<GeneratedId> findById(GeneratedIdId id, ReadConsistency consistency);
    Page<GeneratedId> list(GeneratedIdQuery query, Cursor cursor, int limit);
    GeneratedId save(GeneratedId aggregate, ExpectedVersion expectedVersion);
}

interface GeneratedIdPolicyEvaluator {
    PolicyDecision canRead(Principal principal, GeneratedId resource);
    PolicyDecision canMutate(Principal principal, GeneratedId resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> GeneratedId
  -> emits -> GeneratedIdEvent
...
|---|---|
| `id_allocations` | `worker_id, region_id, sequence_start, sequence_end, lease_until, state` |
| `workers` | `worker_id, region_id, host, last_heartbeat_at, clock_status, state` |
| `clock_events` | `id, tenant_id, generatedid_ref, state, version, created_at, updated_at` |
| `sequence_blocks` | `id, tenant_id, generatedid_ref, state, version, created_at, updated_at` |
...
| --- | --- | --- |
| Source of truth / primary store | Raft/etcd/Consul or PostgreSQL for worker/range allocation, epoch fencing, and configuration; generated IDs are not stored per request | the database protects generator uniqueness and worker ownership, not every generated ID |
| Hot serving / cache | local in-process sequence blocks and clock state on each generator node | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
| Event stream / outbox | Kafka/ClickHouse for generator telemetry and duplicate/drift alerts | decouples projections, notifications, analytics, search indexing, and recovery from the write path |
| Search / analytics | metrics store for latency, clock skew, allocation failures, and duplicate detection | serves text/filter/OLAP queries without overloading transactional tables |
...
- Primary partition key: `worker_id / region_id / allocation_range`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `worker_id + epoch or range_id` should be single-partition whenever possible.
...
- Hot partition mitigation: preallocate ranges and isolate regions so ID generation does not depend on a central write per ID.
...
- Required secondary indexes: `region_id + state, expires_at, allocation_version`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for allocation metadata because duplicate IDs are unacceptable | strong consistency for worker/range leases and fencing tokens | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | local AP generation is allowed only while a valid fenced lease/range is held | Eventual consistency for metrics, utilization reports, audit exports, and duplicate scans | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
- **clock drift**: Detect clock rollback, fence workers, and reserve sequence bits for same-millisecond bursts.
- **sequence**: Keep sequence allocation monotonic per worker and define behavior on exhaustion.
- **region**: Allocate worker IDs centrally or via leases and recycle them only after safety windows.
- **monotonicity**: Be clear whether IDs are globally monotonic, roughly time-sortable, or only unique.
...
- **Hot path clarity:** start from `clock drift` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `ids.created.v1, ids.updated.v1, ids.state_changed.v1, ids.deleted_or_expired.v1, ids.policy_denied.v1, ids.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `clock drift`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `clock drift`.
