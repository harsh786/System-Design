# Design Payment Gateway Like Stripe - System Design Deep Dive

**Problem #71**  
**Category:** Fintech/payments  
**Primary pattern:** payment intent + routing + webhooks + reconciliation  
**Deep-dive focus:** idempotency, PCI, authorization/capture/refund, webhooks, reconciliation

## 0. Interview Framing

A payment gateway abstracts many processors while preventing duplicate charges, callback disorder, PCI leakage, and reconciliation gaps.

Actors: merchants, buyers, processors, card networks, risk/compliance teams. The highest-risk path is **payment intent -> authorize -> capture/refund -> webhook -> ledger reconciliation**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Create payment intents and payment method tokens.
- Authorize, capture, cancel, refund, dispute, and reconcile payments.
- Route by region, cost, success rate, currency, merchant risk, and provider health.
- Deliver signed merchant webhooks with retries, logs, and replay.
- Expose support/admin operations to inspect, replay, correct, and annotate payment_intent history.
...
- Provider callbacks may arrive before responses or multiple times.
- PCI-sensitive data stays in isolated tokenization boundary.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: PaymentIntent plus payment_attempt/capture/refund rows; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Merchants | 1M+ |
| Payment attempts | 100M/day, 10x seasonal |
| Webhook deliveries | 5-20 per payment |
| Provider callbacks | At least once, out of order |
...
- Keep the online payment_intent aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/payment_intents | amount, currency, merchant_id, capture_method | Create payment state. |
| POST /v1/payment_intents/{id}/confirm | payment_method_id, return_url | Authorize/confirm. |
| POST /v1/payment_intents/{id}/capture | amount_to_capture | Capture authorized funds. |
| POST /v1/refunds | payment_intent_id, amount, reason | Refund payment. |
| GET /v1/payment-intents/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/payment-intents/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |
...
service PaymentService {
  ...
  rpc Transition(TransitionPaymentRequest) returns (Payment);
  rpc Reconcile(ReconcilePaymentRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "payment.state_changed.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "payment-service",
  "tenant_or_region_id": "region_123",
  "aggregate_id": "payment_intent_123",
...
- payment.state_changed.v1
- payment.committed.v1
- payment.failed.v1
- payment.reversed.v1
- payment.reconciliation_completed.v1
- payment.manual_review_required.v1
...
```text
Payment Gateway Like Stripe Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Payment method vault -> Idempotency service -> Payment intent orchestrator -> State transition engine -> Risk service -> Provider router
        ...
        |       -> Webhook delivery / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager / Reconciliation worker
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Payment Gateway Like Stripe service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Payment method vault -> Idempotency service -> Payment intent orchestrator -> State transition engine -> Risk service -> Provider router; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
4. **Async path:** Webhook delivery / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Adapter manager / Reconciliation worker consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
- **Payment method vault**: tokenizes PAN/bank credentials and isolates PCI scope.
- **Provider router**: chooses processor and failover route.
- **Webhook delivery**: signs events, retries, and exposes replay logs.
- **Risk service**: card testing, fraud, merchant risk, dispute signals.
...
| Core domain services | Payment method vault, Idempotency service, Payment intent orchestrator, State transition engine, Risk service, Provider router | Own Payment Gateway Like Stripe business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Payment intent orchestrator**: owns payment_intent state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed payment_intent transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Payment Gateway Like Stripe: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| payment_intents | intent_id, merchant_id, amount, currency, state | Merchant-facing aggregate. |
| payment_attempts | attempt_id, intent_id, provider, state, provider_ref | Processor interaction. |
| captures | capture_id, intent_id, amount, state | Capture. |
| refunds | refund_id, intent_id, amount, state | Refund. |
| webhook_deliveries | delivery_id, event_id, endpoint_id, state | Merchant delivery. |
...
1. Create intent stores idempotent payment aggregate.
2. Confirm validates risk and calls provider authorize.
3. Capture/refund updates intent and posts ledger/audit entries.
4. Provider callback is deduped, mapped, and applied before webhook fanout.
...
- PaymentIntent is merchant source of truth; provider reports reconcile against it.
- Separate auth, capture, refund, dispute, and fee because settlement timing differs.
- Webhooks are at-least-once; merchants dedupe event IDs.
- Source-of-truth boundary: PaymentIntent plus payment_attempt/capture/refund rows owns correctness; all low-latency read models are disposable projections.
...
- Protect payment credentials, API keys, webhook secrets, and dispute evidence.
- Detect card testing, stolen cards, refund abuse, merchant laundering, and account takeover.
...
- SLIs: payment_intent mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How are APIs idempotent?
- How do webhooks work when merchant endpoint is down?
- How do you route processors?
- Where exactly is payment_intent source of truth stored?
...
- **Correctness boundary:** PaymentIntent plus payment_attempt/capture/refund rows is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `payment intent -> authorize -> capture/refund -> webhook -> ledger reconciliation` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Payment method vault, Provider router, Webhook delivery, Risk service, Idempotency service, Outbox/inbox workers; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `payment_intents, payment_attempts, captures, refunds, webhook_deliveries, idempotency_keys` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `payment.created.v1, payment.validated.v1, payment.state_changed.v1, payment.committed.v1, payment.failed.v1, payment.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `payment intent -> authorize -> capture/refund -> webhook -> ledger reconciliation`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `payment intent -> authorize -> capture/refund -> webhook -> ledger reconciliation`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Payment intent orchestrator, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
