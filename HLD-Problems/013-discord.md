# Design Discord - System Design Deep Dive

**Problem #13**  
**Category:** Realtime/community  
**Primary pattern:** realtime collab  
**Deep-dive focus:** servers, channels, voice, presence, sharding, moderation
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **servers**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Discord service boundary.
...
| Core domain services | Message or collaboration service, Media/attachment service, Moderation and abuse service, Session registry | Own Discord business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **servers**: Separate metadata master from chunk data and repair under-replicated chunks.
- **channels**: Model membership, roles, retention, and search scope per channel.
- **voice**: Route real-time media through regional SFUs and keep signaling separate from RTP/media paths.
- **presence**: Treat presence as ephemeral, best-effort, and TTL-backed; never make core correctness depend on it.
...
- **Hot path clarity:** start from `servers` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `servers`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `servers`.
