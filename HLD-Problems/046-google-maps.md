# Design Google Maps - System Design Deep Dive

**Problem #46**  
**Category:** Geo/navigation  
**Primary pattern:** search geo  
**Deep-dive focus:** tiles, routing, traffic, ETA, offline maps
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **tiles**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Google Maps service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own Google Maps business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **tiles**: Pre-render/cache map tiles and version tile data for rollback.
- **routing**: Keep route matching deterministic, versioned, and observable with shadow/canary evaluation.
- **traffic**: Ingest real-time traffic signals and apply freshness-aware routing.
- **eta**: Blend historical, real-time, and route features with confidence intervals.
- **offline maps**: Package offline regions with expiration and update deltas.
...
- **Hot path clarity:** start from `tiles` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `tiles`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `tiles`.
