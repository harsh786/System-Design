# Design multi-tenant SaaS platform - System Design Deep Dive

**Problem #92**  
**Category:** SaaS  
**Primary pattern:** infra security  
**Deep-dive focus:** tenant isolation, quotas, billing, data partitioning
...
```http
GET /v1/tenants?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/tenants/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
service TenantService {
  ...
  rpc EvaluateTenant(EvaluateTenantRequest) returns (EvaluateTenantResponse);
  rpc ListTenantEvents(ListTenantEventsRequest) returns (stream TenantEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "tenants.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "tenants-service",
...
- `tenants.created.v1`
- `tenants.updated.v1`
- `tenants.state_changed.v1`
- `tenants.deleted_or_expired.v1`
- `tenants.policy_denied.v1`
- `tenants.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the multi-tenant SaaS platform service boundary.
...
| Core domain services | Propagation service, Policy engine, Versioned config store, Data-plane evaluator or agent, SRE console | Own multi-tenant SaaS platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `TenantController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `TenantApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `TenantDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `TenantRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `TenantPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `TenantEventPublisher`: writes outbox records and publishes domain events to the broker.
- `TenantReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface TenantRepository {
    Optional<Tenant> findById(TenantId id, ReadConsistency consistency);
    Page<Tenant> list(TenantQuery query, Cursor cursor, int limit);
    Tenant save(Tenant aggregate, ExpectedVersion expectedVersion);
}

interface TenantPolicyEvaluator {
    PolicyDecision canRead(Principal principal, Tenant resource);
    PolicyDecision canMutate(Principal principal, Tenant resource, Action action);
}
...
| `tenant_users` | `tenant_id, user_id, role, status, invited_by, created_at` |
| `plans` | `id, name, limits_json, entitlements_json, price_version, state` |
| `quotas` | `tenant_id, quota_type, limit_value, current_usage, reset_at` |
| `tenant_data_partitions` | `tenant_id, shard_id, region, storage_mode, encryption_key_ref` |
...
- **billing**: Make billing explicit in the data model, APIs, scaling plan, and operational dashboards.
- **data partitioning**: Choose pooled, siloed, or hybrid tenancy based on isolation, cost, and scale.
...
- **Hot path clarity:** start from `tenant isolation` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `tenants.created.v1, tenants.updated.v1, tenants.state_changed.v1, tenants.deleted_or_expired.v1, tenants.policy_denied.v1, tenants.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `tenant isolation`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `tenant isolation`.
