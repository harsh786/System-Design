# Design Hotel Booking Platform - System Design Deep Dive

**Problem #68**  
**Category:** Booking/travel  
**Primary pattern:** inventory allocation + rate plans + PMS integration  
**Deep-dive focus:** room inventory, rate plans, holds, PMS/channel sync, cancellation, overbooking

## 0. Interview Framing

Hotel booking balances fast destination search with strict room-type/date inventory controls and unreliable PMS/channel integrations.

Actors: guests, hotels, OTAs, PMS/channel managers, payment providers. The highest-risk path is **room search -> rate quote -> inventory hold -> reservation confirmation -> PMS sync**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Guests search properties by destination, dates, guests, room type, amenities, loyalty, cancellation policy, and price.
- Hotels manage room types, allotments, rate plans, restrictions, blackout dates, taxes, fees, and overbooking limits.
- Support quotes, temporary holds, booking confirmation, guarantee/payment, cancellation, modification, no-show, and refund.
- Integrate PMS/channel managers for availability, rates, restrictions, confirmation numbers, and cancellation acknowledgements.
- Expose support/admin operations to inspect, replay, correct, and annotate reservation history.
...
- Do not oversell beyond configured overbooking policy.
- Rate, tax, and cancellation snapshots remain immutable.
- Partner/PMS outage is isolated and reconciled.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Inventory service room_type/date counters plus reservation service; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Properties | 1M properties |
| Room inventory | room_type x date rows, multi-year |
| Search | 300K QPS avg, high destination skew |
| Booking attempts | 3M/day, 20x holidays |
| PMS endpoints | Thousands with variable reliability |
...
- Inventory rows by room_type/date are source of truth; search index is derived.
- Long stays touch many rows; bound transaction size.
- Keep the online reservation aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| GET /v1/hotels/search | destination, dates, guests, filters | Property availability search. |
| POST /v1/hotel-quotes | property_id, room_type_id, rate_plan_id, dates, guests | Rate and policy quote. |
| POST /v1/hotel-reservations | quote_id, payment_method_id, guest_details | Create reservation. |
| PATCH /v1/hotel-reservations/{id} | expected_version, changed_dates/guests | Modify reservation. |
| POST /v1/hotel-reservations/{id}/cancel | reason | Cancel and refund/no-show policy. |
| GET /v1/hotel-reservations/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/hotel-reservations/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service HotelReservationService {
  rpc Create(CreateHotelReservationRequest) returns (HotelReservation);
  rpc Get(GetHotelReservationRequest) returns (HotelReservation);
  rpc Transition(TransitionHotelReservationRequest) returns (HotelReservation);
  rpc Reconcile(ReconcileHotelReservationRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "hotel_reservation.state_changed.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "hotel_reservation-service",
  "tenant_or_region_id": "region_123",
  "aggregate_id": "reservation_123",
...
- hotel_reservation.created.v1
- hotel_reservation.validated.v1
- hotel_reservation.state_changed.v1
- hotel_reservation.committed.v1
- hotel_reservation.failed.v1
- hotel_reservation.reversed.v1
- hotel_reservation.reconciliation_completed.v1
- hotel_reservation.manual_review_required.v1
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Inventory service -> Reservation service -> Idempotency service -> Inventory calendar -> State transition engine -> Rate service
        |       -> Source-of-Truth Write + Transactional Outbox
        |
        +--> Query / Serving Path
        |       -> Property content / Availability search
        |       -> Canonical Store fallback when strong freshness is required
        |
        +--> Async/Event Path
        |       -> Event Bus / Stream Processing / Workflow Queues
        |       -> PMS adapter / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Partner sync worker / Reconciliation worker
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Hotel Booking Platform service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Inventory service -> Reservation service -> Idempotency service -> Inventory calendar -> State transition engine -> Rate service; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
3. **Query path:** read-heavy traffic is served by Property content / Availability search; strong reads fall back to the canonical store when stale projections are unsafe.
4. **Async path:** PMS adapter / Outbox/inbox workers / Admin/support console / Analytics/warehouse pipeline / Partner sync worker / Reconciliation worker consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
- **Property content**: owns hotel profile, room types, amenities, photos, and policies.
- **Availability search**: serves denormalized availability/rate snapshots.
- **Inventory service**: owns room_type-date allotments, holds, commits, and overbooking.
- **Rate service**: computes rates, restrictions, taxes, loyalty/corporate discounts.
- **Reservation service**: owns reservation lifecycle, modification, cancellation, support.
- **PMS adapter**: normalizes partner protocols, retries, acknowledgements, and reconciliation.
...
| Core domain services | Inventory service, Reservation service, Idempotency service, Inventory calendar, State transition engine, Rate service | Own Hotel Booking Platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Property content, Availability search | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
| Async workers and integrations | PMS adapter, Outbox/inbox workers, Admin/support console, Analytics/warehouse pipeline, Partner sync worker, Reconciliation worker | Consume committed events, call external systems, retry safely, update projections, run cleanup, and isolate slow dependencies from user-facing latency. |
...
- **Inventory calendar**: tracks allotment, held, committed, and overbooking limit per room_type/date.
- **Hold manager**: creates range holds with TTL and commits only if all nights valid.
- **Partner sync worker**: pushes reservation changes and stores ack state.
- **State transition engine**: validates allowed reservation transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Atomic conditional update: committed + held < allotment + overbooking_limit.
- Represent ranges as nightly rows for correctness; compact cold history for analytics.
- Search uses denormalized min-price index; booking rechecks canonical counters.
...
- Reservation: QUOTED -> HELD -> CONFIRMING -> CONFIRMED -> CHECKED_IN -> CHECKED_OUT -> CLOSED.
- PMS sync: PENDING -> SENT -> ACKED or RETRYING -> MANUAL_REVIEW.
- Inventory hold: ACTIVE -> COMMITTED or EXPIRED or RELEASED.
...
| --- | --- | --- |
| properties | property_id, location, status, content_version | Hotel metadata. |
| room_types | room_type_id, property_id, occupancy, amenities | Sellable category. |
| rate_plans | rate_plan_id, property_id, cancellation_policy_id, restrictions | Rate config. |
| inventory_days | property_id, room_type_id, date, allotment, held, committed, overbook_limit, version | Canonical counters. |
| reservations | reservation_id, property_id, state, date_range, price_snapshot | Reservation aggregate. |
| pms_sync_attempts | attempt_id, reservation_id, provider, state, external_ref | Partner audit. |
...
- inventory_days by property/room_type/date, reservations by property_id + date bucket.
...
1. Search reads denormalized property/date availability and content.
2. Quote revalidates room inventory and rate restrictions.
3. Booking creates inventory hold, authorizes payment/guarantee, commits counters, and sends PMS sync.
4. Cancellation releases future inventory, computes refund/no-show fee, and pushes cancellation to PMS.
...
- Overbooking is explicit policy, not accidental oversell.
- PMS acknowledgement policy must be explicit: internal-confirmed vs partner-confirmed.
- Rate snapshots preserve rules used at booking time.
- Source-of-truth boundary: Inventory service room_type/date counters plus reservation service owns correctness; all low-latency read models are disposable projections.
...
- Destination/date search is skewed; precompute popular windows and shard by destination/property.
- Partner APIs are unreliable; isolate adapters and use queues.
...
- Protect guest PII, IDs, payment data, and invoices.
- Detect fake hotels, card testing, loyalty fraud, refund abuse, and stolen rates.
...
- If PMS sync fails after confirmation, mark partner_pending and manual review.
- If inventory commit fails after payment auth, void auth and release hold.
- Reconcile internal reservations with PMS/channel and payment reports.
...
- SLIs: reservation mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- Search and image CDN dominate browse; partner operations dominate support cost.
- Nightly inventory rows are storage-heavy but correctness-friendly.
...
- Async PMS sync improves checkout but can require manual recovery.
- Overbooking improves utilization but increases service risk.
...
- How do you prevent overselling a date range?
- How do modifications work?
- How should PMS failure affect confirmation?
- Where exactly is reservation source of truth stored?
...
- **Correctness boundary:** Inventory service room_type/date counters plus reservation service is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `room search -> rate quote -> inventory hold -> reservation confirmation -> PMS sync` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Property content, Availability search, Inventory service, Rate service, Reservation service, PMS adapter; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `properties, room_types, rate_plans, inventory_days, reservations, pms_sync_attempts` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `hotel_reservation.created.v1, hotel_reservation.validated.v1, hotel_reservation.state_changed.v1, hotel_reservation.committed.v1, hotel_reservation.failed.v1, hotel_reservation.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `room search -> rate quote -> inventory hold -> reservation confirmation -> PMS sync`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `room search -> rate quote -> inventory hold -> reservation confirmation -> PMS sync`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Inventory calendar, Hold manager, Partner sync worker, State transition engine, Reconciliation worker.
