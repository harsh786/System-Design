# Design CI/CD platform - System Design Deep Dive

**Problem #95**  
**Category:** DevOps  
**Primary pattern:** infra security  
**Deep-dive focus:** pipeline execution, isolation, artifacts, secrets, approvals
...
```http
GET /v1/pipelines?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/pipelines/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
service PipelineService {
  ...
  rpc EvaluatePipeline(EvaluatePipelineRequest) returns (EvaluatePipelineResponse);
  rpc ListPipelineEvents(ListPipelineEventsRequest) returns (stream PipelineEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "pipelines.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "pipelines-service",
...
- `pipelines.created.v1`
- `pipelines.updated.v1`
- `pipelines.state_changed.v1`
- `pipelines.deleted_or_expired.v1`
- `pipelines.policy_denied.v1`
- `pipelines.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the CI/CD platform service boundary.
...
| Core domain services | Propagation service, Policy engine, Versioned config store, Data-plane evaluator or agent, SRE console | Own CI/CD platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `PipelineController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `PipelineApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `PipelineDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `PipelineRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `PipelinePolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `PipelineEventPublisher`: writes outbox records and publishes domain events to the broker.
- `PipelineReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface PipelineRepository {
    Optional<Pipeline> findById(PipelineId id, ReadConsistency consistency);
    Page<Pipeline> list(PipelineQuery query, Cursor cursor, int limit);
    Pipeline save(Pipeline aggregate, ExpectedVersion expectedVersion);
}

interface PipelinePolicyEvaluator {
    PolicyDecision canRead(Principal principal, Pipeline resource);
    PolicyDecision canMutate(Principal principal, Pipeline resource, Action action);
}
...
|---|---|
| `pipelines` | `id, tenant_id, repo_ref, definition_ref, default_branch, state, created_at` |
| `pipeline_runs` | `id, pipeline_id, commit_sha, trigger, state, started_at, finished_at` |
| `jobs` | `id, run_id, stage, worker_pool, state, attempt, started_at, finished_at` |
| `artifacts` | `id, run_id, name, object_ref, checksum, signature_ref, created_at` |
| `secrets` | `id, tenant_id, name, vault_ref, scope, rotated_at, state` |
| `approvals` | `id, run_id, approver_id, decision, reason, decided_at` |
...
- **pipeline execution**: Run pipelines as isolated jobs with explicit artifact and secret boundaries.
- **isolation**: Isolate tenants/jobs by quotas, sandboxing, resource pools, and network policy.
- **artifacts**: Store immutable signed artifacts with provenance and retention.
- **secrets**: Inject secrets just-in-time, never persist in logs, and rotate on leak suspicion.
...
- **Hot path clarity:** start from `pipeline execution` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `pipelines.created.v1, pipelines.updated.v1, pipelines.state_changed.v1, pipelines.deleted_or_expired.v1, pipelines.policy_denied.v1, pipelines.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `pipeline execution`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `pipeline execution`.
