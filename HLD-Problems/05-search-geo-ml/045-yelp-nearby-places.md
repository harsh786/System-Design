# Design Yelp / nearby places - System Design Deep Dive

**Problem #45**  
**Category:** Geo/search  
**Primary pattern:** search geo  
**Deep-dive focus:** geohash, quadtree, filtering, ranking, reviews
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **geohash**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Yelp / nearby places service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own Yelp / nearby places business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **geohash**: Use geohash/S2/H3 cells for coarse filtering, then exact distance/ranking.
- **quadtree**: Use spatial indexes where density varies heavily by geography.
- **filtering**: Push cheap filters before expensive rankers and preserve pagination correctness.
- **ranking**: Separate candidate generation from ranking and keep a deterministic fallback ranker.
- **reviews**: Aggregate review stats asynchronously and protect against spam.
...
- **Hot path clarity:** start from `geohash` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `geohash`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `geohash`.
