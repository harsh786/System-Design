# Design image search - System Design Deep Dive

**Problem #49**  
**Category:** Search/media  
**Primary pattern:** search geo  
**Deep-dive focus:** embeddings, metadata, crawling, ranking, safety
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **embeddings**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the image search service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own image search business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **embeddings**: Version embeddings and vector indexes; support reindexing/backfill.
- **metadata**: Keep canonical metadata strongly consistent enough for rights, deletion, and discovery.
...
- **safety**: Apply policy filters before final serving and log decisions for audit.
...
- **Hot path clarity:** start from `embeddings` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `embeddings`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `embeddings`.
