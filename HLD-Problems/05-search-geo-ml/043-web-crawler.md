# Design a web crawler - System Design Deep Dive

**Problem #43**  
**Category:** Search/data  
**Primary pattern:** search geo  
**Deep-dive focus:** frontier, politeness, dedupe, scheduling, retries
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **frontier**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the web crawler service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own a web crawler business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **frontier**: Make frontier explicit in the data model, APIs, scaling plan, and operational dashboards.
- **politeness**: Throttle crawler requests per host and obey robots directives.
- **dedupe**: Deduplicate alerts by fingerprint and suppress during maintenance.
- **scheduling**: Use priority queues and leases for recurring work and retries.
...
- **Hot path clarity:** start from `frontier` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `frontier`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `frontier`.
