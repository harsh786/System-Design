# Design financial reporting pipeline - System Design Deep Dive

**Problem #80**  
**Category:** Finance/data  
**Primary pattern:** fintech risk  
**Deep-dive focus:** batch close, ledger snapshots, controls, lineage
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **batch close**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
financial reporting pipeline Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the financial reporting pipeline service boundary.
...
| Core domain services | Public API and auth gateway, Ledger service, Command/orchestration service, Risk/fraud service | Own financial reporting pipeline business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **batch close**: Use controlled batch windows, approvals, checkpoints, and rerunnable jobs.
- **ledger snapshots**: Snapshot ledger projections for period close and keep source entries immutable.
- **controls**: Add segregation of duties, approvals, reconciliation, and evidence retention.
...
- **Hot path clarity:** start from `batch close` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `batch close`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `batch close`.
