# Design payout system for creators/drivers - System Design Deep Dive

**Problem #78**  
**Category:** Marketplace fintech  
**Primary pattern:** fintech risk  
**Deep-dive focus:** balances, KYC, payout rails, reconciliation
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **balances**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
payout system for creators/drivers Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the payout system for creators/drivers service boundary.
...
| Core domain services | Public API and auth gateway, Ledger service, Command/orchestration service, Risk/fraud service | Own payout system for creators/drivers business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **balances**: Compute balances from ledger projections and reconcile to payouts.
- **kyc**: Gate payout eligibility on KYC state and provider verification.
- **payout rails**: Abstract ACH, RTP, card, bank, and wallet rails behind idempotent adapters.
- **reconciliation**: Reconcile internal records to provider/rail reports and investigate breaks.
...
- **Hot path clarity:** start from `balances` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `balances`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `balances`.
