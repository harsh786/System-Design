# Design Digital Wallet - System Design Deep Dive

**Problem #72**  
**Category:** Fintech/wallet  
**Primary pattern:** wallet account + double-entry ledger + limits  
**Deep-dive focus:** balances, top-up, transfer, withdrawal, KYC, fraud

## 0. Interview Framing

A wallet must separate balance display from immutable ledger entries and enforce KYC/AML limits without creating or losing money.

Actors: users, merchants, banks, payment rails, compliance team. The highest-risk path is **wallet transaction -> risk/limit check -> ledger entries -> balance update -> settlement**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Open wallet accounts after KYC and bank/payment verification.
- Top up, pay merchant, peer transfer, withdraw, refund, and view statements.
- Maintain available, pending, and settled balances with holds and reversals.
- Apply per-user, per-instrument, per-merchant, and regulatory limits.
- Expose support/admin operations to inspect, replay, correct, and annotate wallet_transaction history.
...
- Ledger postings are double-entry and sum to zero per currency.
- Available balance never goes negative unless product explicitly supports credit.
- KYC/AML/risk decisions are auditable.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Double-entry ledger entries plus wallet account state; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Wallets | 50M |
| Transactions | 50M/day, 10x festival peak |
| Balance reads | 100x writes |
| Ledger entries | 2-6 per transaction |
| Retention | Ledger/compliance multi-year |
...
- Keep the online wallet_transaction aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/wallets/{id}/topups | amount, payment_method_id | Add funds. |
| POST /v1/wallet-transfers | from_wallet, to_wallet, amount | Peer/merchant transfer. |
| POST /v1/wallets/{id}/withdrawals | amount, bank_account_id | Bank withdrawal. |
| GET /v1/wallets/{id}/statement | date_range, cursor | Statement. |
| GET /v1/wallet-transactions/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/wallet-transactions/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service WalletService {
  rpc Create(CreateWalletRequest) returns (Wallet);
  rpc Get(GetWalletRequest) returns (Wallet);
  rpc Transition(TransitionWalletRequest) returns (Wallet);
  rpc Reconcile(ReconcileWalletRequest) returns (ReconcileResult);
}
...
- wallet.created.v1
- wallet.validated.v1
- wallet.state_changed.v1
- wallet.committed.v1
- wallet.failed.v1
- wallet.reversed.v1
- wallet.reconciliation_completed.v1
- wallet.manual_review_required.v1
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Wallet service -> Ledger service -> Idempotency service -> Wallet transaction orchestrator -> State transition engine -> Balance projection
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Digital Wallet service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Wallet service -> Ledger service -> Idempotency service -> Wallet transaction orchestrator -> State transition engine -> Balance projection; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
- **Wallet service**: wallet account, limits, holds, transaction state.
- **Ledger service**: immutable balanced postings and reversals.
- **Balance projection**: available/pending/settled balances.
- **Settlement adapter**: bank/card/UPI/ACH movement and reconciliation.
...
| Core domain services | Wallet service, Ledger service, Idempotency service, Wallet transaction orchestrator, State transition engine, Balance projection | Own Digital Wallet business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Wallet transaction orchestrator**: owns wallet_transaction state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed wallet_transaction transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Digital Wallet: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| wallet_accounts | wallet_id, user_id, kyc_tier, status | Wallet. |
| ledger_transactions | txn_id, type, state, idempotency_key | Header. |
| ledger_entries | entry_id, txn_id, account_id, debit_credit, amount | Immutable postings. |
| balance_snapshots | account_id, available, pending, as_of_entry_id | Projection. |
| holds | hold_id, wallet_id, amount, reason, expires_at | Pending debit reservation. |
...
1. Top-up posts pending credit, settles external payment, then makes balance available.
2. Transfer checks limits/balance and atomically posts debit/credit entries.
3. Withdrawal places hold, calls bank rail, and settles or reverses.
4. Event publication: the aggregate write commits with an outbox event; publisher emits to the bus; consumers dedupe and update projections.
5. Reconciliation: scheduled worker compares source of truth with provider reports/read models/ledger and creates correction tasks for mismatches.
6. Operator correction: support action requires authorization, reason, expected version, immutable audit log, and compensating event.

## 9. Deep-Dive Focus Areas

- Ledger is immutable; corrections are reversals.
- Balance projections are rebuildable from ledger.
- Holds prevent double spend during async settlement.
- Source-of-truth boundary: Double-entry ledger entries plus wallet account state owns correctness; all low-latency read models are disposable projections.
...
- Protect KYC, bank accounts, payment methods, statements, and device identifiers.
- Detect mule accounts, synthetic identity, stolen instruments, velocity abuse, and refund laundering.
...
- SLIs: wallet_transaction mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How is balance different from ledger?
- How do reversals work?
- Where exactly is wallet_transaction source of truth stored?
...
- **Correctness boundary:** Double-entry ledger entries plus wallet account state is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `wallet transaction -> risk/limit check -> ledger entries -> balance update -> settlement` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Wallet service, Ledger service, Balance projection, Settlement adapter, Idempotency service, Outbox/inbox workers; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `wallet_accounts, ledger_transactions, ledger_entries, balance_snapshots, holds, idempotency_keys` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `wallet.created.v1, wallet.validated.v1, wallet.state_changed.v1, wallet.committed.v1, wallet.failed.v1, wallet.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `wallet transaction -> risk/limit check -> ledger entries -> balance update -> settlement`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `wallet transaction -> risk/limit check -> ledger entries -> balance update -> settlement`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Wallet transaction orchestrator, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
