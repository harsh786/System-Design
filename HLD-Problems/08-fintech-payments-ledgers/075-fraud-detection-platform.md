# Design Fraud Detection Platform - System Design Deep Dive

**Problem #75**  
**Category:** Risk/data/ML  
**Primary pattern:** stream scoring + feature store + rules + cases  
**Deep-dive focus:** real-time scoring, feature freshness, rules, models, feedback

## 0. Interview Framing

Fraud detection is low-latency decisioning with fresh features, rules, models, explainability, and investigator feedback.

Actors: product services, analysts, investigators, ML platform. The highest-risk path is **event ingestion -> feature lookup -> score -> decision -> feedback loop**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Ingest login, payment, order, refund, device, profile, and behavior events.
- Compute real-time and batch features for users, devices, cards, merchants, IPs, and graphs.
- Score with rules, models, allow/deny lists, and manual review queues.
- Return approve, deny, challenge, step-up, hold, or review with reason codes.
- Expose support/admin operations to inspect, replay, correct, and annotate risk_decision history.
...
- Inline scoring p99 often below 50-150 ms.
- Feature values include freshness and safe defaults.
- Decisions are reproducible with model/rule/feature versions.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: fraud_decisions row with feature/model/rule versions; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Events | 1B+/day |
| Inline scoring | 50K-500K QPS |
| Features | Millions of keys |
| Models/rules | Hundreds active |
| Retention | Raw events and labels for training |
...
- Keep the online risk_decision aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/fraud/score | event_type, entity_ids, context | Inline decision. |
| POST /v1/fraud/rules | rule_expr, action, rollout | Create rule. |
| POST /v1/fraud/cases/{id}/labels | label, analyst_notes | Feedback. |
| GET /v1/fraud/decisions/{id} | decision_id | Explain decision. |
| GET /v1/fraud-decisions/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/fraud-decisions/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service FraudDecisionService {
  rpc Create(CreateFraudDecisionRequest) returns (FraudDecision);
  rpc Get(GetFraudDecisionRequest) returns (FraudDecision);
  rpc Transition(TransitionFraudDecisionRequest) returns (FraudDecision);
  rpc Reconcile(ReconcileFraudDecisionRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "fraud_decision.state_changed.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "fraud_decision-service",
  "tenant_or_region_id": "region_123",
  "aggregate_id": "risk_decision_123",
...
- fraud_decision.created.v1
- fraud_decision.validated.v1
- fraud_decision.state_changed.v1
- fraud_decision.committed.v1
- fraud_decision.failed.v1
- fraud_decision.reversed.v1
- fraud_decision.reconciliation_completed.v1
- fraud_decision.manual_review_required.v1
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Idempotency service -> State transition engine -> Decision orchestration service -> Feature store -> Rule engine -> Model serving
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Fraud Detection Platform service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Idempotency service -> State transition engine -> Decision orchestration service -> Feature store -> Rule engine -> Model serving; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
3. **Query path:** read-heavy traffic is served by Model serving; strong reads fall back to the canonical store when stale projections are unsafe.
...
- **Feature store**: online and offline features with freshness.
- **Rule engine**: versioned deterministic policies.
- **Model serving**: canary, shadow, rollback, feature validation.
- **Case management**: analyst queues, labels, evidence.
...
| Core domain services | Idempotency service, State transition engine, Decision orchestration service, Feature store, Rule engine, Model serving | Own Fraud Detection Platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Model serving | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
...
- **Decision orchestration service**: owns risk_decision state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed risk_decision transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Fraud Detection Platform: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| raw_events | event_id, entity_ids, event_type, occurred_at | Input. |
| online_features | feature_key, entity_id, value, freshness_ts | Feature store. |
| rules | rule_id, version, expression, action | Rules. |
| model_versions | model_id, version, artifact_ref, status | Model registry. |
| fraud_decisions | decision_id, event_id, action, score, reasons | Decision audit. |
...
| --- | --- | --- |
| Source of truth / primary store | Kafka/Pulsar stream plus online feature store (Redis/Cassandra/DynamoDB) for scoring; PostgreSQL for cases/rules; ClickHouse/Snowflake for offline training and investigation | fraud requires low-latency online features, durable case management, and high-volume analytical history |
...
1. Product calls score API with event context.
2. Scoring fetches features, evaluates rules/models, combines policy, and returns action/reasons.
3. All feature/rule/model versions are stored for replay.
4. Investigator labels feed training and rule evaluation.
...
- Training-serving skew is a core risk; share feature definitions.
- Reason codes are mandatory for support and governance.
- Graph features are powerful but expensive; precompute asynchronously.
- Source-of-truth boundary: fraud_decisions row with feature/model/rule versions owns correctness; all low-latency read models are disposable projections.
...
- Protect risk rules, models, user/device/payment data, and case evidence.
- Detect adversarial probing, botnets, synthetic identity, collusion, and insider rule misuse.
...
- SLIs: risk_decision mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do you keep features fresh?
- How do you replay a fraud decision?
- How do rules and models interact?
- Where exactly is risk_decision source of truth stored?
...
- **Correctness boundary:** fraud_decisions row with feature/model/rule versions is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `event ingestion -> feature lookup -> score -> decision -> feedback loop` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Feature store, Rule engine, Model serving, Case management, Idempotency service, Outbox/inbox workers; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `raw_events, online_features, rules, model_versions, fraud_decisions, idempotency_keys` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `fraud_decision.created.v1, fraud_decision.validated.v1, fraud_decision.state_changed.v1, fraud_decision.committed.v1, fraud_decision.failed.v1, fraud_decision.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `event ingestion -> feature lookup -> score -> decision -> feedback loop`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `event ingestion -> feature lookup -> score -> decision -> feedback loop`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Decision orchestration service, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
