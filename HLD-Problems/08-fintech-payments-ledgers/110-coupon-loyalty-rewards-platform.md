# Design Coupon, Loyalty, And Rewards Platform - System Design Deep Dive

**Problem #110**  
**Category:** Commerce/fintech  
**Primary pattern:** eligibility engine + coupon inventory + points ledger  
**Deep-dive focus:** coupon issuance/redemption, reward ledger, loyalty tiers, abuse

## 0. Interview Framing

Rewards systems sit in checkout and affect revenue, so they must be fast, deterministic, and abuse-resistant.

Actors: customers, merchants, marketers, checkout, finance. The highest-risk path is **cart -> eligibility -> reserve reward -> commit redemption -> settlement**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Create campaigns with rules, inventory, budget, validity, stacking, funding, and targeting.
- Issue coupons, validate at cart/order, reserve redemption, commit after payment, reverse on cancellation/refund.
- Maintain points accounts, earn/burn rules, tiers, expirations, statements, and partner rewards.
- Provide experimentation, reporting, fraud controls, and settlement.
- Expose support/admin operations to inspect, replay, correct, and annotate reward_transaction history.
...
- Checkout validation is low-latency and deterministic for rule version.
- Coupon inventory/budget does not materially overspend.
- Points ledger is immutable and auditable.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Reward ledger plus redemption reservations; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Customers | 100M |
| Campaigns | 100K active |
| Eligibility checks | 500K QPS peak |
| Redemptions | 50M/day |
| Ledger | Earn/burn/expire/reverse multi-year |
...
- Keep the online reward_transaction aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/rewards/eligible | cart, customer_id, merchant_id | Evaluate offers. |
| POST /v1/coupon-redemptions | coupon_code, order_id, amount | Reserve/commit coupon. |
| POST /v1/reward-accounts/{id}/burn | points, order_id | Burn points. |
| GET /v1/reward-accounts/{id}/statement | date_range | Statement. |
| GET /v1/reward-transactions/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/reward-transactions/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service RewardsService {
  rpc Create(CreateRewardsRequest) returns (Rewards);
  rpc Get(GetRewardsRequest) returns (Rewards);
  rpc Transition(TransitionRewardsRequest) returns (Rewards);
  rpc Reconcile(ReconcileRewardsRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "rewards.state_changed.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "rewards-service",
  "tenant_or_region_id": "region_123",
  "aggregate_id": "reward_transaction_123",
...
- rewards.created.v1
- rewards.validated.v1
- rewards.state_changed.v1
- rewards.committed.v1
- rewards.failed.v1
- rewards.reversed.v1
- rewards.reconciliation_completed.v1
- rewards.manual_review_required.v1
...
```text
Coupon, Loyalty, And Rewards Platform Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Rewards ledger -> Idempotency service -> State transition engine -> Campaign service -> Redemption service -> Reward transaction service
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Coupon, Loyalty, And Rewards Platform service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Rewards ledger -> Idempotency service -> State transition engine -> Campaign service -> Redemption service -> Reward transaction service; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
- **Campaign service**: versioned rules, budgets, inventory, targeting.
- **Eligibility engine**: deterministic rule evaluation.
- **Redemption service**: reserve/commit/reverse coupon or points.
- **Rewards ledger**: earn, burn, expire, reverse, adjust.
...
| Core domain services | Rewards ledger, Idempotency service, State transition engine, Campaign service, Redemption service, Reward transaction service | Own Coupon, Loyalty, And Rewards Platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Reward transaction service**: owns reward_transaction state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed reward_transaction transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Coupon, Loyalty, And Rewards Platform: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| campaigns | campaign_id, rules_ref, budget, inventory, status | Campaign. |
| coupons | coupon_id, campaign_id, code_hash, state | Coupon. |
| redemptions | redemption_id, coupon_id, order_id, state | Redemption. |
| reward_accounts | account_id, user_id, tier, status | Account. |
| reward_ledger | entry_id, account_id, type, points, expiry_at | Ledger. |
| budget_counters | campaign_id, spent, reserved, version | Budget. |
...
1. Eligibility fetches campaigns and evaluates rules against cart/customer context.
2. Checkout reserves coupon/budget/points with TTL.
3. Payment success commits redemption; cancellation reverses or releases.
4. Tier job evaluates rolling spend and updates benefits.
...
- Rule DSL is versioned for reproducible decisions.
- Points balance is projection from ledger.
- Limited coupons use reservation + commit.
- Source-of-truth boundary: Reward ledger plus redemption reservations owns correctness; all low-latency read models are disposable projections.
...
- Protect coupon codes, customer segments, reward balances, and order history.
- Detect code leaks, coupon farming, referral abuse, fake orders, merchant collusion.
...
- SLIs: reward_transaction mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do you prevent coupon oversubscription?
- How do points expirations work?
- How do you debug rejected coupon?
- Where exactly is reward_transaction source of truth stored?
...
- **Correctness boundary:** Reward ledger plus redemption reservations is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `cart -> eligibility -> reserve reward -> commit redemption -> settlement` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Campaign service, Eligibility engine, Redemption service, Rewards ledger, Idempotency service, Outbox/inbox workers; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `campaigns, coupons, redemptions, reward_accounts, reward_ledger, budget_counters` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `rewards.created.v1, rewards.validated.v1, rewards.state_changed.v1, rewards.committed.v1, rewards.failed.v1, rewards.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `cart -> eligibility -> reserve reward -> commit redemption -> settlement`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `cart -> eligibility -> reserve reward -> commit redemption -> settlement`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Reward transaction service, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
