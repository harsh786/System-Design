# Design inventory reservation system - System Design Deep Dive

**Problem #64**  
**Category:** Commerce  
**Primary pattern:** commerce booking  
**Deep-dive focus:** oversell prevention, locks, consistency, reconciliation
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the inventory reservation system service boundary.
...
| Core domain services | Cart or quote service, Inventory availability service, Reservation/hold service, Order/booking orchestrator, Catalog/search service | Own inventory reservation system business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **oversell prevention**: Use strongly consistent counters/locks for scarce inventory and async reconciliation for soft stock.
- **locks**: Use short leases/fencing tokens; avoid unbounded distributed locks in user flows.
...
- **Hot path clarity:** start from `oversell prevention` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `oversell prevention`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `oversell prevention`.
