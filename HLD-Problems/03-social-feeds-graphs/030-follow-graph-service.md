# Design a follow graph service - System Design Deep Dive

**Problem #30**  
**Category:** Social graph  
**Primary pattern:** social feed  
**Deep-dive focus:** adjacency storage, privacy, fanout, graph queries
...
```text
a follow graph service Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the follow graph service boundary.
...
| Core domain services | Graph service, Feed ranking service, Media service, Content write API, Feed cache | Own a follow graph service business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **adjacency storage**: Optimize separate read paths for outgoing edges, incoming edges, mutuals, and privacy checks.
...
- **graph queries**: Bound graph traversals and precompute common relationship degrees.
...
- **Hot path clarity:** start from `adjacency storage` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `adjacency storage`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `adjacency storage`.
