# Design LinkedIn - System Design Deep Dive

**Problem #26**  
**Category:** Professional graph  
**Primary pattern:** social feed  
**Deep-dive focus:** connection degrees, jobs feed, search, privacy
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the LinkedIn service boundary.
...
| Core domain services | Graph service, Feed ranking service, Media service, Content write API, Feed cache | Own LinkedIn business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **connection degrees**: Precompute first/second-degree relationships and apply privacy filters before ranking.
- **jobs feed**: Separate job search/recommendation from social feed freshness.
...
- **Hot path clarity:** start from `connection degrees` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `connection degrees`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `connection degrees`.
