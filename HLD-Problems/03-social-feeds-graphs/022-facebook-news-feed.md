# Design Facebook News Feed - System Design Deep Dive

**Problem #22**  
**Category:** Social/feed  
**Primary pattern:** social feed  
**Deep-dive focus:** edge ranking, push vs pull, ML ranking, feed cache
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Facebook News Feed service boundary.
...
| Core domain services | Graph service, Feed ranking service, Media service, Content write API, Feed cache | Own Facebook News Feed business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **edge ranking**: Use cached candidates plus real-time signals for ranking without blocking core reads.
- **push vs pull**: Use push for normal users, pull for high-fanout users, and merge at read time.
- **ml ranking**: Version features/models and log impressions for offline evaluation.
- **feed cache**: Cache materialized feed slices and invalidate through append-only feed events.
...
- **Hot path clarity:** start from `edge ranking` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `edge ranking`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `edge ranking`.
