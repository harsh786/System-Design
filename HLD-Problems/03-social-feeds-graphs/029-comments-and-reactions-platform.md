# Design a comments and reactions platform - System Design Deep Dive

**Problem #29**  
**Category:** Social infra  
**Primary pattern:** social feed  
**Deep-dive focus:** nested comments, counters, moderation, hot objects
...
```text
a comments and reactions platform Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the comments and reactions platform service boundary.
...
| Core domain services | Graph service, Feed ranking service, Media service, Content write API, Feed cache | Own a comments and reactions platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **nested comments**: Use materialized path or parent_id plus depth limits; cache hot threads.
- **counters**: Use sharded counters and async reconciliation for high-write reactions.
- **moderation**: Use pre-moderation for high-risk paths and post-moderation plus reports for normal traffic.
- **hot objects**: Make hot objects explicit in the data model, APIs, scaling plan, and operational dashboards.
...
- **Hot path clarity:** start from `nested comments` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `nested comments`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `nested comments`.
