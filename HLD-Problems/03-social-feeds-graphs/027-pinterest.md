# Design Pinterest - System Design Deep Dive

**Problem #27**  
**Category:** Discovery/media  
**Primary pattern:** social feed  
**Deep-dive focus:** image boards, visual search, crawling, recommendations
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Pinterest service boundary.
...
| Core domain services | Graph service, Feed ranking service, Media service, Content write API, Feed cache | Own Pinterest business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **image boards**: Make image boards explicit in the data model, APIs, scaling plan, and operational dashboards.
- **visual search**: Use embeddings plus metadata filters; store vector index versions.
- **crawling**: Maintain frontier priority, robots compliance, dedupe, and retry/backoff state.
- **recommendations**: Separate candidate generation, ranking, exploration, and feedback logging.
...
- **Hot path clarity:** start from `image boards` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `image boards`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `image boards`.
