# Design Twitter/X timeline - System Design Deep Dive

**Problem #21**  
**Category:** Social/feed  
**Primary pattern:** social feed  
**Deep-dive focus:** followers graph, fanout, celebrity problem, ranking
...
```text
Twitter/X timeline Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Twitter/X timeline service boundary.
...
| Core domain services | Graph service, Feed ranking service, Media service, Content write API, Feed cache | Own Twitter/X timeline business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **followers graph**: Store forward and reverse edges for different read paths; protect private edges.
- **fanout**: Choose fanout-on-write, fanout-on-read, or hybrid based on follower count and freshness needs.
- **celebrity problem**: Use hybrid fanout so celebrity writes do not enqueue to every follower synchronously.
...
- **Hot path clarity:** start from `followers graph` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `followers graph`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `followers graph`.
