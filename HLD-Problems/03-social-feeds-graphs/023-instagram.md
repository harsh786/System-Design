# Design Instagram - System Design Deep Dive

**Problem #23**  
**Category:** Social/media  
**Primary pattern:** social feed  
**Deep-dive focus:** photo upload, feed, stories, CDN, moderation
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Instagram service boundary.
...
| Core domain services | Graph service, Feed ranking service, Media service, Content write API, Feed cache | Own Instagram business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **photo upload**: Use direct-to-object-store multipart upload and asynchronous processing.
- **feed**: Cache materialized feed slices and invalidate through append-only feed events.
- **stories**: Implement TTL, viewer receipts, ranking, and privacy checks as separate concerns.
- **cdn**: Use CDN for immutable media and signed URLs for private content.
- **moderation**: Use pre-moderation for high-risk paths and post-moderation plus reports for normal traffic.
...
- **Hot path clarity:** start from `photo upload` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `photo upload`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `photo upload`.
