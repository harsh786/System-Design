# Design TikTok - System Design Deep Dive

**Problem #24**  
**Category:** Social/video  
**Primary pattern:** social feed  
**Deep-dive focus:** video feed, recommendation, watch events, creator scale
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **video feed**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Moderation pipeline / Fanout service / Notification service / Analytics/feature pipeline
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the TikTok service boundary.
...
4. **Async path:** Moderation pipeline / Fanout service / Notification service / Analytics/feature pipeline consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
| Core domain services | Graph service, Feed ranking service, Media service, Content write API, Feed cache | Own TikTok business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
| Async workers and integrations | Moderation pipeline, Fanout service, Notification service, Analytics/feature pipeline | Consume committed events, call external systems, retry safely, update projections, run cleanup, and isolate slow dependencies from user-facing latency. |
...
| --- | --- | --- |
| Source of truth / primary store | Cassandra/ScyllaDB/DynamoDB/Bigtable for posts, timelines, reactions, and feed items; PostgreSQL/MySQL for user/profile/account metadata | feed and social writes are high-volume and partitionable, while account/profile state benefits from relational constraints |
| Hot serving / cache | Redis Cluster for home feed cache, counters, rate limits, sessions, and hot object metadata | keeps hot reads, sessions, counters, quotas, and derived views away from the OLTP source of truth |
...
- Primary partition key: `user_id/author_id/object_id with virtual shards for celebrities; time bucket for feed and event tables`. Choose the key that matches the hottest write/read path, not just the entity name.
- Primary lookup path: `post_id/object_id and user_id + feed_type + score/time` should be single-partition whenever possible.
...
- Hot partition mitigation: hybrid fanout: push for normal users, pull for celebrities, split counters, and cache viral objects.
...
- Required secondary indexes: `author_id + created_at, target_id + reaction_type, follower_id/followee_id, visibility + moderation_state`.
...
| --- | --- | --- | --- |
| Canonical command path | CP for privacy and moderation boundaries | strong consistency for privacy, follow/unfollow, moderation decisions, and post ownership; per-object atomicity for reactions when required | Prefer rejecting or queuing unsafe writes over accepting divergent state. |
| Derived read models | AP/eventual for feeds/counters because freshness can be bounded and repaired | Eventual consistency for feeds, counters, ranking features, search, notifications, recommendations, and analytics | Expose `pending`, `processing`, `stale_at`, or version metadata when users may observe lag. |
...
For this design, use Redis/CDN for hot feeds and counters, Kafka for fanout/ranking/moderation events, Flink for trend/counter windows, CQRS read models for timelines, S3 + Iceberg for event history, and Pinot/ClickHouse/Druid for engagement analytics.
...
- **video feed**: Make video feed explicit in the data model, APIs, scaling plan, and operational dashboards.
- **recommendation**: Separate candidate generation, ranking, exploration, and feedback logging.
- **watch events**: Log watch/listen events asynchronously; sample only after critical analytics are preserved.
- **creator scale**: Protect creator hot spots with precomputed feeds, CDN, and isolated processing quotas.
...
- **Hot path clarity:** start from `video feed` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `video feed`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `video feed`.
