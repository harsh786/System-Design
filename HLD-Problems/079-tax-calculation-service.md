# Design tax calculation service - System Design Deep Dive

**Problem #79**  
**Category:** Commerce fintech  
**Primary pattern:** fintech risk  
**Deep-dive focus:** jurisdiction rules, versioning, audit, latency
...
```text
tax calculation service Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the tax calculation service boundary.
...
| Core domain services | Public API and auth gateway, Ledger service, Command/orchestration service, Risk/fraud service | Own tax calculation service business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **jurisdiction rules**: Version jurisdiction tax rules and evaluate by address, product, and date.
...
- **Hot path clarity:** start from `jurisdiction rules` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `jurisdiction rules`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `jurisdiction rules`.
