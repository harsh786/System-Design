# Design Google Search at a high level - System Design Deep Dive

**Problem #41**  
**Category:** Search  
**Primary pattern:** search geo  
**Deep-dive focus:** crawling, indexing, ranking, freshness, serving

## 0. Interview Framing

A retrieval system where ingestion, indexing, ranking, freshness, geospatial constraints, and low-latency serving are the core concerns.

In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **crawling**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Ingest source documents or entities.
- Build serving indexes and feature records.
- Serve low-latency ranked results.
- Log feedback for quality and freshness loops.
...
- high relevance with predictable tail latency.
- freshness guarantees for critical updates.
- graceful degradation when rankers or personalization fail.
- safe crawling/ingestion and tenant-aware result filtering.
...
- Do not rank directly from the primary OLTP database.
- Do not assume index freshness equals source-of-truth correctness.
...
|---|---|
| Active users | 30M DAU, 150M MAU |
| Queries | 500M/day, 10x peak after cache misses |
| Indexed corpus | 100M to 100B documents/places/items |
| Index freshness | seconds to minutes depending on source |
| Latency target | p99 < 250 ms end-to-end query latency |
...
```http
GET /v1/search-documents/{id}
Authorization: Bearer <token>
```

```http
GET /v1/search-documents?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```

```http
PATCH /v1/search-documents/{id}
Idempotency-Key: <uuid>
Content-Type: application/json
...
```http
POST /v1/search-documents/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
{
  "query": "user text or structured query",
...
```http
POST /v1/search-documents/index-operations
Idempotency-Key: <uuid>

{"operation": "upsert|delete", "document_id": "doc_123", "source_version": 42}
```

```http
GET /v1/search/suggestions?prefix=iph&limit=10
```
...
service SearchDocumentService {
  rpc CreateSearchDocument(CreateSearchDocumentRequest) returns (SearchDocument);
  rpc GetSearchDocument(GetSearchDocumentRequest) returns (SearchDocument);
  rpc UpdateSearchDocument(UpdateSearchDocumentRequest) returns (SearchDocument);
  rpc EvaluateSearchDocument(EvaluateSearchDocumentRequest) returns (EvaluateSearchDocumentResponse);
  rpc ListSearchDocumentEvents(ListSearchDocumentEventsRequest) returns (stream SearchDocumentEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "search.documents.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "search-documents-service",
  "tenant_id": "tenant_123",
  "aggregate_id": "searchDocument_123",
...
- `search.documents.created.v1`
- `search.documents.updated.v1`
- `search.documents.state_changed.v1`
- `search.documents.deleted_or_expired.v1`
- `search.documents.policy_denied.v1`
- `search.documents.operation_failed.v1`
...
```text
Google Search at a high level Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Retrieval service -> Ranking service -> Personalization/feature service -> Normalizer and deduper -> Query parser -> Result cache
        |       -> Source-of-Truth Write + Transactional Outbox
        |
        +--> Query / Serving Path
        |       -> Query parser / Retrieval service / Ranking service / Result cache
        |       -> Canonical Store fallback when strong freshness is required
        |
        +--> Async/Event Path
        |       -> Event Bus / Stream Processing / Workflow Queues
        |       -> Ingestion/crawler pipeline / Indexer
        |
        +--> Operations Path
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Google Search at a high level service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Retrieval service -> Ranking service -> Personalization/feature service -> Normalizer and deduper -> Query parser -> Result cache; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
3. **Query path:** read-heavy traffic is served by Query parser / Retrieval service / Ranking service / Result cache; strong reads fall back to the canonical store when stale projections are unsafe.
4. **Async path:** Ingestion/crawler pipeline / Indexer consume committed events for notifications, indexing, analytics, provider calls, ML/risk feedback, cleanup, and reconciliation.
...
- **Ingestion/crawler pipeline**: Creates upload sessions, validates chunks/checksums, writes originals to object storage, records metadata, and emits processing events.
- **Normalizer and deduper**: Canonicalizes raw input, extracts structured fields, removes duplicates, assigns stable content/entity IDs, and preserves lineage to raw records.
- **Indexer**: Builds and serves query indexes from canonical events, supports ranking/filtering/facets, and tolerates bounded staleness.
- **Query parser**: Parses user/API queries, validates syntax, applies rewrites, filters unsafe terms, and produces an executable retrieval plan.
- **Retrieval service**: Fetches candidates from indexes or serving stores, applies filters/freshness constraints, and returns bounded candidate sets for ranking or response assembly.
- **Ranking service**: Generates/ranks candidates using features, business rules, freshness constraints, online feedback, and experimentation controls.
- **Personalization/feature service**: Serves model/features with versioning, freshness checks, rollout controls, explainability, and offline/online parity.
- **Result cache**: Serves hot derived data with TTLs/invalidation, protects backing stores, and never becomes source of truth for critical state.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own Google Search at a high level business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Query parser, Retrieval service, Ranking service, Result cache | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
| Async workers and integrations | Ingestion/crawler pipeline, Indexer | Consume committed events, call external systems, retry safely, update projections, run cleanup, and isolate slow dependencies from user-facing latency. |
...
- Raw document/object store.
- Inverted/vector/geospatial indexes.
- Feature store for ranking.
- Query log stream.
- Cache for popular queries and tiles.

## 6. Low-Level Design

### Core Modules

- `SearchDocumentController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `SearchDocumentApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `SearchDocumentDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `SearchDocumentRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `SearchDocumentPolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `SearchDocumentEventPublisher`: writes outbox records and publishes domain events to the broker.
- `SearchDocumentReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface SearchDocumentRepository {
    Optional<SearchDocument> findById(SearchDocumentId id, ReadConsistency consistency);
    Page<SearchDocument> list(SearchDocumentQuery query, Cursor cursor, int limit);
    SearchDocument save(SearchDocument aggregate, ExpectedVersion expectedVersion);
}

interface SearchDocumentPolicyEvaluator {
    PolicyDecision canRead(Principal principal, SearchDocument resource);
    PolicyDecision canMutate(Principal principal, SearchDocument resource, Action action);
}
...
```text
Tenant/User
  -> owns or can access -> SearchDocument
  -> emits -> SearchDocumentEvent
...
|---|---|
| `documents` | `id, source_id, canonical_url, content_hash, acl_ref, freshness_ts, state` |
| `index_segments` | `id, index_name, version, shard_id, object_ref, doc_count, created_at` |
| `queries` | `request_id, user_id, query_text_hash, filters_json, latency_ms, result_count, ts` |
| `ranking_features` | `entity_id, feature_version, feature_vector_ref, updated_at` |
| `feedback_events` | `request_id, user_id, entity_id, event_type, position, ts` |
...
- **crawling**: Maintain frontier priority, robots compliance, dedupe, and retry/backoff state.
- **indexing**: Build immutable index segments and atomically publish versions.
- **ranking**: Separate candidate generation from ranking and keep a deterministic fallback ranker.
- **freshness**: Expose data freshness as a product metric and SLO.
- **serving**: Keep query serving stateless over replicated index shards with result caching.
...
- index build and compaction compute.
- ranking CPU/GPU inference.
- query cache memory.
- geospatial or vector index fanout.
...
- **Hot path clarity:** start from `crawling` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Ingestion/crawler pipeline, Normalizer and deduper, Indexer, Query parser, Retrieval service, Ranking service; avoid vague boxes that do not own data or decisions.
...
- **Event model:** use `search.documents.created.v1, search.documents.updated.v1, search.documents.state_changed.v1, search.documents.deleted_or_expired.v1, search.documents.policy_denied.v1, search.documents.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `crawling`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `crawling`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Ingestion/crawler pipeline, Normalizer and deduper, Indexer, Query parser, Retrieval service, Ranking service.
