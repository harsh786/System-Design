# Design product search for Amazon - System Design Deep Dive

**Problem #44**  
**Category:** Search/commerce  
**Primary pattern:** search geo  
**Deep-dive focus:** inverted index, facets, ranking, availability, personalization
...
```text
product search for Amazon Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the product search for Amazon service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own product search for Amazon business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **inverted index**: Make inverted index explicit in the data model, APIs, scaling plan, and operational dashboards.
- **facets**: Precompute or index facets and maintain filter-count latency budgets.
- **ranking**: Separate candidate generation from ranking and keep a deterministic fallback ranker.
- **availability**: Filter unavailable inventory before ranking or clearly mark stale availability.
- **personalization**: Merge global popularity with user/context features behind privacy controls.
...
- **Hot path clarity:** start from `inverted index` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `inverted index`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `inverted index`.
