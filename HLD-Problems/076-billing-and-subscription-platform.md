# Design Billing And Subscription Platform - System Design Deep Dive

**Problem #76**  
**Category:** SaaS/fintech  
**Primary pattern:** subscription lifecycle + usage metering + invoicing  
**Deep-dive focus:** plans, entitlements, metering, proration, invoices, dunning

## 0. Interview Framing

Billing converts plans, entitlements, usage, taxes, invoices, payments, credits, and retries into auditable customer lifecycle.

Actors: customers, admins, metering producers, finance, payment providers. The highest-risk path is **subscription change -> entitlement -> usage -> invoice -> payment/dunning**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Create plans, prices, coupons, trials, subscriptions, add-ons, seats, entitlements, and schedules.
- Ingest usage, aggregate meters, apply tiers/minimums/overages, and generate invoices.
- Support upgrades, downgrades, proration, cancellation, renewal, credits, tax, refund, and dunning.
- Export invoices, revenue schedules, tax reports, and accounting data.
- Expose support/admin operations to inspect, replay, correct, and annotate subscription history.
...
- Invoice totals are reproducible from versioned plan/price/usage/tax/credit snapshots.
- Usage ingestion is at-least-once and deduped by source event ID.
- Payment retry does not duplicate invoice or charge.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Subscription and invoice records plus immutable invoice lines; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Customers | 1M accounts |
| Usage events | 1B/day |
| Invoices | 10M/month |
| Payment retries | Multiple per failed invoice |
| Retention | Finance/tax multi-year |
...
- Keep the online subscription aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/subscriptions | customer_id, plan_id, items, trial | Create subscription. |
| POST /v1/usage-events | source_event_id, customer_id, meter_id, qty | Meter usage. |
| POST /v1/invoices/{id}/finalize | expected_version | Finalize invoice. |
| POST /v1/invoices/{id}/pay | payment_method_id | Collect payment. |
| GET /v1/subscriptions/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/subscriptions/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service BillingService {
  rpc Create(CreateBillingRequest) returns (Billing);
  rpc Get(GetBillingRequest) returns (Billing);
  rpc Transition(TransitionBillingRequest) returns (Billing);
  rpc Reconcile(ReconcileBillingRequest) returns (ReconcileResult);
}
...
- billing.created.v1
- billing.validated.v1
- billing.state_changed.v1
- billing.committed.v1
- billing.failed.v1
- billing.reversed.v1
- billing.reconciliation_completed.v1
- billing.manual_review_required.v1
...
```text
Billing And Subscription Platform Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Idempotency service -> Subscription billing orchestrator -> State transition engine -> Entitlement service -> Metering service -> Dunning service
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Billing And Subscription Platform service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Idempotency service -> Subscription billing orchestrator -> State transition engine -> Entitlement service -> Metering service -> Dunning service; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
...
- **Entitlement service**: projects subscription into product access.
- **Metering service**: dedupes and aggregates usage.
- **Invoice engine**: generates lines from subscription, usage, credits, taxes.
- **Dunning service**: retries, reminders, grace period, access changes.
...
| Core domain services | Idempotency service, Subscription billing orchestrator, State transition engine, Entitlement service, Metering service, Dunning service | Own Billing And Subscription Platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **Subscription billing orchestrator**: owns subscription state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed subscription transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Billing And Subscription Platform: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| customers | customer_id, billing_profile, tax_info_ref | Customer. |
| subscriptions | subscription_id, customer_id, plan_id, state, period | Subscription. |
| subscription_items | item_id, subscription_id, price_id, quantity | Seats/add-ons/meters. |
| usage_events | source_event_id, customer_id, meter_id, qty | Raw usage. |
| invoices | invoice_id, customer_id, period, state, totals | Invoice. |
| invoice_lines | line_id, invoice_id, source_ref, amount, tax | Charge line. |
...
1. Subscription change freezes new plan/price and updates entitlement projection.
2. Usage ingestion dedupes and aggregates into billing buckets.
3. Invoice run computes recurring charges, usage, credits, taxes, and finalizes immutable invoice.
4. Payment failure triggers retry, dunning, and grace-period policy.
...
- Entitlement state is separate from payment state.
- Finalized invoice is immutable; corrections use credit notes.
- Late usage needs explicit window policy.
- Source-of-truth boundary: Subscription and invoice records plus immutable invoice lines owns correctness; all low-latency read models are disposable projections.
...
- Protect billing data, payment tokens, tax IDs, invoices, and admin credits.
- Detect trial abuse, coupon abuse, payment loops, account takeover, and invoice tampering.
...
- SLIs: subscription mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do usage duplicates work?
- How do prorations work?
- How do entitlements react to failed payment?
- Where exactly is subscription source of truth stored?
...
- **Correctness boundary:** Subscription and invoice records plus immutable invoice lines is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `subscription change -> entitlement -> usage -> invoice -> payment/dunning` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Entitlement service, Metering service, Invoice engine, Dunning service, Idempotency service, Outbox/inbox workers; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `customers, subscriptions, subscription_items, usage_events, invoices, invoice_lines` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `billing.created.v1, billing.validated.v1, billing.state_changed.v1, billing.committed.v1, billing.failed.v1, billing.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `subscription change -> entitlement -> usage -> invoice -> payment/dunning`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `subscription change -> entitlement -> usage -> invoice -> payment/dunning`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Subscription billing orchestrator, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
