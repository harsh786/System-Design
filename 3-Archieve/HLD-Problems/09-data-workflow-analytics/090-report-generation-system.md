# Design report generation system - System Design Deep Dive

**Problem #90**  
**Category:** Batch/product  
**Primary pattern:** data scheduling  
**Deep-dive focus:** scheduling, templates, long-running jobs, delivery, retries
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **scheduling**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the report generation system service boundary.
...
| Core domain services | Dispatcher/queue, State/checkpoint service, Control-plane API, Result serving API, Alerting and retry manager, Catalog/lineage service | Own report generation system business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **templates**: Version report templates and isolate rendering from scheduling.
- **long-running jobs**: Checkpoint progress and expose cancellation, retry, and partial-output semantics.
- **delivery**: Track delivery state across email, storage, and web download targets.
...
- **Hot path clarity:** start from `scheduling` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `scheduling`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `scheduling`.
