# Design log search like Splunk/ELK - System Design Deep Dive

**Problem #50**  
**Category:** Search/observability  
**Primary pattern:** search geo  
**Deep-dive focus:** ingestion, indexing, retention, query latency
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **ingestion**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
log search like Splunk/ELK Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the log search like Splunk/ELK service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own log search like Splunk/ELK business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **ingestion**: Separate ingestion durability from indexing or serving freshness.
...
- **query latency**: Bound shard fanout, use top-k merge, and cache popular queries.
...
- **Hot path clarity:** start from `ingestion` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `ingestion`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `ingestion`.
