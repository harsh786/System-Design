# Design Ads Serving Platform - System Design Deep Dive

**Problem #109**  
**Category:** Ads/ML/realtime  
**Primary pattern:** auction + targeting + budget pacing + attribution  
**Deep-dive focus:** candidate selection, targeting, auction, pacing, frequency caps, attribution

## 0. Interview Framing

Ad serving is a low-latency marketplace between advertiser budgets and user attention.

Actors: users, publishers, advertisers, campaign managers. The highest-risk path is **ad request -> candidates -> auction -> budget/frequency commit -> impression/click**. Keep the design centered there; move secondary work async unless it affects correctness, money, privacy, abuse prevention, or user-visible availability.

## 1. Requirements

### Functional Requirements

- Advertisers create campaigns, ad groups, creatives, targeting, bids, budgets, pacing, and goals.
- Publishers/apps request ads with placement, context, device, privacy, and user signals.
- Serve via candidate retrieval, filtering, auction/ranking, budget reservation, frequency cap, and creative response.
- Track impressions, clicks, conversions, billing, reporting, and attribution.
- Expose support/admin operations to inspect, replay, correct, and annotate ad_impression history.
...
- Decision p99 often below 50-100 ms.
- Budget and frequency caps should not materially overspend.
- Consent/privacy constraints filter targeting and attribution.
- Every mutating API is idempotent and accepts an expected version or equivalent conflict guard.
- Source of truth: Budget/frequency counters plus immutable ad_events; caches, search indexes, dashboards, and read models are rebuildable projections.
...
| --- | --- |
| Ad requests | 1M+/sec global |
| Campaigns | Millions active |
| Events | Billions/day |
| Latency | Tens of ms at edge |
| Reporting | Realtime plus billing-grade batch |
...
- Keep the online ad_impression aggregate compact; move large payloads, documents, telemetry, and raw reports to object storage.
...
| --- | --- | --- |
| POST /v1/ad-requests | placement, context, user_signals, consent | Serve ad. |
| POST /v1/campaigns | budget, targeting, bid, creatives | Create campaign. |
| POST /v1/ad-events | request_id, type, timestamp | Impression/click/conversion. |
| GET /v1/reports/campaigns | campaign_id, date_range | Reporting. |
| GET /v1/ad-requests/{id} | path id, caller identity | Read canonical state or authorized projection. |
| POST /v1/admin/ad-requests/{id}/reconcile | scope, dry_run, reason | Operator reconciliation with audit trail. |

### Internal APIs

```protobuf
service AdServingService {
  rpc Create(CreateAdServingRequest) returns (AdServing);
  rpc Get(GetAdServingRequest) returns (AdServing);
  rpc Transition(TransitionAdServingRequest) returns (AdServing);
  rpc Reconcile(ReconcileAdServingRequest) returns (ReconcileResult);
}
...
{
  "event_id": "evt_01H...",
  "event_type": "ad_serving.state_changed.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "ad_serving-service",
  "tenant_or_region_id": "region_123",
  "aggregate_id": "ad_impression_123",
...
- ad_serving.created.v1
- ad_serving.validated.v1
- ad_serving.state_changed.v1
- ad_serving.committed.v1
- ad_serving.failed.v1
- ad_serving.reversed.v1
- ad_serving.reconciliation_completed.v1
- ad_serving.manual_review_required.v1
...
```text
Ads Serving Platform Architecture
...
API Gateway / WAF / Auth / Rate Limits / Request Router
        ...
        |       -> Idempotency service -> State transition engine -> Campaign service -> Ad decision service -> Ad serving edge -> Budget/pacing
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the Ads Serving Platform service boundary.
2. **Synchronous command path:** correctness-sensitive mutations stay inside Idempotency service -> State transition engine -> Campaign service -> Ad decision service -> Ad serving edge -> Budget/pacing; this path performs validation, idempotency checks, source-of-truth writes, and outbox publication.
3. **Query path:** read-heavy traffic is served by Ad serving edge; strong reads fall back to the canonical store when stale projections are unsafe.
...
- **Campaign service**: config, budgets, targeting, creatives, review.
- **Ad serving edge**: candidate retrieval and auction close to user.
- **Budget/pacing**: reserves spend and smooths delivery.
- **Frequency cap**: tracks exposure under privacy limits.
- **Attribution**: impression/click/conversion/billing pipeline.
...
| Core domain services | Idempotency service, State transition engine, Campaign service, Ad decision service, Ad serving edge, Budget/pacing | Own Ads Serving Platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
| Query/serving services | Ad serving edge | Serve low-latency reads from caches, read models, indexes, or specialized serving stores while exposing freshness/consistency guarantees. |
...
- **Ad decision service**: owns ad_impression state machine, policy checks, outbox events, and compensation.
- **Adapter manager**: normalizes provider/partner callbacks and retries.
- **State transition engine**: validates allowed ad_impression transitions using expected_version, policy decisions, and source-of-truth reads.
...
- Ads Serving Platform: CREATED -> VALIDATED -> PENDING -> COMMITTED -> RECONCILED.
...
| --- | --- | --- |
| campaigns | campaign_id, advertiser_id, objective, status | Campaign. |
| ad_groups | ad_group_id, targeting, bid, budget | Auction unit. |
| creatives | creative_id, review_state, asset_ref | Creative. |
| budget_counters | campaign_id, window, spent, reserved | Budget. |
| frequency_caps | user_or_cohort_key, campaign_id, count | Cap. |
| ad_events | event_id, request_id, type, price | Event. |
...
For this design, use Redis/Aerospike/local cache for serving-time candidates and caps, Kafka for impression/click/conversion events, Flink for attribution/pacing/fraud streams, S3 + Iceberg for event lake, and Pinot/ClickHouse/Druid for near-real-time advertiser reporting.
...
1. Ad request validates consent, retrieves candidates, filters, auctions, reserves budget/cap, and returns creative.
2. Impression confirms billable event and finalizes reserved budget.
3. Click/conversion dedupe, fraud score, attribute, and bill.
4. Pacing adjusts bid multiplier/eligibility.
...
- Budget reservation is approximate high throughput plus reconciliation.
- Auction model can be first-price, second-price, or optimized bid.
- Privacy can force contextual targeting and aggregated attribution.
- Source-of-truth boundary: Budget/frequency counters plus immutable ad_events owns correctness; all low-latency read models are disposable projections.
...
- Protect user identifiers and respect consent, opt-out, retention.
- Detect click fraud, impression fraud, bot traffic, creative malware, and publisher fraud.
...
- SLIs: ad_impression mutation success rate, read latency, transition error rate, idempotency replay rate, queue lag, and reconciliation mismatch rate.
...
- How do you enforce budget concurrently?
- How do frequency caps work with privacy?
- What is online vs offline attribution?
- Where exactly is ad_impression source of truth stored?
...
- **Correctness boundary:** Budget/frequency counters plus immutable ad_events is the authority; derived caches, search indexes, dashboards, and analytics must be rebuildable.
- **Hot path clarity:** start from `ad request -> candidates -> auction -> budget/frequency commit -> impression/click` and walk the synchronous command path before discussing secondary features.
- **Service ownership:** explicitly assign responsibilities to Campaign service, Ad serving edge, Budget/pacing, Frequency cap, Attribution, Idempotency service; avoid vague boxes that do not own data or decisions.
- **Data ownership:** ground the design in `campaigns, ad_groups, creatives, budget_counters, frequency_caps, ad_events` and explain partitioning, indexes, retention, and replay.
- **Event model:** use `ad_serving.created.v1, ad_serving.validated.v1, ad_serving.state_changed.v1, ad_serving.committed.v1, ad_serving.failed.v1, ad_serving.reversed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `ad request -> candidates -> auction -> budget/frequency commit -> impression/click`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `ad request -> candidates -> auction -> budget/frequency commit -> impression/click`.
- Minutes 2-3: draw edge, command path, query path, async path, and data stores; name Ad decision service, Adapter manager, State transition engine, Reconciliation worker, Projection builder.
