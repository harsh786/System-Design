# Design a content moderation pipeline - System Design Deep Dive

**Problem #40**  
**Category:** Trust/safety  
**Primary pattern:** media content  
**Deep-dive focus:** ML review, human review, queues, appeals, audit
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **ML review**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
a content moderation pipeline Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the content moderation pipeline service boundary.
...
| Core domain services | Upload/ingest service, Metadata service | Own a content moderation pipeline business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **ml review**: Use ML scores to prioritize and automate review, but keep audit and appeal paths.
- **human review**: Design queues, sampling, reviewer permissions, and quality checks.
- **queues**: Expose wait time, priority, assignment, and escalation state as first-class data.
- **appeals**: Track appeal state separately from initial moderation state.
...
- **Hot path clarity:** start from `ML review` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `ML review`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `ML review`.
