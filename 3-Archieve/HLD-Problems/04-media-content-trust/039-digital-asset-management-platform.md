# Design a digital asset management platform - System Design Deep Dive

**Problem #39**  
**Category:** Enterprise media  
**Primary pattern:** media content  
**Deep-dive focus:** metadata, access control, versioning, previews, search
...
In an interview, start by narrowing the product scope, then anchor the design around the highest-risk path. For this problem, the highest-risk path is usually the path involving **metadata**. Keep secondary capabilities asynchronous unless they affect correctness, money, privacy, or user-visible availability.
...
```text
a digital asset management platform Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the digital asset management platform service boundary.
...
| Core domain services | Upload/ingest service, Metadata service | Own a digital asset management platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
For this design, use object storage/CDN for media bytes, Kafka for asset lifecycle events, Flink/Spark for processing telemetry, batch workers for transcoding/backfills, Iceberg tables on S3 for playback analytics, and ClickHouse/Pinot/Druid for real-time creator/operator dashboards.
...
- **access control**: Evaluate identity, object ACL, tenant boundary, and policy at every metadata and data path.
- **versioning**: Version mutable assets/configs and keep rollback paths explicit.
- **previews**: Generate previews asynchronously and treat them as derived assets.
...
- **Hot path clarity:** start from `metadata` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `metadata`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `metadata`.
