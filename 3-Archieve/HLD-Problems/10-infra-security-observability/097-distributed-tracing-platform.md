# Design distributed tracing platform - System Design Deep Dive

**Problem #97**  
**Category:** Observability  
**Primary pattern:** infra security  
**Deep-dive focus:** trace ingestion, sampling, correlation, query
...
```http
GET /v1/traces?cursor=<cursor>&limit=50&filter=<filter>&sort=<sort>
Authorization: Bearer <token>
```
...
```http
POST /v1/traces/{id}/actions/{action}
Idempotency-Key: <uuid>
Content-Type: application/json
...
service TraceService {
  rpc CreateTrace(CreateTraceRequest) returns (Trace);
  rpc GetTrace(GetTraceRequest) returns (Trace);
  rpc UpdateTrace(UpdateTraceRequest) returns (Trace);
  rpc EvaluateTrace(EvaluateTraceRequest) returns (EvaluateTraceResponse);
  rpc ListTraceEvents(ListTraceEventsRequest) returns (stream TraceEvent);
}
```
...
{
  "event_id": "evt_01H...",
  "event_type": "traces.updated.v1",
  "occurred_at": "2026-05-25T10:15:30Z",
  "producer": "traces-service",
...
- `traces.created.v1`
- `traces.updated.v1`
- `traces.state_changed.v1`
- `traces.deleted_or_expired.v1`
- `traces.policy_denied.v1`
- `traces.operation_failed.v1`
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the distributed tracing platform service boundary.
...
| Core domain services | Propagation service, Policy engine, Versioned config store, Data-plane evaluator or agent, SRE console | Own distributed tracing platform business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- `TraceController`: validates requests, extracts identity, enforces coarse rate limits, and maps errors to API responses.
- `TraceApplicationService`: owns use-case orchestration, idempotency, retries, and state transitions.
- `TraceDomainModel`: contains state machine rules, validation, and invariants that must not leak into controllers.
- `TraceRepository`: hides database access, optimistic locking, partitioning, and pagination details.
- `TracePolicyEvaluator`: centralizes authorization, tenant isolation, quota, and abuse decisions.
- `TraceEventPublisher`: writes outbox records and publishes domain events to the broker.
- `TraceReadModelProjector`: builds caches, search documents, counters, and dashboard projections asynchronously.

### Interfaces

```java
interface TraceRepository {
    Optional<Trace> findById(TraceId id, ReadConsistency consistency);
    Page<Trace> list(TraceQuery query, Cursor cursor, int limit);
    Trace save(Trace aggregate, ExpectedVersion expectedVersion);
}

interface TracePolicyEvaluator {
    PolicyDecision canRead(Principal principal, Trace resource);
    PolicyDecision canMutate(Principal principal, Trace resource, Action action);
}
...
|---|---|
| `traces` | `trace_id, tenant_id, root_service, root_operation, start_time, duration_ms, sampled, status` |
| `spans` | `trace_id, span_id, parent_span_id, service, operation, start_time, duration_ms, tags_ref` |
| `service_dependencies` | `tenant_id, from_service, to_service, edge_stats_ref, window_start` |
| `sampling_policies` | `id, tenant_id, service, operation, rate, rules_json, state` |
| `trace_indexes` | `tenant_id, time_bucket, service, operation, trace_id, status` |
...
- **trace ingestion**: Separate ingestion durability from indexing or serving freshness.
- **sampling**: Sample traces adaptively while retaining errors and high-value traffic.
- **correlation**: Propagate trace IDs, span IDs, tenant IDs, and deployment versions.
- **query**: Index by time, service, operation, trace ID, and tags to bound query fanout.
...
- **Hot path clarity:** start from `trace ingestion` and walk the synchronous command path before discussing secondary features.
...
- **Event model:** use `traces.created.v1, traces.updated.v1, traces.state_changed.v1, traces.deleted_or_expired.v1, traces.policy_denied.v1, traces.operation_failed.v1` to decouple slow work while preserving idempotency and ordering per aggregate.
...
- Which service owns the final decision for `trace ingestion`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `trace ingestion`.
