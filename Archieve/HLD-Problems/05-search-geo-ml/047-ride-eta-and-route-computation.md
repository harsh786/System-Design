# Design ride ETA and route computation - System Design Deep Dive

**Problem #47**  
**Category:** Geo/realtime  
**Primary pattern:** search geo  
**Deep-dive focus:** graph routing, traffic updates, caching, freshness
...
```text
ride ETA and route computation Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the ride ETA and route computation service boundary.
...
| Core domain services | Retrieval service, Ranking service, Personalization/feature service, Normalizer and deduper, Query parser, Result cache | Own ride ETA and route computation business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **graph routing**: Use precomputed shortcuts/tiles and dynamic overlays for traffic incidents.
- **traffic updates**: Represent traffic as time-windowed edge weights with freshness metadata.
- **caching**: Cache by stable keys and define invalidation or TTL for freshness-sensitive data.
...
- **Hot path clarity:** start from `graph routing` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `graph routing`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `graph routing`.
