# Design a file upload service for large files - System Design Deep Dive

**Problem #57**  
**Category:** Web/storage  
**Primary pattern:** storage sync  
**Deep-dive focus:** multipart upload, resumability, checksums, malware scanning
...
```text
a file upload service for large files Architecture
...
1. **Request entry:** actors enter through edge controls that authenticate, authorize, rate-limit, route, and attach trace context before reaching the file upload service for large files service boundary.
...
| Core domain services | Upload/download gateway, Metadata service, Chunk/object service, Sync/change service, Permission service, Virus/DLP scanning service | Own a file upload service for large files business invariants, source-of-truth writes, state transitions, idempotency, and synchronous API responses. |
...
- **resumability**: Use upload sessions, byte ranges, idempotent part commits, and checksums.
- **checksums**: Validate checksums at client, edge, and storage commit; repair corrupt replicas.
- **malware scanning**: Quarantine uploads until scan status permits sharing/download.
...
- **Hot path clarity:** start from `multipart upload` and walk the synchronous command path before discussing secondary features.
...
- Which service owns the final decision for `multipart upload`, and what exact write makes it durable?
...
- First minute: scope actors, constraints, and `multipart upload`.
